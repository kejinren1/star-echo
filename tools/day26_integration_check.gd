...
func _part_regression() -> void:
	...
	# 元数据同步（2026-08-18 · TEST_REPORT #57/#58/#59 挂 #3 观察项）：day31_presentation expect 246→261（+15，实测断言数）
	#   → 61 项 / 期望 1504（1489 + 15）
	# 总指挥第 5 轮同步（2026-08-18）：day31_presentation expect 261→273（+12，F1-E-3 BGM/SFX +§3 audio 段）
	#   + runner +day31_flee_bound(18)（F-44 逃逸与边界）
	#   → 62 项 / 期望 1534（1504 + 12 + 18）
	# F-46 同步（2026-08-18 执行者第 62 轮）：day31_flee_bound 18→22（+4 §5 leash）+ day18_feedback 16→17（+1 §4 分数制）
	...
	# LEVEL_DESIGN LD-A 同步（2026-08-19 #3 执行）：runner +day31_level_design_data_check(24)
	...
	# F1-E-5 同步（2026-08-19 #3 执行 · 批五补落地）：day31_presentation expect 334→349（+15，§10 icon 段）
	...
	if expect_sum == 1677:
		_pass("回归 / 期望断言合计 1677（1662 + F1-E-5 §10 icon 15）")
	else:
		_fail("回归: 期望合计 %d ≠ 1677" % expect_sum)
	# 关键探针 load 抽样
