		...
		for l in lines:
			...
			if t.begins_with("(\"day"):
				...
				if last.is_valid_int():
					...
					probe_count += 1
	if probe_count == 23:
		_pass("回归 / _regression_run.py PROBES 23 项（24 件套 = 23 探针 + baseline）")
	else:
		_fail("回归: PROBES 项数 %d ≠ 23" % probe_count)
	if expect_sum == 609:
		_pass("回归 / 期望断言合计 609（Day 24 收口基准）")
	else:
		_fail("回归: 期望合计 %d ≠ 609" % expect_sum)
	# 关键探针 load 抽样
