#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_v11_batch.py — v11 修正批量（用户 08-16 00:22 反馈）
艾琳继续抽卡×6 / 诺亚科研服融入古典维多利亚风 / Boss5 狼重绘+伤口
Boss4 白骑士已定稿（本批不跑）
"""
...
NOAH_CLASSIC = ("short silver blue hair, blue eyes, "
                "rune-engraved vintage brass goggles with leather strap on forehead, "
                "vintage arcane scholar, classic victorian scholarly outfit, "
                "antique dark blue tailcoat with rune embroidery over white shirt, leather vest, "
                "neat striped trousers, high heel boots, brass pocket watch on chain, "
                "holding a small wrench, magicpunk researcher, elegant classical professional look, "
...
def main():
    ...
    seed = 20261301
    for k in range(6):
        tasks.append((f"v11_elin_mage_{k+1}", ELIN_MAGE, seed + k))
    BODY = [
    ...
    for bi, (bkey, btag) in enumerate(BODY):
        tasks.append((f"v11_noah_{bkey}", f"{btag}, {NOAH_CLASSIC}", seed + 30 + bi * 10))
    tasks.append(("v11_boss_05_wolf",
                  ...
                  "fresh wounds and scars on body, gash marks, battle injuries, "
