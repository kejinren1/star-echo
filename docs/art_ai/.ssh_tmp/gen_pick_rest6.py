...
CHARS = {
    "b04": ("M", "human",
             "1boy, adult man in his mid twenties, mature masculine face, "
             "slim lean athletic build, tall body, eight head tall, long limbs, "
             "golden blonde hair, sharp blue eyes, warm tan skin tone, handsome mature face, "
...
def main():
    ...
    for i, (key, (grp, kind, feat)) in enumerate(CHARS.items()):
        ...
        cnt = 6 if key == "b07" else 4
        for n in range(cnt):
            weight = 0.3
            seed = seed_base + i * 10 + n
            prefix = f"rest6_{key}_{n+1}"
            wf = {
