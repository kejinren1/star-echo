#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""process_v9.py — v9 批量 perfectPixel 精修 + 拼版"""
import os, sys
import numpy as np
...
OUT = r"D:/30DAYS/docs/art_ai/output_abc/final_完美像素/v9_20260816"
...
elin_items = []
for k in [1, 2, 3]:
    p = os.path.join(SRC, f"v9_elin_mage_{k}.png")
    arr = np.array(Image.open(p).convert("RGBA"))
    perfect(f"v9_elin_mage_{k}", arr, f"elin_mage_{k}")
    elin_items.append((f"艾琳·魔法师#{k}", os.path.join(OUT, f"elin_mage_{k}", "96px.png")))
g = grid_view(elin_items, 3, 420, 24)
g.save(os.path.join(OUT, "筛选_艾琳_魔法师_v9.png"))
print("艾琳 v9", flush=True)

BODY_ZH = {"mature": "御姐", "youth": "青年", "loli": "少女"}
noah_items = []
for bkey in ["mature", "youth", "loli"]:
    p = os.path.join(SRC, f"v9_noah_{bkey}.png")
    arr = np.array(Image.open(p).convert("RGBA"))
    perfect(f"v9_noah_{bkey}", arr, f"noah_{bkey}")
    noah_items.append((f"诺亚·{BODY_ZH[bkey]}", os.path.join(OUT, f"noah_{bkey}", "96px.png")))
g = grid_view(noah_items, 3, 420, 24)
g.save(os.path.join(OUT, "筛选_诺亚_v9.png"))
print("诺亚 v9", flush=True)

boss_items = [
    ("v9_boss_04_light_knight", "④白蓝金骑士v9"),
    ("v9_boss_05_wolf", "⑤长尾狼兽v9"),
]
items = []
...
g.save(os.path.join(OUT, "筛选_Boss_v9.png"))
print("Boss v9", g.size, flush=True)
