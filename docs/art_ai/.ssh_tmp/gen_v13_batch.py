#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_v13_batch.py — v13 修正批量（用户 08-16 00:32 反馈）
狼重绘×4：纯粹野兽（纯四足动物形态，去人形狼人/兽人元素）
艾琳已定稿（本批不跑）
"""
...
WOLF_BEAST = ("feral wolf beast, pure quadrupedal animal form, natural wolf anatomy, "
              "not anthropomorphic, not bipedal, not humanoid, "
              "lean slender gaunt body, elongated slender legs, "
              "purple-black fur color scheme, long prominent twisted tail raised, "
              "blood-red eyes, ferocious snarling expression, sharp claws, "
...
def main():
    ...
    seed = 20261401
    for k in range(4):
        tasks.append((f"v13_wolf_{k+1}", WOLF_BEAST, seed + k))
