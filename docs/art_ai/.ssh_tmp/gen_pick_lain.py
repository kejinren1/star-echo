...
EYE_M = "detailed eyes, detailed iris, natural eyelashes, elegant mature face, detailed face, sharp facial features"
...
CHARS = {
    "lain": ("M", "human",
             ...
             "simple blue-tinted armor, uniform blue armor color, consistent blue tone, "
...
def main():
    ...
    for i, (key, (grp, kind, feat)) in enumerate(CHARS.items()):
        eye = {"F": EYE_M, "M": EYE_M, "B": EYE_B, "I": EYE_I}[grp]
        if kind == "human":
            pose = POSEY
        elif kind == "animal":
        ...
        neg = (NEG + ", long eyelashes, feminine face, pretty boy, "
        ...
        for n in range(6):
            seed = seed_base + i * 6 + n
            prefix = f"pick_lain5_{n+1}"
            wf = {
                ...
                "9": {"class_type": "VAEEncode", "inputs": {"pixels": ["8", 0], "vae": ["2", 2]}},
                "10": {"class_type": "KSampler", "inputs": {"seed": seed + 5000, "steps": 34, "cfg": 7.0,
                      "sampler_name": "dpmpp_2m_sde", "scheduler": "karras", "denoise": 0.35,
                      "model": ["3", 0], "positive": ["4", 0], "negative": ["5", 0], "latent_image": ["9", 0]}},
