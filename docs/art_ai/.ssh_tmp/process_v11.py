#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""process_v11.py — v11 批量 perfectPixel 精修 + 拼版"""
import os, sys
import numpy as np
...
OUT = r"D:/30DAYS/docs/art_ai/output_abc/final_完美像素/v11_20260816"
...
for k in [1, 2, 3, 4, 5, 6]:
    p = os.path.join(SRC, f"v11_elin_mage_{k}.png")
    arr = np.array(Image.open(p).convert("RGBA"))
    perfect(f"v11_elin_mage_{k}", arr, f"elin_mage_{k}")
    elin_items.append((f"艾琳·#{k}", os.path.join(OUT, f"elin_mage_{k}", "96px.png")))
g = grid_view(elin_items, 3, 420, 24)
g.save(os.path.join(OUT, "筛选_艾琳_魔法师_v11.png"))
print("艾琳 v11 6 张", flush=True)

BODY_ZH = {"mature": "御姐", "youth": "青年", "loli": "少女"}
noah_items = []
for bkey in ["mature", "youth", "loli"]:
    p = os.path.join(SRC, f"v11_noah_{bkey}.png")
    arr = np.array(Image.open(p).convert("RGBA"))
    perfect(f"v11_noah_{bkey}", arr, f"noah_{bkey}")
    noah_items.append((f"诺亚·{BODY_ZH[bkey]}", os.path.join(OUT, f"noah_{bkey}", "96px.png")))
g = grid_view(noah_items, 3, 420, 24)
g.save(os.path.join(OUT, "筛选_诺亚_古典科研_v11.png"))
print("诺亚 v11", flush=True)

p = os.path.join(SRC, "v11_boss_05_wolf.png")
arr = np.array(Image.open(p).convert("RGBA"))
perfect("v11_boss_05_wolf", arr, "boss_05_wolf")
g = grid_view([("⑤伤痕紫黑狼", os.path.join(OUT, "boss_05_wolf", "96px.png"))], 1, 420, 24)
g.save(os.path.join(OUT, "筛选_狼_v11.png"))
print("狼 v11", flush=True)
