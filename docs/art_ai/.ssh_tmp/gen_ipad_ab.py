#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_ipad_ab.py — IPAdapter 画质回归实验（艾琳，3 变体同 seed）
变体1: weight 0.5 + 96px参考     变体2: weight 0.5 + 768原图参考
变体3: weight 0.7 + 768原图参考
"""
...
def make_wf(ref_img, weight, seed, prefix):
    return {
        ...
        "3": {"class_type": "easy ipadapterApply", "inputs": {
            "model": ["2", 0], "image": ["1", 0],
            "preset": "PLUS (high strength)", "lora_strength": 0.4, "provider": "CUDA",
            "weight": weight, "weight_faceidv2": 1.0, "start_at": 0.0, "end_at": 1.0,
...
VARIANTS = [
    ("refs_px/elin_ref.png",           0.5, "ipad_w05_px96"),
    ("refs_768/elin_768.png",          0.5, "ipad_w05_768"),
    ("refs_768/elin_768.png",          0.7, "ipad_w07_768"),
]

for ref, w, prefix in VARIANTS:
    wf = make_wf(ref, w, 20260816, prefix)
    ...
    while True:
        ...
        if pid in h:
            ...
            if st == "error":
                ...
                print(f"DONE {prefix} w={w} ref={ref.split('/')[1]} ({time.time()-t0:.0f}s)", flush=True)
            break
