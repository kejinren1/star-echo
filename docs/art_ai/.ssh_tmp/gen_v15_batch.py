#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_v15_batch.py — v15 狼重画×5：病态瘦 + 结构/比例修正
v14 四肢比例崩坏（极端瘦词触发变形）→ 平衡病态感与结构正确性
"""
...
WOLF = ("feral wolf beast, pure quadrupedal animal form, natural wolf anatomy, "
        "well-proportioned body, correct stance on all fours, "
        "not anthropomorphic, not bipedal, "
        "lean thin emaciated body, visible ribs, gaunt look, "
        "natural length slender legs, correct joints, "
        "purple-black fur color scheme, long prominent tail raised, "
        "blood-red eyes, ferocious snarling expression, "
...
def make_wf(desc, seed, prefix):
    ...
    return {
        ...
        "8": {"class_type": "KSampler", "inputs": {"seed": seed, "steps": 32, "cfg": 6.0,
...
def main():
    ...
    seed = 20261501
    for k in range(5):
        tasks.append((f"v15_wolf_{k+1}", WOLF, seed + k))
