#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_v12_strawberry_hd.py — v12：Strawberry + 4x-UltraSharp ESRGAN 超分（修复 v11 latent 放大模糊）
管线：KSampler1 512×768 → VAEDecode → ImageUpscaleWithModel(4x-UltraSharp 4x) → ImageScale(1024×1536 lanczos)
     → VAEEncode → KSampler2 refine denoise 0.25 → VAEDecode → 输出
"""
import json
import sys
...
CKPT = " Strawberry-α_v1.safetensors"
OUT = Path("D:/30DAYS/docs/art_ai/output_abc/v12_strawberry_hd_20260815")
...
def build_wf(pos, seed):
    return {
        ...
        # ---- ESRGAN 超分链路 ----
        "14": {"class_type": "UpscaleModelLoader", "inputs": {"model_name": "4x-UltraSharp.pth"}},
        "15": {"class_type": "ImageUpscaleWithModel", "inputs": {"upscale_model": ["14", 0], "image": ["9", 0]}},
        "16": {"class_type": "ImageScale", "inputs": {"image": ["15", 0], "width": 1024, "height": 1536,
              "upscale_method": "lanczos", "crop": "disabled"}},
        "17": {"class_type": "VAEEncode", "inputs": {"pixels": ["16", 0], "vae": ["11", 0]}},
        "13": {"class_type": "KSampler", "inputs": {"seed": seed + 1, "steps": 22, "cfg": 6.0,
             "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.25,
             "model": ["1", 0], "positive": ["2", 0], "negative": ["3", 0], "latent_image": ["17", 0]}},
        "18": {"class_type": "VAEDecode", "inputs": {"samples": ["13", 0], "vae": ["11", 0]}},
        "10": {"class_type": "SaveImage", "inputs": {"filename_prefix": "v12", "images": ["18", 0]}},
    }
...
def main():
    ...
    client = ComfyClient(HOST, token=TOKEN, timeout=900)
    ...
    print(f"[plan] 共 {len(tasks)} 张（Strawberry + ESRGAN 超分 → 1024×1536）")
    ok, fail = 0, []
