...
- [~] F1-E 表现配置抽表（T-016~024：SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX/FX/SHEET_CONFIG/初始枪/炮台默认 → Excel presentation sheet）【✅ 第一批 SPRITE_MAP 已收口（总指挥第 4 轮 2026-08-18 承接动工：GameData.xlsx 新增 enemy_sprites sheet 23 敌 + data_schema 注册 presentation.json/enemy_sprites + excel_export 构建 + DataLoader.get_enemy_sprite_config 消费（Vector2i/Color/scale 组装 + 未命中按 category 兜底 const）+ enemy.gd 改读；新探针 day31_presentation_check 246/246 全绿；day31_skill_icon_check 22/22（HUD se_skill_sword_arc 图标补丁）；两探针并入 runner 58→60 件套）· **✅ 第二批 BEHAVIOR_MAP 已收口（执行者第 56 轮 `b410a8b`：Excel enemy_behavior 表 9 行 + data_schema 注册 + excel_export 构建 + presentation.json behavior_map 9 条（13 JSON 零 diff）+ data_loader get_enemy_behavior（懒加载+枚举名解析+const 兜底）+ enemy.gd 行为解析改读 + 探针 261/261 + 回归 60/60）**· 后续批次：BGM/SFX→FX→SHEET_CONFIG→初始武器→炮台默认（总指挥/主窗口按批推进，每批 Excel→导出→消费→探针→回归，const 兜底）】
...
- [ ] **AF-P0-A1【W1】新建 `scripts/systems/hitstop_controller.gd`**（extends Node）：参数读 `DataLoader.get_stats_feel()`（A3 落地前用常量兜底 = 默认值）——`hitstop_melee=0.15`（近战重，O-2 拍板）/ `hitstop_ranged=0.05`（远程轻）/ `hitstop_crit_bonus=0.1`（暴击叠加）/ `hitstop_boss_kill=0.15`（Boss 击杀）；`trigger(duration: float)` 累加剩余停帧（**多触发取 max 防无限叠加**）；`_process` 内 `_time_left > 0` → `Engine.time_scale = 0.0`；**恢复 = `get_tree().create_timer(_time_left, true, false, true)` 回调强制 `Engine.time_scale = 1.0`**（⚠️ 第 4 参 ignore_time_scale=true，防 time_scale=0 下计时器停摆死卡；SPEC 红线 3）+ 超时兜底（累计停帧 > 0.5s 强制归 1.0 + push_warning）；挂载 = main.gd（`$HitstopController` 子节点，F2 边界原则系统级）；**探针可独立实例化不依赖场景**；退出前归 1.0 防污染后续探针
- [ ] **AF-P0-A2【W1】调用点接线 2 处**：① `scripts/weapons/projectile.gd` `_on_body_entered`（:82 take_damage 后、:93 play_sfx 前）：弹丸 meta 透传 weapon_type（F2-T4 工厂 spawn_projectile 已透传 weapon_data，核对透传链）→ `weapon_type=="melee"` → trigger(hitstop_melee)，否则 → trigger(hitstop_ranged)；`_is_crit_hit()` → 追加 trigger(hitstop_crit_bonus)（取 max 合并）② `scripts/enemy/enemy.gd` `die()`（:344）：`is_boss` → trigger(hitstop_boss_kill)（普通怪零顿帧防节奏碎裂，O-2 近重远轻口径）；⚠️ 调用点与批 B 震屏/批 C 音画同步同点，三批接线集中一次性梳理
- [ ] **AF-P0-A3【W2】Excel feel 段数据侧（O-2 拍板「落地走 Excel 管线」）**：docs/GameData.xlsx stats sheet 新增 `feel` 段（flat_dict 单行键值，仿 stats_combat 先例）：hitstop_melee=0.15 / hitstop_ranged=0.05 / hitstop_crit_bonus=0.1 / hitstop_boss_kill=0.15；tools/data_schema.py 注册 `stats_feel`（sheet→stats.json 顶层 "feel" 键）；excel_export.py 导出 → stats.json +feel 键（**其余 JSON 零漂移断言**）；`data_loader.gd` 新增 `get_stats_feel() -> Dictionary`（缺段兜底 = 上述默认值，仿 get_stats_combat 先例）
- [ ] **AF-P0-A4【W1】探针 `tools/day31_feel_check.gd`（≥14 断言三段 · 批 A 部分）**：§1 hitstop——trigger 后 `Engine.time_scale==0` 断言 / create_timer 恢复回调归 1.0 / 多触发取 max / 超时兜底强制恢复 / **600 帧深探不挂**（探针内白盒驱动长帧模拟，防 time_scale 卡死）；§2 数据驱动——改 Excel feel 数值 → 导出 → get_stats_feel 返回值变化（A3 验收，F1-散 §1 端到端双跑先例）；§3 回归抽样（day24_audio 14/14 锚点零改动 / time_scale 探针结束归 1.0 防污染后续探针）
- [ ] **AF-P0-A-EXIT【W5】回归**：60 件套 1463 断言 + day31_feel §1§3 + baseline **BASELINE CLEAN**

### AF-P0 批 B · 相机震屏分级（SPEC §二 F2 · 依赖批 A 的 feel 段参数）
- [ ] **AF-P0-B1【W1】震屏参数表化**：main.gd F-03 现有 `_SHAKE_DURATION=0.15/_SHAKE_MAGNITUDE=4.0`（:36-37）→ 改读 `get_stats_feel()` 三级（light 命中·玩家受伤 / medium 暴击·普通击杀 / heavy Boss 死亡）；Excel feel 段扩展 6 键（shake_light_duration/magnitude + shake_medium_* + shake_heavy_*，默认 light=0.15/4.0 = 现值**零漂移**、medium=0.2/6.0、heavy=0.3/9.0）；Boss 死亡用 heavy 重档（SPEC F2 四级含 Boss 死亡；红线 4 Boss 豁免仅限僵直/击退域，震屏保留）
- [ ] **AF-P0-B2【W1】调用点补全**：main.gd 新增 `_trigger_camera_shake(level: String) -> void`（按 level 读 feel 表设 _shake_time/_shake_magnitude；现有 `_on_player_hit` :222 改调 `_trigger_camera_shake("light")` 语义保留）；projectile.gd `_on_body_entered`：非暴击命中 → light / 暴击 → medium（与批 A 同点接线）；enemy.gd `die()`：普通 → medium / `is_boss` → heavy（与批 A 同点接线）；**⚠️ 探针坑：`randf_range` 走全局 RNG → 探针白盒直构造或 seed(N)（项目已知坑）**
- [ ] **AF-P0-B3【W1】探针扩展**：day31_feel_check +§4 震屏段（≥8 断言：白盒直调 `_trigger_camera_shake("light"/"medium"/"heavy")` 断言幅度/时长分层 / 衰减归位 ZERO / 玩家受伤 light 路径保留 / Boss 死亡 heavy）
- [ ] **AF-P0-B-EXIT【W5】回归**：60 件套 + day31_feel 全段 + baseline **BASELINE CLEAN**

### AF-P0 批 C · 音画同步（SPEC §二 F5 · 依赖批 A 命中点接线可复用）
- [ ] **AF-P0-C1【W1】audio_manager 扩展**：新增 `play_sfx_delayed(sfx_name: String, delay: float) -> void`（SceneTreeTimer 延迟播放，复用现有 SFX_POOL；**SFX_MAP 键契约（红线 2）：现有 hit/crit/death/skill 键已全齐 → 零新增零删改**；空挥概念不存在——自动射击，SPEC F5 明示）
- [ ] **AF-P0-C2【W1】命中点串联 3 处**：① projectile.gd `_on_body_entered`（:93 已有 `play_sfx("hit")` 保留）：`_is_crit_hit()` 分支追加 `play_sfx("crit")`（延迟≈0 立即 = 暴击音画同步）② enemy.gd `die()`：补 `play_sfx("death")`（击杀音——实测 D24-T3 死亡消费点核对后补缺）③ skill_controller 技能释放：核对现有 `play_sfx("skill")` 消费点（D24-T3 已接线则保留不动，仅核对登记）
- [ ] **AF-P0-C3【W1】探针扩展**：day31_feel_check +§5 音画段（≥8 断言：命中→hit 音调用序 / 暴击→crit 音追加 / 击杀→death 音 / play_sfx_delayed 延迟播放白盒断言 / SFX_MAP 键完整性 = 10 键零漂移）
- [ ] **AF-P0-C-EXIT【W5】收口**：60 件套 + day31_feel 全段 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（hitstop 顿帧节奏 / 震屏层次 / 音画同步听感 → #5，不伪装机器 PASS，红线 7）

### AF-M1 · CC0 素材替换（SPEC §一 M1 · O-1 拍板「M1 先行」· P1 批 · 顺带拆解标注网络依赖 · 可与 P0 并行）
- [ ] **AF-M1-1【W1/W3】CC0 BGM/SFX 替换**：从 GitHub 生态采集 CC0 曲目/音效（openGameArt / Kevin MacLeod / FreePD 的 GitHub 仓库或镜像，SPEC 红线 6：GitHub 生态 raw/codeload 可达；**不把外部下载脚本当提交物**）→ 替换 `assets/audio/**`（**文件名不变、wav 格式不变、SFX_MAP/BGM_MAP 零改动**——红线 2 契约；bgm 2 轨 + sfx 10 个）→ 来源标注入库 `docs/AUDIO_FEEL_SOURCES.md`（曲目/作者/许可/URL）→ headless 加载零 push_warning + 回归 60 件套；⚠️ 替换后 .import 缓存按 mtime 自动重导，若异常移走 .godot 相关缓存重建（项目历史教训）；⚠️ **网络受限（spriters-resource 前例：部分站点 Cloudflare 拦截）则登记阻塞交总指挥，不阻塞 AF-P0 批**（M1 为 P1 非 P0）
- [ ] **AF-M1-EXIT【W5】**：替换完成 + 来源标注 + 回归全绿 + #5 真人听感登记（主观项，红线 7）
