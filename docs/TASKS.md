...
>
> 📌 **第 63 轮（2026-08-19 02:0x · #2）拆解回执**：**P0 检查** = PLAYTEST 追踪区增量 #89 之后无新增量（反馈专员已改 2h→4h 一轮，下一轮 02:38；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）· 但 F1-E 批四已收口 → 本轮核心拆解 = F1-E 批五 SHEET_CONFIG（第 62 轮「下轮起手」约定条件满足）**：git 实测 HEAD=`1ccb419`（#1 进度分析第 67 轮；第 62 轮后 +3 = **#3 执行者第 63 轮 `f67630b`（F1-E-4-2~4-4 消费端收口：DataLoader.get_fx_config 懒加载 + vfx_player.set_effect 改读 + day31_presentation +§6 fx 段 13 断言）→ `681ab36`（第 63 轮收口：**回归 63/63 · 1578 断言全绿** + T-019 转已收口 + **F1-E 4/7 批** + portal 并入 runner = TEST_REPORT #62 观察兑现）+ `1ccb419` #1 第 67 轮确认**）；工作区 = 用户会话在途（lain 动画帧 / AI 美术工具 / player_anim.gd + sprite_frame_factory.gd M）**不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。**F1-E 4/7 批闭环确认**（SPRITE_MAP + BEHAVIOR_MAP + BGM/SFX + **FX 批四**）：**本轮已函数级拆解批五 SHEET_CONFIG（T-020 icon_atlas.gd:8-24，见 F1-E 行下方 F1-E-5，承接方 = #3 执行者）**；剩余批次 = 初始武器→炮台默认（承接方推进时再拆）。**RELIC 全批（已拆 `f19447c` + 方案第 31 轮）+ LEVEL_DESIGN 全批（已拆 `9ebbf5f` + 方案第 32 轮）承接方 = #3 执行者，git 实测仍零开工（HEAD 无 day31_relic_*/day31_level_design_*/spawn_points 提交）→ 挂账观察**。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717` 授权导出，含 F-45~48 + F1-E-4-1，**不含其后 F-49 + F1-E-4 消费端**）→ 待下次打包验证。**回归硬门槛口径更新 = 63 件套 1578 断言**（`681ab36` runner +portal 24 + presentation 286，批五 EXIT 以 63/1578 为准）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ 批四收口确认（上轮约定条件兑现）② **F1-E 第五批 SHEET_CONFIG 函数级拆解（本轮主产出，见 F1-E 行下方 F1-E-5）**——icon_config sheet 3 行（weapons 40/items 54/skills 5）→ data_schema 注册 → excel_export 构建 presentation.json icon_config（frame_size 拆列组装）→ DataLoader.get_icon_config（懒加载 + Vector2i 组装 + const 兜底）→ IconAtlas.get_icon/get_frame_count 消费改读（**SHEET_CONFIG const 保留兜底 = day31_items_atlas_check/day31_skill_icon_check 直接读 const 零改动硬门槛 + day11_12/day20/day24_f13 get_frame_count 动态读行为保持**）→ day31_presentation_check +§7 icon 段 → 回归 63 件套；标「承接方 = #3 执行者」③ Day 30 区补第 63 轮确认块 ④ F1-E 行刷新（批五拆解标注 + 4/7 批）⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：按 F1-E-5 拆解推进批五（每批 Excel→导出→消费→探针→回归，const 兜底）；RELIC/LEVEL_DESIGN 执行序维持（A 先行 / LD-A→B→C→E→D 可选）。
>
> 📌 **方案师第 33 轮确认块（2026-08-19 02:4x · 方案师）**：**P0 结论** = 增量 #89 后无新增量（🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = F1-E 批五 SHEET_CONFIG 正式方案（SOLUTION_PLAN.md 第 33 轮）**——实测复核锚点与第 63 轮拆解**逐一一致**（新增确认 3 处：① data_loader.gd 实际路径 = `scripts/autoload/data_loader.gd`，get_fx_config :661-673 仿写范式（_fx_map :36 / size→Vector2i :670-671）② vfx_player 实例节点 :97 `get_node_or_null("/root/DataLoader")` vs IconAtlas **static 类须 `Engine.get_main_loop().root.get_node_or_null("DataLoader")`**（无现成 static 先例 = 新范式，--script 环境返回 null 回退 const 天然兼容）③ day31_presentation_check 单 _run() + 注释分段结构，+§7 icon 段仿 ⑥ fx 段模式；excel_export :449 现 5 根键 → 追加第 6 键 icon_config 勿覆盖）。**批四 FX 全收口确认**（`681ab36`：4/7 批，T-019 转已收口）→ 上轮挂账全解除。**RELIC 挂账跨 2 轮 / LEVEL_DESIGN 挂账跨 1 轮（拆解后首轮）仍零开工** → 维持观察，承接方 = #3（D-015 交接，拆解+方案齐备即开工）。⚠️ build/ = 08-18 23:22（`2aeb717`：含 F-45~48 + F1-E-4-1，**不含 F-49 + F1-E-4 消费端**）→ 传送门/宝箱/批四抽表验证需最新代码或下次打包。**回归硬门槛 = 63 件套 1578 断言**（批五 EXIT 以 63/1578 为准）。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> **下轮起手**：查批五是否开工（git log 出现 icon_config sheet / presentation.json icon_config / get_icon_config / day31_presentation_check +§7 icon 段）——**若收口**：F1-E 5/7 批标记 + 批六 初始武器（T-004 weapon_controller.gd:50-62 初始枪内联配置 + T-022 hud.gd:240-245 SKILL_ICON_MAP，两子项评估合并拆）拆解；查 RELIC-A 是否开工（stats sheet 改名 + day31_relic_name）；查 LEVEL_DESIGN-A 是否开工（spawn_points/boss_phase_events sheet + day31_level_design_* 探针）——若批 A 收口：确认两 JSON 零漂移 + FK 校验报错生效 → 推进批 B/C；**D30 上传/收口仍等 Owner 拍板**（D30-T3 上传确认 + D30-EXIT 发布摘要落盘）；build/ 是否重打含 F-49 + F1-E-4；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 64 轮（2026-08-19 04:00 · #2）拆解回执**：**高峰检查**：04:00 不在 09-12/14-18 高峰 → 正常执行。**P0 检查** = PLAYTEST 追踪区增量 #89 后无新增（反馈专员 4h 一轮，02:38 轮空转零产出符合 D-018「无反馈轮不写增量不 commit」；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无新用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`5c306b1`（总指挥第 7 轮 03:00 夜间自检；第 63 轮后 +4 = **#3 执行者第 64 轮 LEVEL_DESIGN LD-A 数据地基收口** `96e4cd5`（spawn_points sheet 11 行三型 + boss_phase_events sheet 7 行 6 类型 + waves 加 spawn_set/spawn_order 列 + data_schema 注册 + excel_export FK 校验三态 + **--check-only 已知缺陷顺手修（check_only 参数未消费 → 校验+roundtrip 自检后直接返回不写盘，纯只读回归护栏）** + DataLoader 三接口 + day31_level_design_data_check **24/24** + runner 63→**64 件套** + day26 锚点 1578→**1602**）→ `3509b74`（收口 + **回归阻塞登记 59/64：5 FAIL = 用户会话在途 D-26 `set_frame_offset` Godot 4.4 API 误用**（`player_anim.gd`/`sprite_frame_factory.gd` M，角色动画链 SCRIPT ERROR，与 LD-A 零关联，LD-A 探针/presentation/day26 全 PASS）+ 方案师第 33 轮挂账入库）→ `5c306b1` 总指挥第 7 轮（**D-020 回归阻塞挂账处置：不代修待收口，EXIT 门槛统一挂等复跑** + 审批：LD-A 收口/批四 FX = 实质高质 + #2 拆解 ×2 = 实质 + LOOP_HEALTH 第 15 条心跳）**。工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）**不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **LEVEL_DESIGN LD-A 收口确认（上轮挂账解除）**：LD-A1/A2/A3 全 [x]（`96e4cd5`），**LD-A-EXIT [~] 部分收口**（探针 24/24 + runner 64 件套 + baseline CLEAN 全过；**仅回归硬门槛 59/64 挂 D-26 用户会话在途，D-020 处置 = 不代修待收口，复跑恢复 64/64 后转 [x]**）→ **LD-B（固定出生点）/ LD-C（Boss 演出）/ LD-E（attr 状态）/ LD-D（可选）/ LD-EXIT 仍 [ ] 待执行**（已拆已定案，承接方 #3）。**F1-E 批五 SHEET_CONFIG 已拆解仍未开工**（#3 第 64 轮优先执行 LD-A；F1-E-5-1~4/EXIT 全 [ ]，git 无 icon_config 提交）→ 挂账观察（跨 1 轮）。**RELIC 全批仍零开工**（git 无 day31_relic_*/stats 改名提交）→ 挂账观察（跨 3 轮）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A）→ 待下次打包验证。**回归硬门槛口径更新 = 64 件套 1602 锚点**（LD-A runner 扩容；当前 59/64 5 FAIL 挂 D-26，各批 EXIT 门槛统一挂等复跑恢复 64/64 后全绿）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ LD-A 收口确认（上轮挂账解除）+ D-020 回归阻塞登记确认（59/64 = 用户会话在途非游戏缺陷，不代修待收口）② **F1-E 第六批 初始武器 + SKILL_ICON_MAP 函数级拆解（本轮主产出，见 F1-E 行下方 F1-E-6）**——T-004 初始枪抽表（weapons sheet +starting_gun 行 → build_weapon_from_data 改读 + 内联兜底，**source_id 留空保 day13「占位跳过 inventory」硬门槛**）+ T-022 SKILL_ICON_MAP 抽表（**独立 skill_icon_map 表**：否决 characters 点号列备选 = blade_burst 历史键无主 + 掉落技能槽覆盖不全；5 行 → presentation.json → DataLoader.get_skill_icon_index → hud 两处消费改读，**SKILL_ICON_MAP const 保留兜底 = day31_skill_icon_check §5 直接读 const 零改动硬门槛**）→ day31_presentation +§8 skill_icon 段 → 回归 64 件套；标「承接方 = #3 执行者」③ Day 30 区补第 64 轮确认块 ④ F1-E 行刷新（批六拆解标注）⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：LD-B→C→E 执行序（LD-A 已收口解锁）；F1-E 批五→批六连续推进（拆解齐备）；RELIC 全批待执行；D-26 用户会话收口后复跑回归恢复 64/64。
>
> **下轮起手**：查 LD-B 是否开工（git log 出现 `_get_spawn_position`/spawn_set 透传）与批五是否开工（icon_config/get_icon_config/§7 icon 段）——**若 LD-B 收口**：确认 day31_flee_bound 18/18 零改动（F-48 不回归）+ 探针 §出生点段 → 推进 LD-C；**若批五收口**：F1-E 5/7 批标记 + 批七 炮台默认（T-024 turret.gd:13-15 与 se_auto_turret 数据重复）拆解（批六已拆可连续）；D-26 用户会话是否收口（回归恢复 64/64 即 EXIT 门槛解冻）；RELIC 是否开工；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 70 轮（2026-08-19 22:00 · #2）拆解回执**：**高峰检查**：22:00 不在 09-12/14-18 高峰 → 正常执行。**P0 检查** = PLAYTEST 追踪区止于增量 #89（反馈专员 4h 轮 18:38 空转零产出符合 D-018，git 无 #90 提交）；TEST_REPORT **#69（20:00 · HEAD=`74ad1fd`）= 65 件套 60/65**（1692 期望，5 FAIL 与 #63~#69 同根因 = D-26 用户会话在途 `set_frame_offset` Godot 4.4 API，断言 0 失败复跑 ×10 稳定非 flaky；day31_relic_name **15/15 首纳入** + presentation 349/349 + skill_icon 22/22 + level_design 36/36 全 PASS；BASELINE CLEAN · JSON 18 文件 2861 字段零缺陷）——**🔴P0 无新增 / 🟠 无用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`de0f563`（第 69 轮后 +6 = **#3 执行者第 70 轮 RELIC-0 数据地基全收口**：`01a27c7` **0-1 数据侧**（items relic 类条目 +6 新列 tag/tier/set_id/set_tier/set_effects/unlock_condition + 套装 2 套 4 件占位 星骸孤注/死线舞者 price=0 不进商店池 + 移速流派示例 6 件 T1×3/T2×2/T3×1 + items_effects 子表 +9 行 + excel_export set_effects 分隔串→JSON 数组解析 + KNOWN_EFFECT_KEYS +6 新键，其余 16 JSON 零 diff）+ `6c54857` **0-2/0-3+EXIT 探针**（DataLoader get_relic_defs/get_relic_set_ids 懒加载接口 + save_system meta 两键 relic_affinity/relic_codex 缺省容错旧档零崩 + day31_relic_data_check **55/55** + runner 65→**66 件套** + day26 锚点 1692→**1748** + items_atlas 58→59 占位豁免 + day20 54→64）+ `55fcc5a` **回归同步**（day18_feedback2 flaky 根治：点击前跳过 anvil 服务卡，零游戏逻辑改动 + day23/day24 items 条数 54→64 锚点同步 + f3_compliance bool 白名单 + 回归 **61/66** 5 FAIL 全 D-26 零新增）+ `be647b4` **0 收口挂账**（TASKS RELIC-0-1~EXIT 全 [x] + RELIC 区块头 0 批已收口标注 + SOLUTION_PLAN 第 70 轮执行结果 61/66 66 件套 1748 锚点）+ `de0f563` **总指挥第 8 轮**（D-021 RELIC-F/E 下一卡点 + build 重打挂账等 D-26 收口 + 审批近 6h 10 提交 RELIC-A/0 双批收口 = 实质高质 + 7/7 ACTIVE））；工作区 = 用户会话在途（lain 动画帧 / bg 背景图 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M **一律不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **RELIC-0 收口确认（上轮「RELIC-0 数据地基待执行」挂账解除）= RELIC 双批（A/0）落地 🎉**：RELIC-0-1/0-2/0-3/EXIT 全 [x]（items relic +6 新列 + 套装 2 套 4 件 + 流派 6 件占位 + get_relic_defs/get_relic_set_ids + meta 两键缺省容错 + day31_relic_data **55/55** + 回归 66 件套 61/66 挂 D-26 零新增）→ **剩余 RELIC-F/E/B/C/D/EXIT 已拆已定案待 #3 执行**（执行序 F/E（P0 独立）→ B/C/D（依赖 0 已就绪）→ EXIT）。**LEVEL_DESIGN LD-C（Boss 演出）/ LD-E（attr 状态）/ LD-D（可选）仍 [ ] 待执行**（已拆已定案，承接方 #3；LD-C 跨 5 轮）→ 挂账观察维持。**D-26 回归阻塞维持**（61/66，5 FAIL 清单与上轮完全一致零新增，D-020 不代修待收口）。**阶段 F 真全闭维持**（7/7 批）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七 + RELIC-A/0）→ 总指挥 D-021 已登记 build 重打挂账（等 D-26 收口）维持。**回归硬门槛口径更新 = 66 件套 1748 锚点**（RELIC-0 runner 扩容；当前 61/66 挂 D-26，各批 EXIT 门槛统一挂等复跑恢复 66/66 后全绿）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ TEST_REPORT #69 核查确认（65 件套 60/65 挂 D-26）+ **RELIC-0 收口确认（RELIC 双批 A/0 落地，上轮挂账解除）** ② **登记「无待拆」不空转**（RELIC 剩余 6 批 / LD-C·E·D 全部已拆已定案，承接方 #3 直接执行；阶段 F 真全闭；Day 30 纯 Owner/#4 域）③ Day 30 区补第 70 轮确认块 ④ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑤ 交 #3：RELIC-F/E（P0 独立）→ B/C/D → EXIT 执行序；LD-C→E→D 可选推进；D-26 用户会话收口（适配 Godot 4.3 或移除 set_frame_offset 调用）后复跑回归恢复 66/66 即各批 EXIT 门槛解冻。
>
> 📌 **第 69 轮（2026-08-19 20:00 · #2）拆解回执**：**高峰检查**：20:00 不在 09-12/14-18 高峰 → 正常执行。**P0 检查** = PLAYTEST 追踪区止于增量 #89（mtime 08-19 04:44；反馈专员 4h 轮 06:38/10:38/14:38/18:38 全部空转零产出符合 D-018，git 无 #90 提交）；TEST_REPORT **#68（18:00 · HEAD=`d03750e`）= 64 件套 59/64**（1677 期望，5 FAIL 与 #63~#68 同根因 = D-26 用户会话在途 `set_frame_offset` Godot 4.4 API，断言 0 失败，复跑 ×8 稳定非 flaky；day31_presentation **349/349** + skill_icon 22/22 + level_design 36/36 全 PASS；BASELINE CLEAN · JSON 18 文件 2861 字段零缺陷）——**🔴P0 无新增 / 🟠 无用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`74ad1fd`（第 68 轮后 +2 = **#3 执行者第 69 轮 RELIC-A 属性命名去土豆兄弟化全收口**：`765e3bc` **A1+A2**（Excel stats 两行 name 改——元素伤害→魔法伤害 / 工程学→机械学，id 零改动 + characters/elements 文案同步 + desc_builder STAT_CN 两键同步统一机械学 + data_schema label 同步 + 导出三 JSON 仅文案变其余零 diff）+ `74ad1fd` **A3+EXIT 收口**（day31_relic_name_check **15/15**：§1 stats 两 name 新值 + base 保持 + 全量零残留 / §2 desc_builder STAT_CN / §3 attribute_controller 源码 id 锚点（不 preload 引用 Autoload = 探针三坑①）/ §4 characters 两处 / §5 elements effect+scaling_attr id 零改动 + runner 64→**65 件套** expect15 + day26 锚点 1677→**1692**（34/34）+ **回归 60/65（5 FAIL = D-26 用户会话在途零新增）** + TASKS RELIC-A1~EXIT 全 [x] + SOLUTION_PLAN 第 69 轮执行结果；**执行登记**：elements effect 文本属性名引用同属展示残留（方案点名三 JSON 实测补一处）））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M **一律不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **RELIC-A 收口确认（上轮「RELIC 跨 8 轮零开工」挂账部分解除）**：RELIC-A1/A2/A3/EXIT 全 [x]（`765e3bc`+`74ad1fd`，两 name 新值 + id 零改动 + 全量零残留 + day31_relic_name 15/15 + 回归 65 件套 60/65 挂 D-26 零新增）→ **RELIC 首批落地，剩余 RELIC-0（数据地基）/ F / E / B / C / D / EXIT 仍 [ ] 待执行**（全部已拆已定案，承接方 #3 直接执行，执行序 = RELIC-0 先行 → F/E（P0 独立）→ B/C/D → EXIT）。**LEVEL_DESIGN LD-C（Boss 演出）/ LD-E（attr 状态）/ LD-D（可选）仍 [ ] 待执行**（已拆已定案，承接方 #3；LD-C 跨 4 轮）→ 挂账观察维持。**D-26 回归阻塞维持**（60/65，5 FAIL 清单与上轮完全一致零新增，D-020 不代修待收口）。**阶段 F 真全闭维持**（7/7 批）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七 + RELIC-A）→ 待下次打包验证（D-016 授权自动替换已生效）。**回归硬门槛口径更新 = 65 件套 1692 锚点**（RELIC-A3 runner 扩容；当前 60/65 挂 D-26，各批 EXIT 门槛统一挂等复跑恢复 65/65 后全绿）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ TEST_REPORT #68 核查确认 + **RELIC-A 收口确认（RELIC 首批落地，上轮挂账部分解除）** ② **登记「无待拆」不空转**（RELIC 剩余 7 批 / LD-C·E·D 全部已拆已定案，承接方 #3 直接执行；阶段 F 真全闭；Day 30 纯 Owner/#4 域）③ Day 30 区补第 69 轮确认块 ④ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑤ 交 #3：RELIC-0 数据地基先行（B/C/D 依赖）→ RELIC-F/E（P0 独立）→ B/C/D → EXIT 执行序；LD-C→E→D 可选推进；D-26 用户会话收口（适配 Godot 4.3 或移除 set_frame_offset 调用）后复跑回归恢复 65/65 即各批 EXIT 门槛解冻。
>
> **下轮起手**：查 RELIC-0 是否开工（git log 出现 items relic 字段扩展 rarity/tag/tier/set_id + get_relic_defs + day31_relic_data_check）——若收口：确认字段键齐全 + 存档兼容（day30_save_compat 14/14 零改动）→ 推进 RELIC-F/E；查 LD-C 是否开工（boss_phase_player.gd + phase 钩子接线）——若收口：确认 day18_19 48/48 + day30_boss_skill 49/49 零改动 → 推进 LD-E；D-26 是否收口（回归恢复 65/65 即 EXIT 门槛解冻）；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 68 轮（2026-08-19 18:00 · #2）拆解回执**：**高峰检查**：18:00 不在 09-12/14-18 高峰（14-18 已过）→ 正常执行。**P0 检查** = PLAYTEST 追踪区增量 #89 后无新增（反馈专员 4h 一轮 14:38 轮未见新增量；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`d03750e`（第 67 轮后 +7 = **#1 第 71 轮 `fe471a0` 实测纠正「名义 7/7 实为 6/7」（批五 SHEET_CONFIG 未落地，交总指挥核实）→ #3 执行者批五全收口**：`d990eca` 5-1 icon_config 数据侧（GameData.xlsx +icon_config sheet 3 行 weapons 40/items 54/skills 5 + data_schema 注册 + excel_export presentation 第 7 键，其余 15 JSON 零 diff）/ `057e493` 5-2 get_icon_config 懒加载接口（仿 get_fx_config 范式）/ `3388901` 5-3 IconAtlas 消费改读（_resolve_icon_config 静态新范式 + SHEET_CONFIG const 兜底 = day31_items_atlas/skill_icon 直读 const 零改动硬门槛 + get_frame_count 动态读行为保持）/ `5b92fd3` 5-4 探针 +§10 icon_config 段 **15 断言**（334→**349/349** + runner expect 334→349 + day26 锚点 1662→1677）/ `d03750e` **Day31-F1-E-5-EXIT 收口**（F1-E 行 **7/7 批 = 阶段 F 真全闭 🎉** + T-020 转已收口 + SOLUTION_PLAN 底部第 68 轮执行结果：回归 59/64 挂 D-26 零新增 + 349/349 + BASELINE CLEAN + 锚点 1677））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M **一律不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **F1-E 批五收口确认（上轮「名义 7/7 vs 实测 6/7」更正挂账解除）= 阶段 F 真全闭 🎉**：F1-E-5-1~4/EXIT 全 [x]（T-020 icon_config 表 + get_icon_config + IconAtlas 消费改读 + SHEET_CONFIG const 兜底 + day11_12/day20/day24_f13 get_frame_count 动态读行为保持；探针 **349/349**；回归 59/64 5 FAIL = D-26 用户会话在途零新增）→ **F1-E 真 7/7 批 + 阶段 F 真全闭**（#1 第 71 轮更正由 #3 实际执行闭环，方案师第 37 轮锚点更新 5 处全部兑现；30 天计划 Day 1-30 客观任务 + 阶段 F 技术债全部客观收口，剩余 Day 30 尾项纯 Owner/#4 域）。**RELIC 全批仍零开工**（git 无 day31_relic_*/stats 改名提交）→ 挂账观察（跨 **7** 轮）。**LEVEL_DESIGN LD-C/E 仍零开工**（git 无 boss_phase_player.gd/attr 分支提交）→ 挂账观察（LD-C 跨 **3** 轮）；LD-D 可选批维持待评估。**D-26 回归阻塞维持**（59/64 5 FAIL 清单与上轮完全一致零新增，D-020 不代修待收口）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七）→ 待下次打包验证（D-016 授权自动替换已生效，等 #3/总指挥产出新版本后归档重导出）。**回归硬门槛口径 = 64 件套 1677 锚点**（批五收口 day26 1662→1677；当前 59/64 挂 D-26）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ **F1-E 批五收口确认（阶段 F 真全闭 🎉，上轮更正挂账解除）** ② **AF-P1 预留登记维持**（SPEC §二 F3 命中粒子/数字 P1——F-11 伤害数字子系统 + F-45 hit 特效渐隐已覆盖命中反馈面，持续判断不拆）③ **登记「无待拆」不空转**（RELIC 全批跨 7 轮 / LD-C·E 全部已拆已定案，承接方 #3 直接执行；F1-E 7/7 真全闭；Day 30 纯 Owner/#4 域）④ Day 30 区补第 68 轮确认块 ⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：RELIC（A 先行）→ LD-C→E 执行序维持；D-26 用户会话收口后复跑回归恢复 64/64 即各批 EXIT 门槛解冻。
>
> **下轮起手**：查 RELIC-A 是否开工（git log 出现 stats sheet 改名 + day31_relic_name）——若收口：确认 stats.json id 零改动 + 回归全绿 → 推进 RELIC-0 数据地基；查 LD-C 是否开工（boss_phase_player.gd + day31_level_design_data_check +§演出段）——若收口：确认 day18_19 48/48 + day30_boss_skill 49/49 零改动 → 推进 LD-E；D-26 是否收口（回归恢复 64/64 即 EXIT 门槛解冻）；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 67 轮（2026-08-19 12:00 · #2）拆解回执**：**高峰检查**：12:00 不在 09-12/14-18 高峰（09-12 已过、14-18 未到）→ 正常执行。**P0 检查** = PLAYTEST 追踪区增量 #89 后无新增（反馈专员 4h 一轮 10:38 轮未见新增量；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`60a4e96`（第 66 轮后 +7 = **#3 执行者 F1-E 批七全收口**：`8f6ecff` 7-1 turret_config 数据侧（GameData.xlsx +turret_config sheet 1 行 5.0/0.5/220.0 + data_schema 注册 + excel_export presentation 第 6 键，其余 16 JSON 零 diff）/ `c0606e1` 7-2 get_turret_config 懒加载接口（白盒 9/9）/ `c8ad1b7` 7-3 turret 消费改读（TURRET_DEFAULTS const 收敛 :13-15 编译期一致 + _resolve_turret_defaults + setup 三处装载兜底改走，day13 炮台段 6b 零改动 36/36）/ `afce477` 7-4 探针 +§9 turret 段 **18 断言**（316→**334/334** + runner expect 316→334 + day26 锚点 1644→1662）/ `77d7928` 方案师第 36 轮锚点复核入库 / `9a6fb40` #1 进度分析第 70 轮（批六标记修正 6/7→5/7 实锤批五未落地口径已过时——批六已收口）/ `60a4e96` **Day31-F1-E-7-EXIT 收口**（F1-E 行 **7/7 批标记 = 阶段 F 全闭 🎉** + T-024 转已收口 + SOLUTION_PLAN 底部第 67 轮执行结果：回归 59/64 挂 D-26 零新增 + 334/334 + BASELINE CLEAN + 锚点 1662））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 未跟踪目录）+ #4 TEST_REPORT.md M **一律不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **F1-E 批七收口确认（上轮「批七跨 1 轮挂账」解除）= 阶段 F 全闭 🎉**：F1-E-7-1~4/EXIT 全 [x]（T-024 turret_config 表 + TURRET_DEFAULTS const 兜底 + day13 炮台段 6b 零改动；**执行登记**：weapons se_auto_turret duration=20 未消费键登记观察不拆入；回归 59/64 5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API 零新增）→ **F1-E 7/7 批 + 阶段 F 全闭**（30 天计划 Day 1-30 客观任务 + 阶段 F 技术债全部收口，剩余 Day 30 尾项纯 Owner/#4 域）。**RELIC 全批仍零开工**（git 无 day31_relic_*/stats 改名提交）→ 挂账观察（跨 **6** 轮）。**LEVEL_DESIGN LD-C/E 仍零开工**（git 无 boss_phase_player.gd/attr 分支提交）→ 挂账观察（LD-C 跨 2 轮）；LD-D 可选批维持待评估。**D-26 回归阻塞维持**（59/64 5 FAIL 清单与上轮完全一致零新增，D-020 不代修待收口；批七 EXIT 门槛统一挂等复跑恢复 64/64 后全绿）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A/B + 批六/批七）→ 待下次打包验证（D-016 授权自动替换已生效，等 #3/总指挥产出新版本后归档重导出）。**回归硬门槛口径 = 64 件套 1662 锚点**（批七收口 day26 1644→1662；当前 59/64 挂 D-26）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ **F1-E 批七收口确认（阶段 F 全闭 🎉，上轮挂账解除）** ② **AF-P1 预留登记维持**（SPEC §二 F3 命中粒子/数字 P1——F-11 伤害数字子系统 + F-45 hit 特效渐隐已覆盖命中反馈面，持续判断不拆）③ **登记「无待拆」不空转**（RELIC 全批 / LD-C·E 全部已拆已定案，承接方 #3 直接执行；F1-E 7/7 全闭；Day 30 纯 Owner/#4 域）④ Day 30 区补第 67 轮确认块 ⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：RELIC（A 先行）→ LD-C→E 执行序维持；D-26 用户会话收口后复跑回归恢复 64/64 即各批 EXIT 门槛解冻。
>
> **下轮起手**：查 RELIC-A 是否开工（git log 出现 stats sheet 改名 + day31_relic_name）——若收口：确认 stats.json id 零改动 + 回归全绿 → 推进 RELIC-0 数据地基；查 LD-C 是否开工（boss_phase_player.gd + day31_level_design_data_check +§演出段）——若收口：确认 day18_19 48/48 + day30_boss_skill 49/49 零改动 → 推进 LD-E；D-26 是否收口（回归恢复 64/64 即 EXIT 门槛解冻）；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 66 轮（2026-08-19 08:0x · #2）拆解回执**：**高峰检查**：08:0x 不在 09-12/14-18 高峰 → 正常执行。**P0 检查** = PLAYTEST 追踪区增量 #89 后无新增（反馈专员 4h 一轮，06:38 轮空转零产出符合 D-018；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无新用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`15390de`（第 65 轮后 +1 = **#3 执行者 F1-E 批六全收口标记**：`38f7c2e`→`c2b5a0e` 批六 6-1~6-5 全 [x]（T-004 starting_gun 抽表 + T-022 skill_icon_map 表，探针 316/316，回归 59/64 挂 D-26 零新增）+ `15390de` **Day31-F1-E-6-EXIT 收口**（F1-E 行 6/7 批标记 + T-004/T-022 转已收口 + SOLUTION_PLAN 底部执行结果 + 方案师第 35 轮批七正式方案挂账一并入库））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）+ #4 TEST_REPORT.md M + PROGRESS.md M（#1）**一律不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **F1-E 批六收口确认（上轮挂账解除）+ 批七待执行**：F1-E-6-1~5+EXIT 全 [x]（6/7 批标记；**执行登记**：starting_gun 入表致 day7/day8 武器全量断言 FAIL → 两探针豁免占位初始枪复跑全绿 + shop.gd 排除 starting_gun 防商店池 +1；回归 59/64 = D-26 在途零新增）→ **F1-E 仅剩批七 炮台默认值（F1-E-7-1~4/EXIT，第 65 轮已拆 + 方案师第 35 轮方案已定）待 #3 执行**（git 实测无 turret_config 提交，跨 1 轮挂账）；**批七收口 = F1-E 7/7 批 + 阶段 F 全闭**（T-024 转已收口 + F1-E 行标记）。**RELIC 全批仍零开工**（git 无 day31_relic_*/stats 改名提交）→ 挂账观察（跨 **5** 轮）。**LEVEL_DESIGN LD-C/E 仍零开工**（git 无 boss_phase_player.gd/attr 分支提交）→ 挂账观察（LD-C 跨 1 轮）；LD-D 可选批维持待评估（D4 拍板成本 >0.5 天降级 TECH_DEBT）。**D-26 回归阻塞维持**（59/64 5 FAIL = 用户会话在途 `set_frame_offset` 4.4 API，清单与上轮完全一致零新增，D-020 不代修待收口）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A/B + 批六）→ 待下次打包验证。**回归硬门槛口径 = 64 件套 1614 锚点**（当前 59/64 挂 D-26，各批 EXIT 门槛统一挂等复跑恢复 64/64 后全绿）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ F1-E 批六收口确认（上轮挂账解除，6/7 批）+ D-020 回归阻塞维持登记（59/64 与上轮清单一致零新增）② **F-49 传送门+宝箱地基落地确认 → RELIC-E1 行加注衔接**（`4f1e791`：wave_manager 敌全灭/Boss 死不再立即结算 → `_open_exit_portal` + 玩家 `enter_portal` 手动推进 + 新 loot_chest.gd 金色方块占位金币 50/经验 30，day31_portal_check 24/24 + 回归 62/62；**RELIC-E1「章 Boss 战后宝箱三选一」落地时：传送门手动推进语义已就绪 + loot_chest 奖励直接升级为遗物三选一，机制地基零重做**）③ **登记「无待拆」不空转**（F1-E 批七 / RELIC 全批 / LD-C·E 全部已拆已定案，承接方 #3 直接执行；Day 30 纯 Owner/#4 域）④ Day 30 区补第 66 轮确认块 ⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：F1-E 批七 → RELIC（A 先行）→ LD-C→E 执行序维持；D-26 用户会话收口后复跑回归恢复 64/64。
>
> **下轮起手**：查批七是否开工（git log 出现 turret_config sheet / get_turret_config / day31_presentation +§9 turret 段）——**若收口**：F1-E 7/7 批 + **阶段 F 全闭确认**（T-024 收口 + F1-E 行标记）；查 RELIC-A 是否开工（stats sheet 改名 + day31_relic_name）——若收口：确认 stats.json id 零改动 + 回归全绿 → 推进 RELIC-0 数据地基；查 LD-C 是否开工（boss_phase_player.gd + day31_level_design_data_check +§演出段）——若收口：确认 day18_19 48/48 + day30_boss_skill 49/49 零改动 → 推进 LD-E；D-26 是否收口（回归恢复 64/64 即 EXIT 门槛解冻）；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 65 轮（2026-08-19 06:00 · #2）拆解回执**：**高峰检查**：06:00 不在 09-12/14-18 高峰 → 正常执行。**P0 检查** = PLAYTEST 追踪区增量 #89 后无新增（反馈专员 4h 一轮，02:38 空转零产出符合 D-018，下轮 06:38；F-45~F-49/AF-P0 全 🟢 待真人回归，🟡 仅 H-05 家族主观审阅域）——**🔴P0 无新增 / 🟠 无新用户拍板调度指令 → 无新机器可验证 P0 需拆**；美术资源策略遵守（本轮零美术生成）。
>
> **目标日 = Day 30 发布准备（收尾）+ Day 31+ 独立窗口（RELIC/LEVEL_DESIGN 同窗口）**：git 实测 HEAD=`b213296`（第 64 轮后 +1 = **#3 执行者第 65 轮 LEVEL_DESIGN LD-B 固定出生点生成收口**：`enemy_spawner._get_spawn_position` 表驱动主路径（edge/anchor/ring 三型解析 + sequence 轮换/random 组内随机 + min_dist 过近原样生成 + `_clamp_to_ground` 钳制）+ `_get_random_spawn_position` 原函数保留缺省回退 F-48 零回归 + `wave_manager.spawn_wave` 透传 spawn_set/spawn_order（可选参数缺省空兼容旧调用）+ day31_level_design_data_check +§6 出生点 **12 断言（36/36）** + runner 24→**36** + day26 锚点 64/**1614** + PLAYTEST 主观项登记（LD-B 固定出生点行）+ 回归 **59/64（5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API，零新增）**）。工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）**不碰**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。
>
> **LD-B 收口确认（上轮挂账解除）**：LD-B1/B2/B3 全 [x]（`b213296`）+ **LD-B-EXIT [~] 部分收口**（探针 36/36 + day31_flee_bound **22/22 零改动**（F-48 不回归）+ wave5_repro 9/9 + excel_export --check-only EXIT=0 纯消费端零数据改动；**仅回归硬门槛 59/64 挂 D-26 用户会话在途，D-020 处置 = 不代修待收口，复跑恢复 64/64 后转 [x]**）→ **LD-C（Boss 阶段演出）/ LD-E（attr 正向状态）/ LD-D（可选）仍 [ ] 待执行**（已拆已定案，承接方 #3）。**F1-E 批五 SHEET_CONFIG 已拆解仍未开工**（git 无 icon_config 提交，跨 **2** 轮）**+ 批六 初始武器已拆解仍未开工**（git 无 starting_gun/skill_icon_map 提交，跨 **1** 轮）→ 挂账观察，承接方 #3。**RELIC 全批仍零开工**（git 无 day31_relic_*/stats 改名提交）→ 挂账观察（跨 **4** 轮）。⚠️ build/ 仍 08-18 23:22 产物（HEAD=`2aeb717`，不含其后 F-49 + F1-E-4 消费端 + LD-A/B）→ 待下次打包验证。**回归硬门槛口径 = 64 件套 1614 锚点**（LD-B runner 扩容；当前 59/64 5 FAIL 挂 D-26，各批 EXIT 门槛统一挂等复跑恢复 64/64 后全绿）。
>
> **本轮动作**：① P0 结论（无新机器可验证 P0）+ LD-B 收口确认（上轮挂账解除，flee_bound 22/22 零改动 = F-48 不回归实证）+ D-020 回归阻塞维持登记（59/64 5 FAIL 与上轮清单完全一致零新增）② **F1-E 第七批 炮台默认值函数级拆解（本轮主产出，见 F1-E 行下方 F1-E-7）**——T-024 turret.gd:13-15 字段默认 + setup 兜底两处散落与 se_auto_turret 数据重复 → **turret_config 表**（1 行 damage/fire_interval/attack_range，与 const 现值 5.0/0.5/220.0 逐一一致）→ data_schema 注册 → excel_export presentation 第 7 键 → DataLoader.get_turret_config（懒加载）→ turret.gd `_resolve_turret_defaults`（DataLoader 命中优先 / 空表回退 TURRET_DEFAULTS const 兜底 = 现字段值，**day13_build_check 炮台段 6b 零改动硬门槛**）；**duration 15.0 不拆入本批**（属 skill 域 skill_controller:255 `sd.get("duration", 15.0)`，weapons se_auto_turret duration=20 为未消费键 = build_weapon_from_data :162 先例，登记观察）；→ day31_presentation +§9 turret 段 → 回归 64 件套；标「承接方 = #3 执行者」③ Day 30 区补第 65 轮确认块 ④ F1-E 行刷新（批七拆解标注）⑤ 提交 TASKS.md 单文件（工作区用户会话在途不碰）⑥ 交 #3：LD-C→E 执行序（LD-B 已收口解锁，已拆已定案直接执行）；F1-E 批五→六→七连续推进（三批拆解齐备）；RELIC 全批待执行；D-26 用户会话收口后复跑回归恢复 64/64。
>
> **下轮起手**：查 LD-C 是否开工（git log 出现 `boss_phase_player.gd`/phase 钩子接线）——**若收口**：确认 day18_19_boss_check 48/48 + day30_boss_skill 49/49 零改动 → 推进 LD-E；查批五/批六是否开工（icon_config/starting_gun/skill_icon_map 提交）——**若批六收口**：F1-E 6/7 批标记（批七已拆可连续）；D-26 是否收口（回归恢复 64/64 即 EXIT 门槛解冻）；RELIC 是否开工；D30 上传/收口仍等 Owner；build/ 重打验证；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 62 轮（2026-08-19 00:05 · #2）拆解回执**：**P0 检查** = PLAYTEST 追踪区增量 #89（23:4x 反馈专员：**用户 08-18 23:3x 拍板 F-49 通关不突兀落地登记**——`4f1e791` 传送门延迟结算 + 掉落宝箱（wave_manager 敌全灭/Boss 死不再立即 `_end_wave` → `_open_exit_portal` + 玩家 `enter_portal` 才走结算链；新 `exit_portal.gd` 紫色圆环占位 + `loot_chest.gd` 金色方块占位金币 50/经验 30，**RELIC-E 三选一后续叠加**）；护栏 day31_portal_check **24/24** + **全量回归 62/62 · 1534 断言全绿**；F-49 主观回归面已登记表格行）——**🔴P0 无新增机器可验证项 / 🟠 用户拍板调度指令 = `docs/LEVEL_DESIGN_SPEC.md`（用户 08-18 22:57 拍板：① 刷新位置改**固定出生点 spawn_points 表**（非随机参数化）② 独立 **boss_phase_events 演出表**（横幅/特效/音效/台词/震屏/buff）③ **正向属性状态 attr** 通用型一次到位；总指挥第 6 轮 `57ff1d8` 明示「交 #2 00:05 拆解」）→ 本轮核心拆解 = LEVEL_DESIGN 批**；美术资源策略遵守（本轮零美术生成，点位/演出全部占位纯色/文字口径）。
>
> **目标日 = Day 30 发布准备（收尾）· 但 🟠 LEVEL_DESIGN 规格为新用户拍板调度指令 → 独立目标日（Day 31+）函数级拆解（本轮主产出，见文末「LEVEL_DESIGN 关卡设计扩展」区块，与 RELIC 同窗口）**：git 实测 HEAD=`144321a`（反馈专员 #89；第 61 轮后 +8 = **总指挥第 6 轮 5 连 + F-49**：`57ff1d8` **LEVEL_DESIGN_SPEC 新规格入库**（D-014 §8 五决策点拍板：inset 40 / anchor(0.5,0.3) / dialogue 横幅占位 / 特殊波挂 TECH_DEBT / attr 实测登记 + D-015 遵用户 23:1x 指令「#3 直接执行」交接）/ `2aeb717` **F1-E-4-1 检查点**（fx_config sheet 落地）/ `08ee5f0` **D-016 build/ 替换常设授权落档**（build/ 已按新约定执行：归档 `RoguelikeStudio_20260818_1956_archive` → 导出 HEAD=`2aeb717` RELEASE OK，**D30-T3 本地替换阻塞解除**）+ `83a27a7` D-017 审批制 + `a95c73e` D-018 反馈专员 2h→4h + `a9a8c6f` D-019 复用铁律 / `4f1e791` **F-49 传送门+宝箱** / `144321a` 增量 #89）；工作区 CLEAN 零在途。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆（维持）**。**F1-E 4/7 批**（SPRITE_MAP+BEHAVIOR_MAP+BGM/SFX 闭环）：**F1-E-4-1 [x]（总指挥第 6 轮）** + **F1-E-4-2~4-4/EXIT 仍 [ ]，承接方 = #3 执行者（用户 08-18 23:1x 拍板解除「#3 勿自行开工」历史约定，D-015 已交接）**。**RELIC 全批已拆解 + 方案已定（SOLUTION_PLAN 第 31 轮），承接方 = #3 执行者，git 实测仍未开工（无 day31_relic_* 提交）→ 挂账观察**。**⚠️ build/ 观察：D-016 授权后 build/ 已自动替换为 HEAD=`2aeb717` 产物（含 F-45/46/47/48 + F1-E-4-1），但不含其后 F-49（`4f1e791`）→ 传送门/宝箱需等下次打包验证**。回归硬门槛 = 62 件套 **1534 断言**。
>
> **本轮动作**：① 增量 #89 P0 结论（F-49 机器侧闭环，主观回归面已登记）+ F-49 落地确认 ② **LEVEL_DESIGN_SPEC 函数级拆解（本轮主产出，见文末 LEVEL_DESIGN 区块）**——批 A 数据地基（两新 sheet + excel_export 注册/FK 校验 + DataLoader 接口 + 探针）→ 批 B 固定出生点（enemy_spawner 按点位生成 + 缺省回退 + F-48 不回归）→ 批 C Boss 演出（boss_phase_player 执行器 + phase 钩子接线 + 探针）→ 批 E attr 正向状态（status_component 扩展 + elements 示例行 + buff 事件联动）→ 批 D 特殊波参数化（可选，成本评估）→ LD-EXIT；每批函数级（文件/改动点/探针/回归门槛），数据管线铁律全程标注（改数走 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）；标「独立目标日 Day 31+，与 RELIC 同窗口，承接方 = #3 执行者」③ Day 30 区补第 62 轮确认块 ④ F1-E 行刷新（批四 4-1 [x] + 承接方 #3）⑤ 提交 TASKS.md 单文件（工作区 CLEAN 零在途）⑥ 交总指挥/#3：LEVEL_DESIGN 拆解就绪（建议执行序 A→B→C→E→D 可选）；F1-E 批四 4-2 起按拆解续做；RELIC 全批待执行。
>
> **下轮起手**：查 #3 是否按 F1-E-4-2 开工（git log 出现 get_fx_config / vfx_player 改读 / day31_presentation_check +§6 fx 段）——**若收口**：F1-E 4/7 批标记 + 批五 SHEET_CONFIG（icon_atlas.gd:8-24，T-020）拆解；查 RELIC-A 是否开工（stats sheet 改名 + day31_relic_name）；查 LEVEL_DESIGN-A 是否开工（git log 出现 spawn_points/boss_phase_events sheet / get_spawn_points / day31_level_design_* 探针）——若批 A 收口：确认两 JSON 零漂移 + FK 校验报错生效 → 推进批 B/C；**D30 上传/收口仍等 Owner 拍板**（D30-T3 上传确认 + D30-EXIT 发布摘要落盘）；build/ 是否重打含 F-49；新 P0（🔴 用户拍板调度指令）→ 优先拆解。
>
> 📌 **第 32 轮（2026-08-19 00:4x · 方案师）确认块**：P0 = PLAYTEST 增量 #89（F-49 传送门+宝箱落地登记）无新增机器可验证项 → **🔴P0 无新增**；🟠 **LEVEL_DESIGN 规格（用户 08-18 22:57 拍板）已由 #2 第 62 轮（`9ebbf5f` 00:05）函数级拆解 → 本轮方案已定（SOLUTION_PLAN.md 第 32 轮，锚点实测复核 9 项逐一一致 + 2 处行号微差修正：status_component _revert :157→:160 / get_enemy_sprite_config :600-604→:605）**——批 A 数据地基 → 批 B 固定出生点 → 批 C Boss 演出 → 批 E attr → 批 D 可选 → LD-EXIT，承接方 = #3 执行者（D-015），回归硬门槛 62 件套 1534 断言；**F1-E 批四 4-1 [x] 已收口**（总指挥第 6 轮 `2aeb717`/`716a9d8` fx_config 数据侧 10 行）→ **上轮「批四零开工」挂账部分解除，剩 4-2~4-4/EXIT 消费端待 #3**；RELIC 全批方案已定（第 31 轮）git 实测仍零开工（HEAD=`9ebbf5f` 无 day31_relic_*）→ 挂账观察维持（跨 1 轮）；D30-T3 上传 + D30-EXIT = 纯 Owner/#4 域；⚠️ build/ 已替换为 `2aeb717` 产物（含 F-45~48 + F1-E-4-1）但不含 F-49（`4f1e791`）→ 传送门/宝箱需等下次打包验证。工作区在途 docs/TEST_REPORT.md（#4）+ art_ai 工具（美术域）一律不碰；本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
...
### Day 30 — 发布准备　🎯 **当前目标日 · 第 50 轮已函数级拆解（方案已定：SOLUTION_PLAN.md 第 26 轮 + 第 27 轮 + 第 28 轮 + 第 29 轮 + 第 30 轮 + 第 31 轮）**
> 📌 **方案师第 39 轮确认块（2026-08-19 20:4x · 方案师）**：**高峰检查**：20:37 不在 09-12/14-18 高峰 → 正常执行。**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 06:38/10:38/14:38/18:38 全空转零产出符合 D-018，git 无 #90 提交；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = RELIC-A 收口确认（SOLUTION_PLAN.md 第 39 轮）**——git 实测 HEAD=`39662ba`（#2 第 69 轮回执）：**RELIC-A1/A2/A3/EXIT 全 [x]**（`765e3bc` stats 两 name 改名——元素伤害→魔法伤害 / 工程学→机械学，id 零改动 + desc_builder STAT_CN 两键同步统一机械学 + 三 JSON 仅文案变零 diff；`74ad1fd` day31_relic_name_check **15/15** + runner 64→**65 件套** + day26 锚点 1677→**1692** + 回归 **60/65** 挂 D-26 零新增）→ **RELIC 首批落地，跨 8 轮零开工挂账部分解除**（第 31 轮方案 RELIC-A 兑现：desc_builder.gd:32-33 硬编码映射改名两处同步 = STAT_CN 两键统一机械学）。**RELIC 剩余 7 批（0/F/E/B/C/D/EXIT）+ LD-C/E/D 全部已拆已定案不重写**（执行序 = RELIC-0 先行 → F/E → B/C/D → EXIT；LD-C 跨 4 轮挂账维持），承接方 #3 直接执行。**回归硬门槛 = 65 件套 1692 锚点**（当前 60/65 5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API 误用，与 #63~#68 同根因零新增，D-020 不代修待收口，复跑恢复 65/65 后各批 EXIT 门槛解冻）；⚠️ build/ 08-18 23:22（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五~七 + RELIC-A 交 Owner/总指挥（D-016 授权自动替换已生效）。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> 📌 **第 70 轮（2026-08-19 22:00 · #2）确认**：git 实测 HEAD=`de0f563`（总指挥第 8 轮；第 69 轮后 +6 = **#3 执行者第 70 轮 RELIC-0 数据地基全收口**：`01a27c7` 0-1（items relic +6 新列 + 套装 2 套 4 件 + 流派 6 件占位 + set_effects 解析，其余 16 JSON 零 diff）+ `6c54857` 0-2/0-3+EXIT（get_relic_defs/get_relic_set_ids + meta 两键 + day31_relic_data **55/55** + runner 65→**66 件套** + day26 锚点 1692→**1748**）+ `55fcc5a` 回归同步（day18_feedback2 flaky 根治 + day23/day24 锚点同步，回归 **61/66** 挂 D-26 零新增）+ `be647b4` 0 收口挂账 + `de0f563` 总指挥第 8 轮（D-021 RELIC-F/E 下一卡点 + build 重打挂账））；工作区 = 用户会话在途（lain 动画帧 / bg 背景图 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M 一律不碰。**🟠 无新用户拍板调度指令 → 本轮为状态回执**；**RELIC-0 收口确认（RELIC 双批 A/0 落地）**：剩余 RELIC-F/E/B/C/D/EXIT 已拆已定案待 #3；**LD-C·E·D 待执行挂账维持**（LD-C 跨 5 轮）；D-26 回归阻塞维持（61/66，D-020 不代修待收口）。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七 + RELIC-A/0 → 总指挥 D-021 build 重打挂账维持（D-016 授权自动替换已生效）。回归硬门槛 = 66 件套 **1748 锚点**（当前 61/66 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **方案师第 40 轮确认块（2026-08-19 22:4x · 方案师）**：**高峰检查**：22:37 不在 09-12/14-18 高峰 → 正常执行。**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 18:38 空转零产出符合 D-018，22:38 轮未到，git 无 #90 提交；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = RELIC-0 数据地基收口确认（SOLUTION_PLAN.md 第 40 轮）**——git 实测 HEAD=`60ab765`（#2 第 70 轮回执）：**RELIC-0-1/0-2/0-3/EXIT 全 [x]**（`01a27c7` items relic 类条目 +6 新列——tag/tier/set_id/set_tier/set_effects/unlock_condition（set_effects 分隔串→JSON 数组解析 = 第 31 轮方案定案兑现）+ 套装 2 套 4 件占位（星骸孤注/死线舞者 price=0 不进商店池）+ 流派 6 件占位，其余 16 JSON 零 diff；`6c54857` get_relic_defs/get_relic_set_ids 懒加载接口 + meta 两键缺省容错旧档零崩 + day31_relic_data_check **55/55** + runner 65→**66 件套** + day26 锚点 1692→**1748**；`55fcc5a` day18_feedback2 flaky 根治（零游戏逻辑改动）+ day23/day24 锚点同步 + 回归 **61/66** 挂 D-26 零新增）→ **RELIC 双批（A/0）落地 = 遗物扩展 2/7 批，上轮挂账部分解除**（第 31 轮方案 RELIC-0 兑现：save_compat 14/14 + day27_meta 35/35 零改动硬门槛 + set_effects 定案 + price=0 防泄漏 + f3 白名单全部按方案执行）。**RELIC 剩余 6 批（F/E/B/C/D/EXIT）+ LD-C/E/D 全部已拆已定案不重写**（执行序 = RELIC-F/E（P0 独立·总指挥 D-021 已标下一卡点）→ B/C/D → EXIT；LD-C 跨 5 轮挂账维持），承接方 #3 直接执行。**回归硬门槛 = 66 件套 1748 锚点**（当前 61/66 5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API 误用，与 #63~#69 同根因零新增，D-020 不代修待收口，复跑恢复 66/66 后各批 EXIT 门槛解冻）；⚠️ build/ 08-18 23:22（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五~七 + RELIC-A/0 → 总指挥 D-021 已登记 build 重打挂账（等 D-26 收口后统一重打，D-016 授权自动替换已生效）。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> 📌 **第 69 轮（2026-08-19 20:00 · #2）确认**：git 实测 HEAD=`74ad1fd`（第 68 轮后 +2 = **#3 执行者第 69 轮 RELIC-A 全收口**：`765e3bc` A1+A2（stats 两 name 改——元素伤害→魔法伤害 / 工程学→机械学，id 零改动）+ `74ad1fd` A3+EXIT（day31_relic_name_check **15/15** + runner 64→**65 件套** expect15 + day26 锚点 1677→**1692** + **回归 60/65** 挂 D-26 零新增 + RELIC-A1~EXIT 全 [x]））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M 一律不碰。**🟠 无新用户拍板调度指令 → 本轮为状态回执**；**RELIC-A 收口确认（RELIC 首批落地）**：剩余 RELIC-0/F/E/B/C/D/EXIT 已拆已定案待 #3；**LD-C·E·D 待执行挂账维持**（LD-C 跨 4 轮）；D-26 回归阻塞维持（60/65，D-020 不代修待收口）。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七 + RELIC-A → 待下次打包验证（D-016 授权自动替换已生效）。回归硬门槛 = 65 件套 **1692 锚点**（当前 60/65 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **第 68 轮（2026-08-19 18:00 · #2）确认**：git 实测 HEAD=`d03750e`（**#3 执行者 F1-E 批五 SHEET_CONFIG 全收口 = 阶段 F 真全闭 🎉**：`d990eca`→`5b92fd3` 批五 5-1~5-4 全 [x]（T-020 icon_config 表 + get_icon_config + IconAtlas 消费改读 + SHEET_CONFIG const 兜底，探针 349/349 + day26 锚点 1662→1677）+ `d03750e` EXIT 收口（F1-E 行 7/7 批 + T-020 转已收口 + SOLUTION_PLAN 底部第 68 轮执行结果，回归 59/64 挂 D-26 零新增；**#1 第 71 轮「名义 7/7 实为 6/7」更正由实际执行闭环**））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 摩托车/ 未跟踪目录）+ #4 TEST_REPORT.md M 一律不碰。**🟠 无新用户拍板调度指令 → 本轮为状态回执**；**F1-E 批五收口确认（上轮更正挂账解除）→ 阶段 F 真全闭**（30 天计划 + 技术债全部客观收口）；RELIC 全批跨 7 轮 / LD-C·E 待执行挂账维持；D-26 回归阻塞维持（59/64，D-020 不代修待收口）。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五/六/七 → 待下次打包验证（D-016 授权自动替换已生效）。回归硬门槛 = 64 件套 **1677 锚点**（当前 59/64 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **方案师第 38 轮确认块（2026-08-19 18:4x · 方案师）**：**高峰检查**：18:37 不在 09-12/14-18 高峰 → 正常执行。**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 14:38 轮 git 无 #90 提交 = 空转零产出符合 D-018；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = 阶段 F 实测真全闭确认（SOLUTION_PLAN.md 第 38 轮）**——git 实测 HEAD=`e31e0bc`（#1 第 72 轮）：**F1-E 批五 SHEET_CONFIG 全收口确认**（`d990eca`~`d03750e` 5 commits 全 [x]：icon_config 3 行抽表 + get_icon_config 懒加载 + IconAtlas 消费改读 + §10 探针 15 断言 **349/349** + day26 锚点 1662→**1677** + EXIT 标 7/7 批）→ **上轮「名义 7/7 实为 6/7」更正挂账解除 = 阶段 F 实测真全闭 🎉**（#1 第 72 轮磁盘实测 7 键齐 + 消费端 5 文件命中确认；方案师第 37 轮 5 处锚点更新全部兑现）→ **无新任务需方案化**（F1-E 全 7 批收口；RELIC/LD 方案已定 31/32 轮不重写；D30 尾项纯 Owner/#4 域）。**挂账刷新**：RELIC 全批零开工（**跨 8 轮**）/ LD-C·E·D 零开工（**LD-C 跨 4 轮**）维持，承接方 = #3；**回归硬门槛 = 64 件套 1677 锚点**（当前 59/64 5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API 误用，与 #63~#68 同根因零新增，D-020 不代修待收口，复跑恢复 64/64 后各批 EXIT 门槛解冻）；⚠️ build/ 08-18 23:22（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批五~七 交 Owner/总指挥（D-016 授权自动替换已生效）。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> 📌 **方案师第 37 轮确认块（2026-08-19 12:4x · 方案师）**：**高峰检查**：12:37 不在 09-12/14-18 高峰 → 正常执行。**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 10:38 空转零产出符合 D-018；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = F1-E 批七收口确认 + 批五 SHEET_CONFIG 方案锚点复核更新（SOLUTION_PLAN.md 第 37 轮）**——**批七收口确认**（`8f6ecff..60a4e96` 5 commits 全 [x]：T-024 turret_config 全链落地 + 334/334 + files dict 第 6 键 + day26 锚点 1662，上轮挂账解除）；**⚠️ 阶段 F 名义全闭更正**：#2 第 67 轮「7/7 全闭🎉」vs #1 第 71 轮「实测 6/7」矛盾 → 本轮 4 文件逐一实证（excel_export files dict 6 根键无 icon_config / data_schema 无 icon_config 注册 / data_loader 无 get_icon_config / icon_atlas SHEET_CONFIG :8-24 原样）确认 **批五 SHEET_CONFIG 跨 4 轮仍未落地 → 实际 6/7，F1-E 行「阶段 F 全闭🎉」标记已更正（交总指挥核实）**；**批五方案锚点更新 5 处**：files dict → 第 7 键 / data_schema 注册 → turret_config 后约 :285+ / data_loader 仿写 → get_turret_config 后约 :716+ / 探针段号 → §10（334→≥347，runner expect 334→347）/ 回归硬门槛 → 64 件套 1662 锚点。**挂账刷新**：批五（跨 4 轮，阶段 F 真全闭最后一批）/ RELIC（跨 6 轮零开工）/ LD-C·E（已拆已定案待 #3，LD-C 跨 2 轮）维持；D30-T3/EXIT = 纯 Owner/#4 域维持。⚠️ build/ 08-18 23:22（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批六/批七 交 Owner/总指挥。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
> 📌 **第 67 轮（2026-08-19 12:00 · #2）确认**：git 实测 HEAD=`60a4e96`（**#3 执行者 F1-E 批七全收口 = 阶段 F 全闭 🎉**：`8f6ecff`→`afce477` 批七 7-1~7-4 全 [x]（T-024 turret_config 表 + TURRET_DEFAULTS const 兜底 + day13 炮台段 6b 零改动，探针 334/334 + day26 锚点 1644→1662）+ `60a4e96` EXIT 收口（F1-E 行 7/7 批 + T-024 转已收口 + SOLUTION_PLAN 底部第 67 轮执行结果，回归 59/64 挂 D-26 零新增））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源** + 人物动画/ 未跟踪目录）+ #4 TEST_REPORT.md M 一律不碰。**🟠 无新用户拍板调度指令 → 本轮为状态回执**；**F1-E 批七收口确认（上轮挂账解除）→ 阶段 F 全闭**（30 天计划 + 技术债全部客观收口）；RELIC 全批跨 6 轮 / LD-C·E 待执行挂账维持；D-26 回归阻塞维持（59/64，D-020 不代修待收口）。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批六/批七 → 待下次打包验证（D-016 授权自动替换已生效）。回归硬门槛 = 64 件套 **1662 锚点**（当前 59/64 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **第 66 轮（2026-08-19 08:0x · #2）确认**：git 实测 HEAD=`15390de`（**#3 执行者 F1-E 批六全收口标记**：`38f7c2e`→`c2b5a0e` 批六 6-1~6-5 全 [x]（T-004 starting_gun 抽表 + T-022 skill_icon_map 表，探针 316/316 + day7/day8 豁免占位 + shop 排除初始枪，回归 59/64 挂 D-26 零新增）+ `15390de` EXIT 收口（F1-E 行 6/7 批 + T-004/T-022 已收口 + 方案师第 35 轮批七方案挂账入库））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）+ #4 TEST_REPORT.md M + PROGRESS.md M 一律不碰。**🟠 无新用户拍板调度指令 → 本轮为状态回执 + F-49 地基衔接加注（RELIC-E1 行）**；**F1-E 批六收口确认（上轮挂账解除）→ 仅剩批七 炮台默认值（F1-E-7，已拆已定案）待 #3，批七收口 = 阶段 F 全闭**；RELIC 全批跨 5 轮 / LD-C·E 待执行挂账维持；D-26 回归阻塞维持（59/64，D-020 不代修待收口）。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批六 → 待下次打包验证。回归硬门槛 = 64 件套 **1614 锚点**（当前 59/64 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **第 65 轮（2026-08-19 06:00 · #2）确认**：git 实测 HEAD=`b213296`（#3 执行者第 65 轮 **LEVEL_DESIGN LD-B 固定出生点生成收口**：`_get_spawn_position` 表驱动主路径 + spawn_set/spawn_order 透传 + 探针 36/36 + runner 64 件套 1614 锚点 + 回归 59/64 5 FAIL = D-26 用户会话在途零新增）；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）不碰。**🟠 无新用户拍板调度指令 → 本轮核心产出 = F1-E 批七 炮台默认值函数级拆解（F1-E 最后一批 7/7，见 F1-E 行下方 F1-E-7，承接方 = #3 执行者：T-024 turret.gd:13-15 字段默认 + setup 兜底 → turret_config 表 + TURRET_DEFAULTS const 兜底 + duration 属 skill 域不拆入）**。**LEVEL_DESIGN LD-B 收口确认（上轮挂账解除）**：LD-B1/B2/B3 [x] + LD-B-EXIT [~]（仅回归挂 D-26，flee_bound 22/22 零改动 = F-48 不回归）→ **LD-C/E/D 已拆已定案解锁**。**F1-E 批五（跨 2 轮）+ 批六（跨 1 轮）已拆未开工 + RELIC 全批仍零开工（跨 4 轮）** → 挂账维持，承接方 #3。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物不含 F-49 + F1-E-4 消费端 + LD-A/B → 待下次打包验证。回归硬门槛 = 64 件套 **1614 锚点**（当前 59/64 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **方案师第 36 轮确认块（2026-08-19 08:4x · 方案师）**：**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮：02:38 空转零产出符合 D-018，06:38 轮 git 无 #90/#91 提交 = 同样空转零产出；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = F1-E 批七方案锚点复核更新（SOLUTION_PLAN.md 第 36 轮）**——第 35 轮方案已由 `15390de` 入库，**批六收口（`38f7c2e..15390de` 6 commits 全 [x]：316/316 + T-004/T-022 转已收口，上轮挂账解除）导致 4 处锚点漂移，本轮实测修正**：① weapons se_auto_turret 行号 :2668-2673→**:2680-2690**（数值 5/0.5/220 零漂移）② data_schema fx_config :261-262 保持 + **skill_icon_map :270-271 已落地** → turret_config 注册追加其后（约 :272+）③ excel_export files dict 现 **:525 · 5 根键**（无 icon_config = **批五 SHEET_CONFIG 未落地实锤**，#1 第 70 轮修正 5/7 成立）→ **批七 = 第 6 键（非第 7 键）** ④ day31_presentation **316/316** → 批七 +§9 = **≥324/324** · runner expect 316→324；**回归硬门槛 = 64 件套 1644 锚点**（批六收口 day26 1614→1644）。零漂移确认 5 项：turret.gd :13-15/:31-35（:33 maxf 钳制）/ skill_controller :243-280 / world.gd `scripts/world/world.gd` :104-112 / day13 炮台段 6b :629-657 / weapons 数值。**批七收口 = 阶段 F 全闭（7/7）**，承接方 #3。**挂账刷新**：批五（跨 3 轮实锤未落地）/ RELIC（跨 5 轮零开工）/ LD-C·E（已拆已定案待 #3）维持；F-49 地基已落地 → RELIC-E 三选一零重做。⚠️ build/ 08-18 23:22（`2aeb717`）不含 F-49 + F1-E-4 消费端 + LD-A/B + 批六 交 Owner/总指挥。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> 📌 **方案师第 35 轮确认块（2026-08-19 06:4x · 方案师）**：**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 02:38 空转零产出符合 D-018，下一轮 06:38 未到；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = F1-E 批七 炮台默认值正式方案（SOLUTION_PLAN.md 第 35 轮）**——实测复核锚点 6 项与第 65 轮拆解**逐一一致**，1 处行号微差修正 + 1 处细节补充：① data_schema.py fx_config 注册实位 **:261-262**（拆解写 :255-258，批五/批六落地后行号再漂移，执行者按实位追加勿覆盖）② turret.gd:3 头部注释已明示「数值全部来自 DataLoader.get_weapon("se_auto_turret")…禁止硬编码」= T-024 债务本质（字段声明 :13-15 + setup 兜底 :31-35 与 weapons 表重复），turret_config 表 = 默认值唯一事实源，方案成立。**关键决策复认**：turret_config 独立表 1 行（id=se_auto_turret，不直接消费 weapons.json）+ TURRET_DEFAULTS const 兜底 = day13 炮台段 6b :629-657 零改动硬门槛 + duration 属 skill 域不拆入（weapons se_auto_turret duration=20 未消费键登记观察）+ setup :33 maxf 钳制保留。**LD-B 收口确认（`b213296`）**：LD-B1/B2/B3 [x] + LD-B-EXIT [~] 仅回归挂 D-26（flee_bound 22/22 零改动 = F-48 不回归）→ **LD-C/E/D 已拆已定案（第 32 轮）解锁待 #3**。**批五（跨 2 轮）/ 批六（跨 1 轮）/ RELIC（跨 4 轮）仍零开工** → 挂账观察维持，承接方 = #3（D-015 交接，拆解+方案齐备即开工）。⚠️ build/ = 08-18 23:22（`2aeb717`：含 F-45~48 + F1-E-4-1，**不含 F-49 + F1-E-4 消费端 + LD-A/B**）→ 交 Owner/总指挥。**回归硬门槛 = 64 件套 1614 锚点（当前 59/64 挂 D-26，D-020 不代修待收口，批七 EXIT 以 D-26 复跑恢复后全绿为准）**。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
>
> 📌 **第 64 轮（2026-08-19 04:00 · #2）确认**：git 实测 HEAD=`5c306b1`（总指挥第 7 轮 03:00；第 63 轮后 +4 = **#3 执行者第 64 轮 LEVEL_DESIGN LD-A 数据地基收口** `96e4cd5`（spawn_points 11 行三型 + boss_phase_events 7 行 6 类型 + waves spawn_set/spawn_order 列 + FK 校验三态 + --check-only 只读修复 + 三接口 + 探针 24/24 + runner 64 件套 1602 锚点）→ `3509b74`（**回归阻塞登记 59/64：5 FAIL = 用户会话在途 D-26 `set_frame_offset` Godot 4.4 API 误用**，与 LD-A 零关联）+ `5c306b1` 总指挥第 7 轮（**D-020：不代修待收口，EXIT 门槛统一挂等复跑** + 审批 + LOOP_HEALTH 心跳））；工作区 = 用户会话在途（lain 动画帧 / art_ai 工具 / `player_anim.gd` + `sprite_frame_factory.gd` M = **D-26 阻塞源**）不碰。**🟠 无新用户拍板调度指令 → 本轮核心产出 = F1-E 批六 初始武器 + SKILL_ICON_MAP 函数级拆解（第 63 轮「批五收口后拆批六」约定提前兑现备好，见 F1-E 行下方 F1-E-6，承接方 = #3 执行者：T-004 starting_gun 抽表 source_id 留空 + T-022 独立 skill_icon_map 表 5 行 + const 兜底双硬门槛）**。**LEVEL_DESIGN LD-A 收口确认（上轮挂账解除）**：LD-A1/A2/A3 [x] + LD-A-EXIT [~]（仅回归挂 D-26）→ **LD-B/C/E/D 待执行**。**F1-E 批五 SHEET_CONFIG 已拆解仍未开工（跨 1 轮挂账）+ RELIC 全批仍零开工（跨 3 轮挂账）** → 维持观察，承接方 #3。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。⚠️ build/ 08-18 23:22 产物不含 F-49 + F1-E-4 消费端 + LD-A → 待下次打包验证。回归硬门槛 = 64 件套 **1602 锚点**（当前 59/64 挂 D-26，复跑恢复后全绿）。按下方批次收尾，勿重复拆解。
>
> 📌 **方案师第 34 轮确认块（2026-08-19 04:3x · 方案师）**：**P0 结论** = 增量 #89 后无新增量（反馈专员 4h 轮 02:38 空转零产出符合 D-018；🔴 无新增 / 🟠 无用户拍板调度指令，F-45~F-49/AF-P0 全 🟢 待真人回归）。**本轮核心产出 = F1-E 批六 初始武器 + SKILL_ICON_MAP 正式方案（SOLUTION_PLAN.md 第 34 轮）**——实测复核锚点 8 项与第 64 轮拆解**逐一一致**，2 处微差修正 + 1 处新阻塞点裁决：① `_equip_default_weapon` 实 :59-71 且字段 9 项（拆解写 :59-76/7 键）② **build_weapon_from_data :162 明确不消费 projectile_speed/lifetime**（weapons.json 36 武器零这两键）→ 拆解「7 键零漂移」隐含两键可装配假设不成立 → **裁决：两键不进表 + 消费端补设 360/1.5 对齐现值**（备选 C = builder 补消费键默认 400 兜底，可选不强制）③ excel_export files dict 现 :512 4 根键（LD-A 插入漂移，第 33 轮记 :449/5 键作废）→ 批六加第 6 键 skill_icon_map 勿覆盖。**LD-A 收口确认（`96e4cd5`）**：LD-A1/A2/A3 [x] + LD-A-EXIT [~] 仅回归挂 D-26 → **LD-B/C/E/D 已拆已定案（第 32 轮）解锁待 #3**。**批五挂账跨 1 轮 / RELIC 挂账跨 3 轮仍零开工** → 维持观察，承接方 = #3（D-015 交接，拆解+方案齐备即开工）。⚠️ build/ = 08-18 23:22（`2aeb717`：含 F-45~48 + F1-E-4-1，**不含 F-49 + F1-E-4 消费端 + LD-A**）→ 交 Owner/总指挥。**回归硬门槛 = 64 件套 1602 锚点（当前 59/64 挂 D-26，D-020 不代修待收口，批六 EXIT 以 D-26 复跑恢复后全绿为准）**。本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账，交下一岗入库。
> 📌 **第 63 轮（2026-08-19 02:0x · #2）确认**：git 实测 HEAD=`1ccb419`（#1 第 67 轮；第 62 轮后 +3 = **#3 执行者第 63 轮 F1-E 批四 FX 全收口**：`f67630b` 4-2~4-4 消费端（get_fx_config 懒加载 + vfx_player 改读 + §6 探针 286/286）+ `681ab36` 收口（**回归 63/63 · 1578 断言** + T-019 收口 + **F1-E 4/7 批** + portal 并入 runner）+ `1ccb419` #1 确认）；工作区 = 用户会话在途（lain 动画素材/AI 美术工具/2 脚本 M）不碰。**🟠 无用户拍板调度指令 → 本轮核心产出 = F1-E 批五 SHEET_CONFIG 函数级拆解（第 62 轮「批四收口后拆批五」约定兑现，见 F1-E 行下方 F1-E-5，承接方 = #3 执行者）**。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。RELIC（已拆已定案）+ LEVEL_DESIGN（已拆已定案）承接 #3 仍零开工 → 挂账观察维持。⚠️ build/ 08-18 23:22 产物不含 F-49 + F1-E-4 消费端 → 待下次打包验证。回归硬门槛 = 63 件套 **1578 断言**。按下方批次收尾，勿重复拆解。
> 📌 **第 62 轮（2026-08-19 00:05 · #2）确认**：git 实测 HEAD=`144321a`（反馈专员 #89；第 61 轮后 +8 = 总指挥第 6 轮 5 连（LEVEL_DESIGN_SPEC 入库/F1-E-4-1 检查点/D-016 build 授权/D-017 审批制/D-018 反馈 2h→4h/D-019 复用铁律）+ F-49 传送门+宝箱 `4f1e791` + 增量 #89）；工作区 CLEAN 零在途。**🟠 用户拍板调度指令 = LEVEL_DESIGN_SPEC（08-18 22:57 拍板 · 总指挥第 6 轮明示「交 #2 00:05 拆解」）→ 本轮已函数级拆解（见文末 LEVEL_DESIGN 区块，独立目标日 Day 31+ 与 RELIC 同窗口，承接方 = #3 执行者）**。**D-016 生效确认**：build/ 已自动替换为 HEAD=`2aeb717` 产物（RELEASE OK，旧产物归档 20260818_1956_archive）→ D30-T3 本地替换阻塞解除；⚠️ 但不含其后 F-49（`4f1e791`）→ 传送门/宝箱待下次打包验证。**Day 30 剩余 [ ] = 纯 Owner/#4 域**（D30-T3 上传等 Owner 明确确认 + D30-EXIT 发布摘要待 #4 落盘 + 最终标记）→ **无 #2 可拆**。按下方批次收尾，勿重复拆解。
> 📢 **用户拍板 2026-08-18 23:1x（主窗口记录）**：**解除「#3 勿自行开工」历史约定**（第 42 轮 2026-08-10 起 F1-E 系列标注「🏠 主窗口承接」，RELIC 拆解沿用同标注）——**拆解+方案齐备的任务一律由 #3 执行者按方案直接执行，该做就做**；总指挥回归调度/兜底角色（仅挂账超轮时介入）。**生效范围：F1-E 剩余 4 批（FX/SHEET_CONFIG/初始武器/炮台默认）+ RELIC 全批次（A/0/F/E/B/C/D/EXIT），承接方 = #3 执行者**。本约定自 2026-08-19 起全岗生效，历史轮次记录中的「🏠 主窗口承接」表述为当时口径，不再作为执行约束。
> 📌 **方案师第 31 轮（2026-08-18 22:4x）确认**：P0 = 增量 #85 无新增（F-01~F-46/AF-P0 全 🟢 待真人回归；🟡 仅 H-05 家族）；TEST_REPORT #61（22:00 · HEAD=`980aa7d`）= **62/62 全绿 · 1534 断言 · BASELINE CLEAN · 0 action item**。**🟠 RELIC_EXPANSION_SPEC 已由 #2 第 61 轮 `f19447c` 函数级拆解（O-1~O-4 全拍板：元素伤害→魔法伤害 / 工程学→机械学 / 玻璃大炮 / affinity 不可改+事件洗点 / 流派不限移速）→ 方案师本轮按规格写 RELIC 正式方案（SOLUTION_PLAN.md 第 31 轮：RELIC-A/0/F/E/B/C/D/EXIT 全批次，含实测复核新增确认）→ RELIC 区块标注「方案已定」**；**F1-E 批四 FX 仍未开工（git 实测 HEAD=`144b2bb` 最近 12 提交无 fx_config 相关，跨 2 轮挂账观察维持，承接方 🏠 主窗口/总指挥）**；**F-46 怪物追踪重写+HUD 分数制用户直派已落地（`5556cb3` 闭环，探针 22/22+17/17+回归 62/62 全绿）非本岗方案对象**；D30-T3/EXIT 纯 Owner/#4 域维持（**⚠️ F-45/F-46 未进 build/**——pck 08-18 19:56 早于 `bdd3ed5`/`5556cb3`，交 Owner/总指挥核实）。红线遵守：零游戏代码改动、不跑探针、不 git commit（本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账交下一岗入库）。
...
- [x] F1-E 表现配置抽表（T-016~024：SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX/FX/SHEET_CONFIG/初始枪/炮台默认 → Excel presentation sheet）【✅ 第一批 SPRITE_MAP 已收口（总指挥第 4 轮 2026-08-18 承接动工：GameData.xlsx 新增 enemy_sprites sheet 23 敌 + data_schema 注册 presentation.json/enemy_sprites + excel_export 构建 + DataLoader.get_enemy_sprite_config 消费（Vector2i/Color/scale 组装 + 未命中按 category 兜底 const）+ enemy.gd 改读；新探针 day31_presentation_check 246/246 全绿；day31_skill_icon_check 22/22（HUD se_skill_sword_arc 图标补丁）；两探针并入 runner 58→60 件套）· **✅ 第二批 BEHAVIOR_MAP 已收口（执行者第 56 轮 `b410a8b`：Excel enemy_behavior 表 9 行 + data_schema 注册 + excel_export 构建 + presentation.json behavior_map 9 条（13 JSON 零 diff）+ data_loader get_enemy_behavior（懒加载+枚举名解析+const 兜底）+ enemy.gd 行为解析改读 + 探针 261/261 + 回归 60/60）**· **✅ 第三批 BGM/SFX 已收口（总指挥第 5 轮 2026-08-18：audio_config 12 行抽表 + get_audio_config + audio_manager._resolve_audio_path 消费改读（const 兜底）+ 探针 273/273 + 回归 61/61 1504 断言 + baseline CLEAN，见下方 F1-E-3）**· **✅ 第四批 FX 已收口（2026-08-19 #3 执行者 `f67630b`：F1-E-4-1~EXIT 全闭环——fx_config sheet 10 行 → get_fx_config（懒加载+Vector2i 组装）→ vfx_player.set_effect 改读（FX_CONFIG const 兜底 = day23_vfx_check 18/18 零改动）+ 探针 +§6 fx 段 13 断言 286/286 + runner +portal 24 → **回归 63/63 1578 断言** + baseline CLEAN + T-019 转已收口，见下方 F1-E-4）**· **✅ 第五批 SHEET_CONFIG 已收口（2026-08-19 #3 执行第 68 轮 `d990eca`/`057e493`/`3388901`/`5b92fd3`：icon_config 表 3 行（weapons 40/items 54/skills 5，path/frame_count/frame_size 与 const 零漂移）+ data_schema 注册 + files dict 第 7 键 + get_icon_config 懒加载 + IconAtlas._resolve_icon_config 消费改读（static 类 Engine 访问新范式，SHEET_CONFIG const 兜底 = 两图标探针直读零改动）+ 探针 §10 icon 段 15 断言 **349/349** + day26 锚点 1677 + T-020 转已收口，见下方 F1-E-5）**· **✅ 第六批 初始武器 + SKILL_ICON_MAP 已收口（2026-08-19 #3 执行 `38f7c2e`/`80fc19d`/`3b3aff8`/`20b03f9`/`c2b5a0e`：T-004 starting_gun 抽表（8/0.4/180/knockback0/max1 ranged，两键不进表）+ 消费端改读（补设 360/1.5 + remove_meta 保 day13 硬门槛 + shop.gd 排除初始枪防商店池 +1）+ T-022 skill_icon_map 独立表 5 行 + get_skill_icon_index + hud 两消费点改走（const 兜底），探针 316/316 + day7/day8 豁免占位 + 回归 59/64 挂 D-26，见下方 F1-E-6）**· **✅ 第七批 炮台默认值已收口（2026-08-19 #3 执行 `8f6ecff`/`c0606e1`/`c8ad1b7`/`afce477`/`60a4e96`：T-024 turret_config 表（1 行 5.0/0.5/220.0 + data_schema 注册 + files dict 第 6 键）+ get_turret_config 懒加载 + turret 消费改读（TURRET_DEFAULTS const 收敛 :13-15 + _resolve_turret_defaults + setup 兜底，day13 炮台段 6b 零改动）+ 探针 §9 turret 段 18 断言 **334/334** + day26 锚点 1662 + T-024 转已收口，见下方 F1-E-7）**· **✅ 阶段 F 真全闭 7/7（2026-08-19 #3 执行第 68 轮：批五补落地，7/7 批全收口，回归 59/64 挂 D-26 用户会话在途零新增 + day31_presentation 349/349 + day26 锚点 1677 + BASELINE CLEAN，方案师第 37 轮「6/7 不实」更正解除）**（用户 2026-08-18 23:1x 拍板解除「#3 勿自行开工」历史约定，拆解+方案齐备即 #3 直接执行，每批 Excel→导出→消费→探针→回归，const 兜底）】
...
> 📌 **F1-E-4 第四批 FX 抽表（2026-08-18 第 60 轮拆解 · 承接方 = #3 执行者（用户 2026-08-18 23:1x 拍板解除「#3 勿自行开工」历史约定）· 数据管线铁律：改 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）· ✅ 方案已定（SOLUTION_PLAN.md 第 30 轮，方案师实测复核锚点与拆解逐一一致）**：沿前三批（SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX）同模式——Excel sheet → data_schema 注册 → excel_export 构建 presentation.json → DataLoader 接口（懒加载 + const 兜底）→ 消费点改读 → 探针 → 回归。**实测锚点**：`scripts/effects/vfx_player.gd:17-29` FX_CONFIG 10 键（5 旧 hit/crit/death/levelup/pickup + 5 新 fireball/turret_deploy/blade_burst/meteor/shield，各含 path/frames/size(Vector2i)/fps）、`set_effect` :43-62（FX_CONFIG.has 未知键 push_warning → cfg 取用 → load(path) null 静默跳过 → create_from_sheet 构建）、`spawn` :65-73 静态（load VfxPlayer.tscn + set_effect）；**回归硬门槛 = `tools/day23_vfx_check.gd` §1 配置层断言 FX_CONFIG 10 键（:138-143）+ set_effect 未知键/命中白盒（:164-171）**；excel_export.py:399-434 presentation 构建段（audio_map 先例 :424-434 + enemy_sprites size 组装 :408-410）+ data_schema.py:241-245 注册范式（dict 形 id 主键）+ day31_presentation_check 探针（273/273，§1-§5 已含 enemy_sprites/behavior_map/audio_map 段）。**关键决策：FX_CONFIG const 保留为兜底**（F 系列缺省兜底约定）→ day23_vfx_check §1 零改动硬门槛 + 抽表零数值变化（仅帧配置数据化；size 为 Vector2i 无法直存 JSON → Excel 拆 size_w/size_h 两列，导出组装 {"x","y"}，DataLoader 消费组装 Vector2i，仿 get_enemy_sprite_config :610-612 先例）。**执行序 F1-E-4-1 → 4-2 → 4-3 → 4-4 → EXIT，每任务一收口 commit 带 F1-E-4 编号**。⚠️ **方案师第 31 轮（08-18 22:4x）观察：批四仍未开工（git 实测 HEAD=`144b2bb`，最近 12 提交 = RELIC 拆解/F-45×3/F-46/各岗 docs，无 fx_config sheet / get_fx_config / vfx_player 改读）→ 挂账记录（跨 2 轮）；用户 08-18 23:1x 已拍板解除「#3 勿自行开工」约定 → 承接方 = #3 执行者，下轮即可按拆解开工**。
- [x] **F1-E-4-1【W2】Excel 抽表（数据侧）**【✅ 收口 2026-08-18 总指挥第 6 轮（用户 23:1x 拍板「#3 直接执行」决策落地前已动工完成，提交为检查点，后续 4-2~EXIT 交接 #3 执行者）：GameData.xlsx 新增 fx_config sheet（10 行 × id/frames/fps/path/size_w/size_h 双行表头）+ data_schema 注册 fx_config + excel_export presentation 段追加 fx_map 构建（size_w/size_h → {"x","y"} 组装）+ 导出 fx_config 10 键零漂移、**其余 14 JSON 零 diff**】：docs/GameData.xlsx 新增 `fx_config` sheet（**10 行 × id/frames/fps/path/size_w/size_h 6 列**：id = hit/crit/death/levelup/pickup/fireball/turret_deploy/blade_burst/meteor/shield；path = res://assets/sprites/effects/fx_*.png **与 FX_CONFIG 现值逐一一致**；frames/fps 与 const 一致；size_w/size_h = size.x/size.y 拆列）；data_schema.py 注册 `fx_config`（file: presentation.json / root: "fx_config" / kind: "dict" / key: id / json_cols: []，仿 audio_config :241-245）；excel_export.py presentation 构建段（:399-434）追加 fx_map 解析（id 主键 → {path, frames, fps, size: {"x": int(size_w), "y": int(size_h)}}，仿 audio_map 段 :424-434 + enemy_sprites size 组装 :408-410）；导出 → presentation.json +fx_config 10 项，**其余 13 JSON 零 diff 断言**（前三批先例）。**测试点**：JSON 校验通过；fx_config 10 键齐；path/frames/fps/size 与 vfx_player const 现值一致（零漂移）
- [x] **F1-E-4-2【W1】DataLoader 接口**【✅ 收口 2026-08-19 #3 执行（commit `f67630b`）：get_fx_config(fx_name) 懒加载 + 空表重试标记 + 命中组装 {path, frames, fps, size: Vector2i}（size JSON → Vector2i 仿 get_enemy_sprite_config 先例）/ 未命中/损坏 → 空字典；探针白盒实证 hit 键齐 + size==Vector2i(32,32) + meteor 128×128 + 未知名空字典】：新增 `get_fx_config(fx_name: String) -> Dictionary`（懒加载 presentation.json fx_config 缓存 + 空表标记，仿 get_audio_config :641-646 / get_enemy_sprite_config :602-617 范式）；命中 → 组装返回 {path, frames, fps, size: Vector2i}（size JSON → Vector2i，仿 :610-612）；未命中/损坏 → 空字典（消费端 const 兜底，零崩）。**测试点**：白盒读 get_fx_config("hit") → 键齐全 + size == Vector2i(32,32)；改 Excel frames → 导出 → 返回值变化（端到端双跑，F1-散 §1 先例）
- [x] **F1-E-4-3【W1】vfx_player 消费改读**【✅ 收口 2026-08-19 #3 执行（commit `f67630b`）：新增 _resolve_fx_config（DataLoader.get_fx_config 命中优先/未命中·空表回退 FX_CONFIG const，仿 audio_manager._resolve_audio_path 范式）；set_effect 改走；FX_CONFIG const 保留兜底 → **day23_vfx_check 18/18 零改动硬门槛通过**；未知键 push_warning 行为不变（§6 探针实证 current_fx 不写）】：`set_effect`（:43-48）改走 `get_fx_config`（命中 fx_config[fx_name] 优先 → cfg 取用；未命中/空表 → FX_CONFIG const 兜底）；**FX_CONFIG const 保留为兜底**（→ day23_vfx_check §1 :138-143 零改动硬门槛）；未知键 push_warning（:44-46）行为不变；`load(path)` null 静默跳过（:49-51）行为不变；`spawn` 静态（:65-73）经 set_effect 自动继承。**⚠️ 零行为变化**（仅帧配置来源数据化）。**测试点**：白盒 set_effect("hit") 走 fx_config 路径（current_fx 观测）；fx_config 清空 → 回退 const 仍可播（load 不崩）；未知键仍 push_warning + current_fx 为空
- [x] **F1-E-4-4【W1】探针扩展**【✅ 收口 2026-08-19 #3 执行（commit `f67630b`）：day31_presentation +§6 fx 段 13 断言（10 键齐/键集一致/逐键 path·frames·fps·size 零漂移/get_fx_config 消费 6 项/白盒改值 E2E 双跑/空表兜底 set_effect 仍播/未知键 current_fx 不写）→ **286/286 PASS**】：day31_presentation_check +§6 fx 段（**≥13 断言**：fx_config 10 键齐 / path/frames/fps/size 与 FX_CONFIG const 现值逐一一致（抽表零漂移）/ 改 Excel 一例 frames → 导出 → get_fx_config 变化（端到端双跑）/ 空表兜底 const 仍可播（白盒 set_effect）/ 未知键 push_warning 保留）。**回归硬门槛**：day23_vfx_check 零改动（FX_CONFIG const 保留）+ 62 件套 1534 断言 + baseline CLEAN
- [x] **F1-E-4-EXIT【W5】收口**【✅ 收口 2026-08-19 #3 执行：回归 **63/63（1578 断言）** 全绿（runner +day31_portal_check 24 + presentation expect 273→286）+ day31_presentation **286/286** + baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 4/7 批 + T-019 转已收口】：回归 62 件套（1534 断言）+ day31_presentation ≥286（273+13）+ baseline **BASELINE CLEAN** + F1-E 行 4/7 批标记 + TECH_DEBT_ISSUES T-019（FX_CONFIG 抽表）转已收口

> 📌 **F1-E-5 第五批 SHEET_CONFIG 图标配置抽表（2026-08-19 第 63 轮拆解 · 承接方 = #3 执行者（用户 2026-08-18 23:1x 拍板解除「#3 勿自行开工」历史约定）· 数据管线铁律：改 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）· ✅ 方案已定（SOLUTION_PLAN.md 第 33 轮，方案师实测复核锚点与拆解逐一一致）· ✅ 方案师第 37 轮锚点复核更新（2026-08-19 12:4x，批六/批七收口后 5 处漂移修正，执行者以 SOLUTION_PLAN.md 第 37 轮锚点为准）：① excel_export.py files dict 现于 **:543 · 现 6 根键**（enemy_sprites/behavior_map/audio_map/fx_config/skill_icon_map/turret_config，**无 icon_config = 批五未落地第 4 次实证**）→ **icon_config = 追加第 7 键（第 33 轮方案「第 6 键」作废）** ② data_schema.py 现三注册（fx_config :261-262 / skill_icon_map :270-271 / turret_config :281-282）→ **icon_config 注册追加在 turret_config 后（现约 :285+）** ③ data_loader.gd 现 get_skill_icon_index :691 + get_turret_config :709 → **get_icon_config 仿写位置 = get_turret_config 后（约 :716+）**，缓存字段 _icon_map 追加 :41+ ④ **段号修正：原拆解定 icon 段 = §7，但 §7/§8 已被批六占用、§9 被批七占用 → icon 段 = §10 追加文件尾（§9 :429 后）**；day31_presentation_check 当前 **334/334** → +§10 icon 段 ≥13 断言 → **≥347/347**，runner expect 同步 334→347 ⑤ **回归硬门槛口径更新 = 64 件套 1662 锚点**（批七收口 day26 1644→1662；当前 59/64 挂 D-26）；零漂移保持 3 项 = icon_atlas SHEET_CONFIG :8-24 原样 + 两图标探针直读 const + get_frame_count 三探针行为保持）· ✅ 方案师第 38 轮收口确认（2026-08-19 18:4x：`d990eca`~`d03750e` 5 commits 全 [x]，第 37 轮 5 处锚点更新全部兑现 → **批五收口 = 阶段 F 实测真全闭 7/7 🎉，上轮「6/7」更正挂账解除**）**：沿前四批（SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX/FX）同模式——Excel sheet → data_schema 注册 → excel_export 构建 presentation.json → DataLoader 接口（懒加载 + const 兜底）→ 消费点改读 → 探针 → 回归。**实测锚点**：`scripts/utils/icon_atlas.gd:8-24` SHEET_CONFIG 3 键（weapons 40 帧 / items 54 帧 / skills 5 帧，各含 path/frame_count/frame_size(Vector2i)）、`get_icon` :37-49（SHEET_CONFIG.has 未知 sheet push_warning → config 取用 → load(path) null 返回 → index 越界 push_warning）、`get_frame_count` :71-74（SHEET_CONFIG.has → frame_count / 未知 0）、`clear_cache` :77-79；**回归硬门槛 = 两个图标探针直接读 SHEET_CONFIG const**：`tools/day31_items_atlas_check.gd:27` `IconAtlas.SHEET_CONFIG["items"]`（path + 54 帧遍历 IconAtlas.get_icon）+ `tools/day31_skill_icon_check.gd:28` `IconAtlas.SHEET_CONFIG["skills"]`（path + :37-38 `cfg["frame_count"]==5` + `get_frame_count("skills")==5` 断言）+ 三探针动态读 `get_frame_count("items")`（day11_12 :482 / day20 :369 / day24_f13 :338，行为保持零改动）；`day7_weapon_data_check.gd:245` 仅注释提及；excel_export.py:435-449 fx_config 构建段 + data_schema.py:255-258 注册范式（dict 形 id 主键）+ day31_presentation_check 探针（286/286，§1-§6 已含 enemy_sprites/behavior_map/audio_map/fx_config 段）。**关键决策：SHEET_CONFIG const 保留为兜底**（F 系列缺省兜底约定）→ day31_items_atlas_check/day31_skill_icon_check 直接读 const 零改动硬门槛 + get_frame_count 行为保持；抽表零数值变化（仅帧配置数据化）。**⚠️ 技术要点（static 类访问 Autoload）**：IconAtlas 为 `class_name extends RefCounted` 全静态工具类——静态方法内经 `Engine.get_main_loop().root.get_node_or_null("DataLoader")` 获取 Autoload（不可达 → 回退 const 兜底零崩，--script 探针环境天然兼容：直接读 const 的探针不受影响）；参照 vfx_player（实例节点直接 DataLoader 引用）不同，**本批消费端解析须内聚在 icon_atlas.gd 内部私有静态函数 `_resolve_icon_config`**，外部调用方（hud/shop/level_up_panel 等经 IconAtlas.get_icon）零改动。**执行序 F1-E-5-1 → 5-2 → 5-3 → 5-4 → EXIT，每任务一收口 commit 带 F1-E-5 编号**。回归硬门槛 = 63 件套 1578 断言。
- [x] **F1-E-5-1【W2】Excel 抽表（数据侧）**【✅ 收口 2026-08-19 #3 执行（commit `d990eca`）：GameData.xlsx +icon_config sheet（3 行 × id/path/frame_count/frame_size_w/frame_size_h 双行表头：weapons/items/skills；path 与 const 现值逐一一致；frame_count 40/54/5；frame_size 32×32 三行）+ data_schema 注册（dict 形 id 主键，追加 turret_config 后）+ excel_export presentation 段追加解析（frame_size 拆列组装 {"x","y"}）+ files dict **第 7 键**（现 6 根键 + icon_config，第 37 轮锚点修正）→ 导出 icon_config 3 项、**其余 15 JSON 零 diff** + --check-only EXIT=0】：docs/GameData.xlsx 新增 `icon_config` sheet（**3 行 × id/path/frame_count/frame_size_w/frame_size_h 双行表头**：id = weapons/items/skills（与 SHEET_CONFIG 键一致）；path = res://assets/sprites/ui/weapons.png / res://assets/sprites/ui/items.png / res://assets/sprites/skills/skills.png **与 const 现值逐一一致**；frame_count = 40/54/5 与 const 一致；frame_size_w/h = 32/32 三行（frame_size 全 32×32，拆列仿 fx_config size_w/size_h 先例，导出组装 {"x","y"}））；data_schema.py 注册 `icon_config`（file: presentation.json / root: "icon_config" / kind: "dict" / key: id / json_cols: []，仿 fx_config :255-258）；excel_export.py presentation 构建段（:435-449）追加 icon_map 解析（id 主键 → {path, frame_count, frame_size: {"x": int(frame_size_w), "y": int(frame_size_h)}}，仿 fx_config 段 :440-447）；导出 → presentation.json +icon_config 3 项，**其余 16 JSON 零 diff 断言**（前四批先例）。**测试点**：JSON 校验通过；icon_config 3 键齐；path/frame_count/frame_size 与 IconAtlas const 现值一致（零漂移）
- [x] **F1-E-5-2【W1】DataLoader 接口**【✅ 收口 2026-08-19 #3 执行（commit `057e493`）：+_icon_map 缓存字段（is_empty 重试标记 F3 §4 零新增 bool）+ get_icon_config(sheet_name) 懒加载命中 → duplicate + frame_size JSON → Vector2i 组装 / 未命中损坏 → 空字典】：新增 `get_icon_config(sheet_name: String) -> Dictionary`（懒加载 presentation.json icon_config 缓存 + `_icon_map` 空表标记，仿 get_fx_config :661-683 范式）；命中 → 组装返回 {path, frame_count, frame_size: Vector2i}（frame_size JSON → Vector2i，仿 get_fx_config :674-679）；未命中/损坏 → 空字典（消费端 const 兜底，零崩）。**测试点**：白盒读 get_icon_config("items") → 键齐全 + frame_size == Vector2i(32,32)；改 Excel frame_count → 导出 → 返回值变化（端到端双跑，F1-散 §1 先例）
- [x] **F1-E-5-3【W1】IconAtlas 消费改读**【✅ 收口 2026-08-19 #3 执行（commit `3388901`）：+静态私有 _resolve_icon_config（Engine.get_main_loop().root.get_node_or_null("DataLoader") 命中优先 / 未命中空表无 DataLoader 回退 const SHEET_CONFIG）+ get_icon 与 get_frame_count 改走 + SHEET_CONFIG const 保留兜底 = day31_items_atlas/skill_icon 直读 const 零改动硬门槛，未知 sheet push_warning 行为不变】：新增静态私有 `_resolve_icon_config(sheet_name: String) -> Dictionary`（`Engine.get_main_loop().root.get_node_or_null("DataLoader")` → 非空则 `get_icon_config(sheet_name)` 命中优先返回；未命中/空表/无 DataLoader → `SHEET_CONFIG.get(sheet_name, {})` const 兜底）；`get_icon`（:41 `var config: Dictionary = SHEET_CONFIG[sheet_name]` → `_resolve_icon_config`）与 `get_frame_count`（:72-74）改走；**SHEET_CONFIG const 保留为兜底**（→ day31_items_atlas_check :27 直接读 const + day31_skill_icon_check :28/:37-38 零改动硬门槛 + day11_12/day20/day24_f13 get_frame_count 动态读行为保持）；未知 sheet_name push_warning（:37-39）行为不变（_resolve_icon_config 空字典 → 前置 SHEET_CONFIG.has 检查保留或兜底空 cfg 分支按原样 warn+return）；`load(path)` null 返回（:43-44）与 index 越界 push_warning（:47-49）行为不变。**⚠️ 零行为变化**（仅帧配置来源数据化）。**测试点**：白盒 get_icon("items", 0) 走 icon_config 路径（返回值非空）；icon_config 清空 → 回退 const 仍可 get_icon（load 不崩）；未知 sheet 仍 push_warning；get_frame_count 未知 sheet 仍 0
- [x] **F1-E-5-4【W1】探针扩展**【✅ 收口 2026-08-19 #3 执行（commit `5b92fd3`）：day31_presentation +§10 icon_config 段 15 断言（334→**349/349**）+ runner expect 334→349 + day26 锚点 1662→**1677** + 两图标探针零改动复跑全绿】：day31_presentation_check +§7 icon 段（**≥13 断言**：icon_config 3 键齐 / 键集合与 SHEET_CONFIG 一致（零多余零缺失）/ 逐键 path·frame_count·frame_size 与 const 现值逐一一致（抽表零漂移）/ 改 Excel 一例 frame_count → 导出 → get_icon_config 变化（端到端双跑）/ 空表兜底 const 仍可 get_icon（白盒）/ 未知 sheet_name push_warning 保留 / get_frame_count 行为一致）。**回归硬门槛**：day31_items_atlas_check（直接读 SHEET_CONFIG["items"]）+ day31_skill_icon_check（直接读 SHEET_CONFIG["skills"] + get_frame_count 断言）零改动 + 63 件套 1578 断言 + baseline CLEAN
- [x] **F1-E-5-EXIT【W5】收口**【✅ 收口 2026-08-19 #3 执行第 68 轮：回归 64 件套（59/64 挂 D-26 用户会话在途零新增）+ day31_presentation **349/349** + baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 7/7 批（阶段 F 真全闭）+ T-020 转已收口】：回归 **63 件套（1578 断言）** + day31_presentation ≥299（286+13）+ baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 5/7 批标记 + TECH_DEBT_ISSUES T-020（SHEET_CONFIG 抽表）转已收口

> 📌 **F1-E-6 第六批 初始武器 + 技能图标映射抽表（2026-08-19 第 64 轮拆解 · 承接方 = #3 执行者（D-015 交接）· 数据管线铁律：改 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）· ✅ 方案已定（SOLUTION_PLAN.md 第 34 轮，方案师实测复核锚点 8 项逐一一致 + 2 处微差修正（_equip_default_weapon :59-71 实 9 字段 / excel_export files dict 现 :512 4 根键）+ **1 处新阻塞点裁决：build_weapon_from_data :162 不消费 projectile_speed/lifetime → 两键不进表 + 消费端补设 360/1.5 对齐现值**）**：两子项合并批 = **T-004 初始枪内联配置抽表** + **T-022 SKILL_ICON_MAP 抽表**（第 63 轮「下轮起手」约定批五收口后拆，本轮提前拆解备好——#3 可于 LD-B 或批五之后连续推进，避免再等拆解窗口）。**实测锚点**：① `scripts/weapons/weapon_controller.gd:59-76` `_equip_default_weapon()` 内联构造 7 键（weapon_name="初始枪"/weapon_type="ranged"/base_damage=8.0/fire_rate=2.5/projectile_speed=360.0/attack_range=180.0/lifetime=1.5/pierce=0/knockback=0.0），调用点 :40 `_ready`；:185 注释「按 id 装备数据驱动武器，覆盖 _ready() 装上的占位初始枪」= 数据驱动装配语义已有（build_weapon_from_data，D10-T2 记录）② `scripts/ui/hud.gd:336-341` `SKILL_ICON_MAP` const（**5 键**：se_skill_fireball=0/se_skill_deploy_turret=1/se_skill_blade_burst=2/se_skill_holy_shield=3/se_skill_sword_arc=4）+ 消费点 :354 `_apply_skill_icon`（槽 0 skill_data.id）/ :403 `_apply_skill_slot_icon`（槽 1/2 掉落技能 entry.id，未映射 → 灰显现状）+ 未知 id push_warning（:356-357）③ **回归硬门槛 = `tools/day31_skill_icon_check.gd` §5 :57-81 直接读 `hud_script.SKILL_ICON_MAP` const**（覆盖 data 全部 se_skill_* id + 帧索引有效 + se_skill_sword_arc==4 断言，**零改动**）+ `tools/day13_build_check.gd:617-625`「无 source_id 占位武器跳过 inventory」断言（**初始枪不污染 HUD 硬门槛**）④ data_schema.py :286-291 characters 注册（skill 为 dict → 点号宽列自动）——**否决「characters skill.icon_index 点号列」备选**：SKILL_ICON_MAP 含 `blade_burst=2` 历史键（4 角色技能实测 = fireball/deploy_turret/sword_arc/holy_shield，blade_burst 无主——D3 时代莱恩技能已改 sword_arc，skills.png 第 2 帧仍存在）+ 掉落技能槽（skill_relics relic_dash/relic_blink 等 per_character 变体）亦需映射 → **独立 `skill_icon_map` 表与 const 结构 1:1**（抽表零漂移 + 兜底零改动 + 未来掉落技能加行即可）。**关键决策**：① `starting_gun` 条目不设 source_id（保持「无 source_id 占位武器跳过 inventory」语义，day13 :617-625 硬门槛）② **SKILL_ICON_MAP const 保留为兜底**（day31_skill_icon_check §5 直接读 const 零改动硬门槛；抽表零数值变化，仅映射来源数据化）③ T-022 只抽现 const 5 键，**掉落技能图标 = 技能遗物扩展域**（skills.png 仅 5 帧，新图标需新帧 = 美术占位口径超批内成本，登记 TECH_DEBT 或留 PS-EXIT 扩展，不并入本批——槽 1/2 灰显现状行为不变）④ starting_gun 数值零变化（8.0/2.5/360/180/1.5/0/0 逐键断言零漂移）。**执行序 F1-E-6-1 → 6-2 → 6-3 → 6-4 → 6-5 → EXIT，每任务一收口 commit 带 F1-E-6 编号**。回归硬门槛 = 64 件套 1602 锚点（当前 59/64 5 FAIL 挂 D-26 用户会话在途，EXIT 门槛统一挂等复跑恢复 64/64 后全绿）。
- [x] **F1-E-6-1【W2】T-004 初始枪抽表（数据侧）**【✅ 收口 2026-08-19 #3 执行（commit `38f7c2e`）：weapons sheet +starting_gun 行（damage=8/cooldown=0.4/range=180/knockback=0/max_level=1 ranged 归组；price/crit/icon/special 留空 = JSON 键缺失 = build 兜底默认与内联 Weapon.new 一致；projectile_speed/lifetime 不进表；levels 空）→ excel_export 导出 ranged 10 把，其余 16 JSON 零 diff】（id=starting_gun / name=初始枪 / weapon_type=ranged / base_damage=8.0 / fire_rate=2.5 / projectile_speed=360.0 / attack_range=180.0 / lifetime=1.5 / pierce=0 / knockback=0.0 —— **与 `_equip_default_weapon()` 现值逐一一致**；max_level=1 单级，levels 子表（weapons_levels）可不填或填 Lv1 行与基础字段一致；**source_id 留空** = day13 硬门槛）；data_schema 零新注册（weapons 已有 :202-208 注册）；excel_export 导出 → weapons.json +starting_gun 条目，**其余 JSON 零 diff 断言**（前五批先例）。**测试点**：JSON 校验通过；weapons.json starting_gun 7 键与内联现值一致（零漂移）；source_id 缺失
- [x] **F1-E-6-2【W1】T-004 消费端改读**【✅ 收口 2026-08-19 #3 执行（commit `80fc19d`）：_equip_default_weapon 改走 build_weapon_from_data(starting_gun) 非 null 补设 360/1.5 + remove_meta(source_id) 保 day13 :617-625 + null 兜底内联；白盒 17/17（9 键+无 source_id+不写 inventory+兜底）+ E2E 双跑 9→9/8→8 + day13 36/36；**执行登记：starting_gun 入表致商店池 23→24（day13 探针 2 FAIL）→ shop.gd 排除 starting_gun（F31-1「初始武器出商店池」用户拍板同构）**】（方法名先核对：:185 注释「按 id 装备数据驱动武器」= 既有装配路径，D10-T2 透传 evolution_result 记录在位）→ 返回非 null → `equip_weapon(w)`；**null/异常 → 保留现内联构造兜底**（防 Excel 未导出/数据缺失零崩，F 系列缺省兜底约定）；⚠️ 装配后 7 键逐键核对（与内联现值一致，抽表零数值变化）。**测试点**：白盒 _equip_default_weapon → weapon 字段与内联现值逐一相等；Excel 改 base_damage=9 → 导出 → 装配值变化（端到端双跑，F1-散 §1 先例）→ 强制改回 8 重导出；数据缺失 → 内联兜底仍可装备（weapon_name 仍「初始枪」）
- [x] **F1-E-6-3【W2】T-022 skill_icon_map 抽表（数据侧）**【✅ 收口 2026-08-19 #3 执行（commit `3b3aff8`）：skill_icon_map sheet 5 行（id/icon_index 双行表头 与 const 0-4 逐一一致）+ data_schema 注册（dict 形 id 主键 仿 fx_config）+ excel_export presentation 段追加（id→int(icon_index)）+ files dict 第 6 键 → 导出 5 键零漂移，其余 16 JSON 零 diff】（**5 行 × id/icon_index 双行表头**：id = se_skill_fireball/se_skill_deploy_turret/se_skill_blade_burst/se_skill_holy_shield/se_skill_sword_arc，icon_index = 0/1/2/3/4 **与 SKILL_ICON_MAP const 现值逐一一致**）；data_schema.py 注册 `skill_icon_map`（file: presentation.json / root: "skill_icon_map" / kind: "dict" / key: id / json_cols: []，仿 icon_config :255-258 先例）；excel_export.py presentation 构建段（:435-449）追加 skill_icon_map 解析（id 主键 → int(icon_index)，仿 icon_config 段）；导出 → presentation.json +skill_icon_map 5 项，**其余 16 JSON 零 diff 断言**。**测试点**：JSON 校验通过；skill_icon_map 5 键齐；icon_index 与 hud const 现值一致（零漂移）
- [x] **F1-E-6-4【W1】T-022 消费端改读**【✅ 收口 2026-08-19 #3 执行（commit `20b03f9`）：DataLoader +get_skill_icon_index（懒加载 _skill_icon_map + is_empty 重试标记 命中 int/未命中 -1）+ hud _resolve_skill_icon_index（DataLoader 命中优先/回退 const）两消费点改走；SKILL_ICON_MAP const 保留 = day31_skill_icon_check 22/22 零改动；白盒 10/10（接口/路径/兜底/未知 id push_warning/槽灰显）】（懒加载 presentation.json skill_icon_map 缓存 + 空表标记，命中 → int 值 / 未命中·空表 → -1，仿 get_fx_config :661-683 范式）；hud.gd `_apply_skill_icon`（:354）与 `_apply_skill_slot_icon`（:403）的 `SKILL_ICON_MAP.get(skill_id, -1)` 改走 `get_skill_icon_index(skill_id)`（**实现取方案师定案：DataLoader 接口命中优先，未命中 → 回退 `SKILL_ICON_MAP.get(skill_id, -1)` const 兜底**；hud 为实例节点 → `get_node_or_null("/root/DataLoader")` 先例 vfx_player :97，不可达 → 直接 const 查值）；**SKILL_ICON_MAP const 保留为兜底**（day31_skill_icon_check §5 :57-81 直接读 const 零改动硬门槛）；未知 id push_warning 行为不变（:356-357，仅槽 0 路径）。**⚠️ 零行为变化**（仅映射来源数据化）。**测试点**：白盒 _apply_skill_icon → 走 skill_icon_map 路径（帧纹理非空）；表清空 → 回退 const 仍映射；未知 id 仍 push_warning；槽 1/2 掉落技能未映射 → 灰显行为不变
- [x] **F1-E-6-5【W1】探针扩展**【✅ 收口 2026-08-19 #3 执行（commit `c2b5a0e`）：day31_presentation +§7 T-004 段 17 断言 + §8 skill_icon 段 13 断言 → 286→316/316；runner expect 286→316 + day26 锚点 1614→1644；**执行登记：starting_gun 入表致 day7/day8 武器全量断言 FAIL（icon_index 越界/levels 0 条）→ 两探针豁免占位初始枪复跑全绿**；回归 59/64（5 FAIL = D-26 在途零新增）】（**≥8 断言**：skill_icon_map 5 键齐 / 键集合与 SKILL_ICON_MAP 一致（零多余零缺失）/ 逐键 icon_index 与 const 现值一致（抽表零漂移）/ 改 Excel 一例 icon_index → 导出 → get_skill_icon_index 变化（端到端双跑）/ 空表兜底 const 仍映射（白盒）/ 未知 id push_warning 保留 / 槽 1/2 掉落技能灰显不变）。**回归硬门槛**：day31_skill_icon_check 零改动（直接读 const）+ day13_build_check :617-625 零改动（starting_gun 无 source_id 不污染 inventory）+ 64 件套 1602 锚点 + baseline CLEAN
- [x] **F1-E-6-EXIT【W5】收口**【✅ 收口 2026-08-19 #3 执行：回归 **59/64**（5 FAIL = D-26 用户会话在途零新增，与第 65 轮清单一致）+ day31_presentation **316/316**（286+30）+ baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 **6/7 批**标记 + TECH_DEBT_ISSUES T-004/T-022 转已收口；**EXIT 门槛挂 D-26 复跑恢复 64/64 后全绿**】（当前 59/64 5 FAIL 挂 D-26，收口以 D-26 复跑恢复后全绿为准）+ day31_presentation ≥307（299+8）+ baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 6/7 批标记 + TECH_DEBT_ISSUES T-004/T-022 转已收口

> 📌 **F1-E-7 第七批 炮台默认值抽表（2026-08-19 第 65 轮拆解 · 承接方 = #3 执行者（D-015 交接）· 数据管线铁律：改 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）· ✅ 方案已定（SOLUTION_PLAN.md 第 35 轮，方案师实测复核锚点 6 项与拆解逐一一致 + 1 处行号微差修正（data_schema fx_config 注册实位 :261-262）+ 1 处细节补充（turret.gd:3 注释已明示「数值全部来自 DataLoader.get_weapon…禁止硬编码」= T-024 债务本质，turret_config 表 = 默认值唯一事实源））· ✅ 方案师第 36 轮锚点复核更新（2026-08-19 08:4x，批六收口后 4 处漂移修正，执行者以 SOLUTION_PLAN.md 第 36 轮锚点为准）：① weapons.json se_auto_turret 行号 :2668-2673 → **:2680-2690**（starting_gun 行插入 +12，数值 5/0.5/220 零漂移）② data_schema.py fx_config :261-262 保持 + **skill_icon_map 注册 :270-271 已落地（批六）→ turret_config 注册追加在其后（现约 :272+）** ③ excel_export.py files dict **现于 :525 · 现 5 根键**（enemy_sprites/behavior_map/audio_map/fx_config/skill_icon_map，**无 icon_config = 批五 SHEET_CONFIG 未落地实锤**）→ **批七 turret_config = 追加第 6 键（非第 7 键；若先补批五则第 7 键，追加勿覆盖既有键）** ④ day31_presentation_check 当前 **316/316**（批六收口）→ +§9 turret 段 ≥8 断言 → **≥324/324**，runner expect 同步 316→324；**回归硬门槛 = 64 件套 1644 锚点**（批六收口 day26 1614→1644，当前 59/64 挂 D-26 待复跑）**：F1-E 最后一批（7/7）——**T-024「炮台默认值与 se_auto_turret 数据重复」**。沿前六批（SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX/FX/SHEET_CONFIG）同模式——Excel sheet → data_schema 注册 → excel_export 构建 presentation.json → DataLoader 接口（懒加载 + const 兜底）→ 消费点改读 → 探针 → 回归。**实测锚点**：`scripts/weapons/turret.gd:13-15` 字段声明默认值（`damage: float = 5.0` / `fire_interval: float = 0.5` / `attack_range: float = 220.0`）+ `setup()` :31-35 装载兜底（`weapon_data.get("damage", 5.0)` / `get("cooldown", 0.5)` / `get("range", 220.0)`）——**两处散落 = 与 Excel weapons 表 se_auto_turret 行（damage 5 / cooldown 0.5 / range 220）重复**（`data/weapons.json:2668-2673` 实证）；调用链 = `skill_controller.gd:243-280` `_cast_deploy_turret`（summon_id 默认 se_auto_turret → `DataLoader.get_weapon` → count（summon_count 2 + bonus）→ duration（skill 域）→ `world.spawn_turret(TurretScene, weapon_data, duration, player)`）→ `world.gd:104-110` `spawn_turret`（instantiate → `turret.setup(weapon_data, duration, owner_player)` 透传）；**回归硬门槛 = `tools/day13_build_check.gd:629-654` `_part_turret_array` 炮台段**（未装备 se_turret_array → 3 台临时 duration 15 / 装备 → 常驻多台 = turret 行为零变化断言）。**关键决策**：① **turret_config 独立表**（1 行 id=se_auto_turret，dict 形仿 fx_config 先例）——不直接消费 weapons.json（turret 的 weapon_data 本就由 setup 传入，turret_config 管的是「字段声明 + 装载兜底」这份重复默认值的唯一事实源；未来新炮台武器加行即可）② **TURRET_DEFAULTS const 保留为兜底**（收敛 :13-15 字段声明默认值 → const 编译期可用，字段声明 `var damage: float = TURRET_DEFAULTS["damage"]` 保持零改动语义；setup 兜底 `weapon_data.get("damage", TURRET_DEFAULTS["damage"])`）→ **day13_build_check 炮台段 6b 零改动硬门槛** + 抽表零数值变化（仅默认值来源数据化）③ **duration 不拆入本批**（临时炮台存活 15s 为 skill 域：`skill_controller.gd:255` `sd.get("duration", 15.0)`；weapons se_auto_turret `duration=20` 为未消费键 = build_weapon_from_data :162 不消费两键先例，登记观察不拆）④ 消费端 turret 为实例节点 → `get_node_or_null("/root/DataLoader")` 直接引用（vfx_player :97 先例，非 IconAtlas static 类，无需 Engine.get_main_loop）。**执行序 F1-E-7-1 → 7-2 → 7-3 → 7-4 → EXIT，每任务一收口 commit 带 F1-E-7 编号**。回归硬门槛 = 64 件套 1614 锚点（当前 59/64 挂 D-26，EXIT 以 D-26 复跑恢复后全绿为准）。
- [x] **F1-E-7-1【W2】Excel 抽表（数据侧）**【✅ 收口 2026-08-19 #3 执行（commit `8f6ecff`）：GameData.xlsx +turret_config sheet（1 行 × id/damage/fire_interval/attack_range 双行表头：se_auto_turret / 5.0 / 0.5 / 220.0 与 turret.gd:13-15 现值逐一一致）+ data_schema 注册（dict 形 id 主键 追加 skill_icon_map 后）+ excel_export presentation 段追加解析（数值 coerce float）+ **files dict 第 6 键**（现 5 根键 + turret_config，按实位追加勿覆盖）→ 导出 turret_config 1 项、**其余 16 JSON 零 diff** + --check-only EXIT=0】：docs/GameData.xlsx 新增 `turret_config` sheet（**1 行 × id/damage/fire_interval/attack_range 双行表头**：id = se_auto_turret（与 weapons 表炮台武器键一致）；damage = 5.0 / fire_interval = 0.5 / attack_range = 220.0 **与 turret.gd:13-15 现值逐一一致**；⚠️ fire_interval 语义 = cooldown（= 开火间隔），Excel 列名与 const 键一致防消费端映射漂移）；data_schema.py 注册 `turret_config`（file: presentation.json / root: "turret_config" / kind: "dict" / key: id / json_cols: []，仿 fx_config :255-258）；excel_export.py presentation 构建段（:462-512 现 4 根键 + 批五 icon_config + 批六 skill_icon_map = 6 键后追加第 7 键）turret_config 解析（id 主键 → {damage, fire_interval, attack_range} 数值 coerce，仿 fx_config 段 :500-510）；导出 → presentation.json +turret_config 1 项，**其余 16 JSON 零 diff 断言**（前六批先例）。**测试点**：JSON 校验通过；turret_config 1 键齐；三键数值与 turret const 现值一致（零漂移）
- [x] **F1-E-7-2【W1】DataLoader 接口**【✅ 收口 2026-08-19 #3 执行（commit `c0606e1`）：+get_turret_config() 懒加载 presentation.json turret_config 整表返回（空字典 = 未加载 is_empty 即重试 F3 §4 零新增 bool 仿 get_fx_config/get_spawn_points 范式）未命中/损坏 → 空字典；白盒 9/9（命中键齐三值 / 注入改值 E2E / 空缓存重载）】：新增 `get_turret_config() -> Dictionary`（懒加载 presentation.json turret_config 缓存 + `_turret_map` 空表标记，仿 get_fx_config :661-683 范式）；命中 → 整表返回 {id: {damage, fire_interval, attack_range}}；未命中/损坏 → 空字典（消费端 const 兜底，零崩）。**测试点**：白盒读 get_turret_config → se_auto_turret 键齐 + 三值正确；改 Excel fire_interval → 导出 → 返回值变化（端到端双跑，F1-散 §1 先例）
- [x] **F1-E-7-3【W1】turret.gd 消费改读**【✅ 收口 2026-08-19 #3 执行（commit `c8ad1b7`）：+const TURRET_DEFAULTS（收敛 :13-15 字段声明默认值 编译期求值零行为变化）+ _resolve_turret_defaults（DataLoader.get_turret_config 命中 se_auto_turret 优先 / 未命中空表无 DataLoader 回退 const 仿 vfx_player._resolve_fx_config 范式）setup() :32-35 三处装载兜底改走 + maxf(0.01) 钳制保留 + duration 域零改动；白盒 15/15（字段=const 编译期一致 / 空数据=现值 / 注入改值 E2E / 空表兜底不崩 / weapon_data 优先）+ day13 炮台段 6b 零改动 36/36】：新增 `const TURRET_DEFAULTS := {"damage": 5.0, "fire_interval": 0.5, "attack_range": 220.0}`（**收敛 :13-15 字段默认值**——字段声明改 `var damage: float = TURRET_DEFAULTS["damage"]` 等 3 处，const 编译期求值合法零行为变化）+ 新增私有 `_resolve_turret_defaults() -> Dictionary`（`get_node_or_null("/root/DataLoader")` → 非空则 `get_turret_config().get("se_auto_turret", {})` 命中优先返回；未命中/空表/无 DataLoader → `TURRET_DEFAULTS` const 兜底）；`setup()` :32-35 三处装载兜底改走（`weapon_data.get("damage", _resolve_turret_defaults().get("damage", 5.0))` 或方法顶部一次性取 defaults 局部复用）；**TURRET_DEFAULTS const 保留为兜底**（→ day13_build_check 炮台段 6b :629-654 零改动硬门槛）；duration_left/duration_max/permanent 逻辑零改动。**⚠️ 零行为变化**（仅默认值来源数据化：Excel turret_config 命中 → 用表值；缺表/未导出 → const 现值）。**测试点**：白盒 turret.setup 空 weapon_data → 字段 = 现值（5.0/0.5/220.0）；turret_config 清空 → 回退 const 仍可 setup 不崩；改 Excel 值 → 导出 → setup 空 weapon_data 字段变化（端到端双跑）→ 强制改回重导出；三字段声明值 = const（编译期一致）
- [x] **F1-E-7-4【W1】探针扩展**【✅ 收口 2026-08-19 #3 执行（commit `afce477`）：day31_presentation +§9 turret 段 **18 断言**（外层 1 键齐 se_auto_turret / 内层键集与 TURRET_DEFAULTS 一致零多余零缺失 / 三键数值逐一零漂移 / get_turret_config 白盒 / 注入改值 E2E 双跑 / 空表兜底 setup 不崩 / 未知武器 id 回退 const / 字段声明=const 编译期一致 / weapon_data 优先）→ **316→334/334** + runner expect 316→334 + day26 锚点 1644→1662】：day31_presentation_check +§9 turret 段（**≥8 断言**：turret_config 1 键齐 / 键集合与 TURRET_DEFAULTS 一致 / 逐键数值与 const 现值逐一一致（抽表零漂移）/ 改 Excel 一例 fire_interval → 导出 → get_turret_config 变化（端到端双跑）/ 空表兜底 const 仍可 setup（白盒）/ 未知武器 id 回退 const / 字段声明值 = const 编译期一致）。**回归硬门槛**：day13_build_check 零改动（炮台段 6b）+ 64 件套 1614 锚点 + baseline CLEAN
- [x] **F1-E-7-EXIT【W5】收口**【✅ 收口 2026-08-19 #3 执行：回归 **59/64**（5 FAIL = D-26 用户会话在途 `set_frame_offset` 4.4 API 误用，与第 65/66 轮 FAIL 清单完全一致零新增，D-020 不代修待收口；EXIT 门槛以 D-26 复跑恢复 64/64 后全绿为准）+ day31_presentation **334/334**（316+18）+ baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 **7/7 批标记（阶段 F 全闭 🎉）** + TECH_DEBT_ISSUES T-024 转已收口；**执行登记**：weapons se_auto_turret duration=20 为未消费键（build_weapon_from_data 不消费先例，第 35 轮方案已登记观察，不拆入本批）】：回归 **64 件套（1614 锚点）**（当前 59/64 5 FAIL 挂 D-26，收口以 D-26 复跑恢复后全绿为准）+ day31_presentation ≥315（307+8）+ baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 + F1-E 行 **7/7 批标记（阶段 F 全闭）** + TECH_DEBT_ISSUES T-024 转已收口
...
## RELIC 遗物扩展（2026-08-18 用户拍板 · 规格 docs/RELIC_EXPANSION_SPEC.md · O-1~O-4 全拍板（§10）· ✅ 已拆解 2026-08-18 第 61 轮 · ✅ 方案已定 2026-08-18 第 31 轮（SOLUTION_PLAN.md，方案师实测复核锚点：stats.json .stats.offensive[2]/.stats.economy[3] + desc_builder.gd:32-33 硬编码映射存在（「工程」≠「工程学」需统一）+ items relic 2 件 + data_loader:437 范式 + save_system 缺省容错范式）· 未拆解前禁动工 · 拆解完成即解锁 · 方案唯一事实源 RELIC_EXPANSION_SPEC.md · 独立目标日 Day 31+ 不插入 D30 收尾窗口 · **✅ A 批已收口 2026-08-19 第 69 轮（`765e3bc`+`74ad1fd`：day31_relic_name 15/15 + 回归 65 件套 60/65 挂 D-26 零新增）** · **✅ RELIC-0 数据地基已收口 2026-08-19 第 70 轮（`01a27c7`+0-2/0-3 收口：items relic +6 新列 + 套装 2 套 4 件 + 流派 6 件占位 + get_relic_defs/get_relic_set_ids + meta 两键 + day31_relic_data 55/55 + 回归 66 件套 61/66 挂 D-26 零新增）** · **✅ RELIC-F Boss 行为节奏已收口 2026-08-19 第 71 轮（`fb9442b`+`25f4e2f`+`4f35211`：boss_pattern +3 节奏列 + 三段态状态机 + day31_relic_boss_rhythm 26/26 + 回归 67 件套 59/67 挂 D-26×5+美术在途×3 零引入）** · **剩余 RELIC-E/B/C/D/EXIT 已拆已定案待 #3 执行**（执行序 E（P0 独立）→ B/C/D（依赖 0 已就绪）→ EXIT））

> 📌 **第 61 轮（2026-08-18 22:0x · #2）函数级拆解**（实测锚点 HEAD=`980aa7d`；规格书入库 = 执行者第 61 轮 `d82a4c2` 随 RELIC_EXPANSION_SPEC.md 落盘；方案师第 30 轮（20:4x）预判「量大横切多域 + 与发布收尾零数据改动口径冲突 → 建议独立目标日（Day 31+）而非插入收尾窗口，请 #2 下轮（22:05）优先拆解评估排期」→ **本轮拆解评估结论 = 采纳独立目标日，批次拆解如下，承接方 = #3 执行者（用户 2026-08-18 23:1x 拍板：解除「#3 勿自行开工」历史约定，拆解+方案齐备即 #3 直接执行——原「总指挥/主窗口承接」标注废止），勿插入 D30 收尾窗口**）。**规格 7 大块**：A 属性命名去土豆兄弟化 ⭐P1（**O-1 已拍板：元素伤害→魔法伤害 / 工程学→机械学**，id 零改动）/ B 构筑转换型套装遗物 ⭐P1（O-2 拍板：不做控血，改玻璃大炮：血上限 -90% + 高减伤 + 高输出）/ C 遗物图鉴+条件解锁 ⭐P1（池子 ≥60 件）/ D 流派遗物树+动态权重 ⭐P0（O-3 拍板 affinity 默认不可改、开放事件洗点；O-4 拍板流派不限于移速）/ E Boss 宝箱收获+通关成就感 ⭐P0 / F Boss 行为节奏（走走停停+大范围主导）⭐P0 / G 反馈质感 ⭐P2（并入 E）。**关键拆解决策（本轮实测定锚）**：① **遗物池概念 = items.json `slot="relic"` 属性遗物**（D20 已实装 2 槽 MAX_RELICS=2 直装范式 + broken_crown/mech_engine 先例）——B/C/D 项的「遗物」全部落在该池扩展；**skill_relics.json 技能遗物（relic_dash/blink 等装配技能槽）不在本次字段扩展范围**（规格 §1.3 未纳入，仅 C 项图鉴可顺带纳入「遗物」分类展示，标注可选）② **数据层落地 = Excel items sheet**（data_schema.py :364-366 skill_relics 注册先例；items.json 由 Excel items sheet 导出，字段扩展走 Excel → excel_export.py → 探针，**data/*.json 禁手改**）③ **meta_progress 扩展**（relic_affinity / relic_codex，缺省字段零值容错 = day30_save_compat_check 14/14 范式，旧档不崩）④ **美术硬性约束**：宝箱/套装切换视觉 = 占位纯色/文字/复用现有 VfxPlayer 特效，零美术生成。**批次依赖（规格 §9）**：数据层先行（RELIC-0：字段扩展+接口+存档）→ 之后 B/C/D 并行；F 独立可先行；E 独立可先行；A 独立低成本先行。**执行序建议：RELIC-A（低成本独立）→ RELIC-0（数据地基）→ RELIC-F / RELIC-E（P0 独立）→ RELIC-B / RELIC-C / RELIC-D（依赖 0）→ RELIC-EXIT**。每任务一收口 commit 带 RELIC 编号；**回归硬门槛 = 62 件套 1534 断言 + baseline BASELINE CLEAN**。

### RELIC-A 属性命名去土豆兄弟化（SPEC §2 · ⭐P1 · 用户点名成本低 · O-1 已拍板 · 独立可先行）✅ A 批已收口（2026-08-19 第 69 轮 #3：`765e3bc` A1+A2 + 探针 15/15 + 回归 65 件套 60/65 挂 D-26 零新增 + day26 锚点 1677→1692）
- [x] **RELIC-A1【W2】Excel stats sheet 改名（数据侧）**：docs/GameData.xlsx stats sheet 两行 name 改（**id 零改动**）——`elemental_damage` name「元素伤害」→「**魔法伤害**」（与剑士物理伤害直观区分；「元素反应」内部机制名不动）/ `engineering` name「工程学」→「**机械学**」（诺亚召唤成长用，贴星骸机械设定）；tools/excel_export.py 导出 → stats.json 仅两 name 变（**其余字段/JSON 零 diff 断言**，F1-散 §1 先例）。**测试点**：JSON 校验通过；stats.json 两 name 新值 + id 不变；其余 JSON 零 diff
- [x] **RELIC-A2【W1】展示消费点核对（零行为改动）**：引用面 = data_loader.gd / attribute_controller.gd / skill_controller.gd / desc_builder.gd（grep `elemental_damage|engineering` 实测 4 脚本）+ data JSON 文案（characters.json 多行 / items.json 多行 / weapons.json:485）——**只改展示名/文案，属性 id 零改动**（防探针/存档/数据层连锁）；HUD 属性面板与商店 tooltip 经 stats.json name 自动取新名（核对 desc_builder STAT_CN 中文映射是否另有硬编码，有则同步）。**测试点**：grep 确认无 `「元素伤害」「工程学」` 硬编码残留（注释白名单除外）；游戏内属性面板/商店 tooltip 显示新名（探针文本锚点）
- [x] **RELIC-A3【W1】探针扩展**：新建 `tools/day31_relic_name_check.gd`（≥8 断言：stats.json 两 id name 新值 + id 零改动 + desc_builder/attribute_controller 文本锚点新名 + 引用面 grep 零残留 + 回归抽样）。**回归硬门槛**：62 件套 1534 断言 + baseline CLEAN
- [x] **RELIC-A-EXIT【W5】收口**：回归 62 件套 + day31_relic_name ≥8 + baseline **BASELINE CLEAN**

### RELIC-0 数据层地基（SPEC §3/§4/§5.3 · 前置批 · B/C/D 全依赖）✅ 已收口（2026-08-19 第 70 轮 #3：`01a27c7` 0-1 + 0-2/0-3/探针收口 commit + 回归 61/66 挂 D-26 零新增 + day26 锚点 1692→1748）
- [x] **RELIC-0-1【W2】items relic 类条目字段扩展（数据侧）**：docs/GameData.xlsx items sheet relic 类条目（slot="relic"）+6 新列——`tag`（speed/offense/defense/summon/economy…，D 项流派打标）/ `tier`（1/2/3，D 项分层开放）/ `set_id`+`set_tier`+`set_effects`（B 项套装：set_effects Excel 分隔串 `档:键=值;|` → 导出 JSON 数组 [{tier,effects}...]，**方案师第 31 轮定案「分隔串+导出解析数组」**）/ `unlock_condition`（first_kill_boss:<chapter_id> / fail_count>=N / codex_count>=N / affinity_tag>=N，字符串表达）；**新增套装 2 套 4 件占位**（「星骸孤注」starbound_gamble 单件=max_hp -90%+减伤 40% / 2 件=伤害+100%+攻速+50%+被摸 2s 减伤再+30%；「死线舞者」deadline_dancer 单件=max_hp -70%+移速+30% / 2 件=移动叠层伤害）+ **移速流派示例 6 件**（T1 common ×3 疾行靴/轻装契约/残影步 + T2 uncommon ×2 动能转化/冲刺余波 + T3 rare ×1 音速分裂，数值按拆解占位）；**⭐ 执行登记：新 10 件 price=0**（resonant_shard 先例天然排除商店池，RELIC-C 解锁后再定价入池，防半成品泄漏）；rarity 列既有（legendary 现 2 件不动）；data_schema 无列白名单自动透传；excel_export items 构建段 +parse_set_effects + KNOWN_EFFECT_KEYS +6 新键；导出 items.json 54→**64 条**（**其余 16 JSON 零 diff 断言**）。**测试点**：JSON 校验通过；relic 条目键齐全；套装 set_effects 数组解析正确；其余 JSON 零 diff ✅
- [x] **RELIC-0-2【W1】DataLoader 接口**：新增 `get_relic_defs() -> Array`（全量属性遗物 = items.json slot="relic" 过滤，懒加载缓存 `_relic_defs_loaded` 一次性构建，缺失 → 空表零崩）+ `get_relic_set_ids() -> Dictionary`（set_id → {count 件数/set_tier 触发档/set_effects 档位效果表}，B 项套装激活用）+ 字段透传（rarity/tag/tier/unlock_condition/set_effects 消费端可读）。**测试点**：白盒读 get_relic_defs → 12 条 + 字段齐全；set_id 分组 starbound_gamble/deadline_dancer 各 2 件 ✅
- [x] **RELIC-0-3【W1】meta_progress 存档扩展（兼容旧档）**：save_system.gd（F4-T4 拆分产物）_default_meta + 两键——`relic_affinity: Dictionary`（D 项：各 tag 计数 {speed: N, ...}）/ `relic_codex: Array`（C 项：已见遗物 id 列表）；**缺省字段零值容错**（旧档 load 缺失 → 空字典/空数组，day30_save_compat_check 14/14 + day27_meta_check 35/35 零改动硬门槛复跑全绿）。**测试点**：day31_relic_data_check §4 源码锚点（不 preload save_system = 引用 Autoload 编译失败 探针三坑①）+ 两硬门槛探针复跑 ✅
- [x] **RELIC-0-EXIT【W5】收口**：新建 `tools/day31_relic_data_check.gd` **55/55**（§1 字段键齐全 12 条 tag/tier 全覆盖 + 套装 4 件 set_* 齐 + unlock 有值≥5 / §2 套装分组两套各 2 件 + set_effects 档位表 1/2 档 + 数值锚点 / §3 池过滤 price=0 不进商店池 = 商店可见遗物仍 2 件 / §4 存档源码锚点 / §5 回归抽样 64 条 + 新 10 id + rarity 合法 + effects 子表解析）+ 回归 **66 件套**（runner +1 + items_atlas 58→59 占位豁免 + day20 54→64 同步）+ baseline **BASELINE CLEAN**；**⭐ 执行登记：day18_feedback2 flaky 根治**（随机池首卡=anvil 服务无可升级武器购买被拒源，探针点击前跳过 anvil 卡，零游戏逻辑改动）✅

### RELIC-F Boss 行为节奏（SPEC §7 · ⭐P0 · 直接改手感 · 独立可先行 · 承接 BS-A~D 已实装底座）✅ F 批已收口（2026-08-19 第 71 轮 #3：`fb9442b` F1 数据侧 + `25f4e2f` F2 消费端 + `4f35211` F3 探针/回归：day31_relic_boss_rhythm 26/26 + 回归 67 件套 59/67 挂 D-26×5+美术在途×3 零引入 + baseline CLEAN）
- [x] **RELIC-F1【W1】施法站定态**：enemy_boss.gd 技能施放期（circle/fan/beam 预警→结算四拍子）Boss 移速归零或大幅下降（enemy_movement.gd 追踪减速/站定，F4-T1 拆分产物；预警期间不追人）；后摇期可被输出——给玩家「躲技能→反打」节奏。**参数数据化**（enemies.json phases 或 boss_pattern 表扩展 `cast_slowdown`/`cast_stationary` 键，禁硬编码，数据管线铁律）；**公平底线公式（BOSS_SKILL_SPEC §2.2 t_w>2r/v+0.4s）零破坏硬门槛** + day30_boss_skill 49/49 + day18_19_boss_check 48/48 零改动 ✅（Excel boss_pattern 表 +cast_slowdown 列，circle 系 3 行 = 0.0 站定；enemy_movement _move_boss_cast 消费，rhythm 缺省 1.0 满速兜底；硬门槛复跑全绿）
- [x] **RELIC-F2【W1】时间分配倒置**：追踪与技能占比 追踪 30-40% / 技能+走位 60-70%——参数表控制（`chase_ratio` / `skill_window` 键）；enemy_boss.gd 行为循环调度权重调整（技能循环已存在，调调度权重与站定逻辑）✅（Excel +chase_ratio=0.2/skill_window=1.0 列；_process_boss_patterns 三段态 = 施放→走位（window 不 pick）→追踪（冷却×ratio）；白盒统计 circle_aoe 追踪占比 35% ∈[30,40]）
- [x] **RELIC-F3【W1】大范围技能权重提升**：boss_pattern 权重表 circle/fan/beam 出现频率 > 贴身追击类（charge/突进）——改权重数据（Excel boss_pattern 表），禁硬编码 ✅（现有 3 行全 circle 系 = 大范围权重 100% > 贴身追击 0%；探针 §4 断言成立，零权重表改动）
- [x] **RELIC-F4【W1】走走停停节奏感**：移动段短而明确（逼近到技能射程即停）+ 技能段完整四拍子 + 后摇结束再决定下一步；enemy_movement.gd 移动段控制 ✅（追踪段 dist ≤ 当前技能 radius×0.9 → 停；施放/走位/追踪三态分派）
- [x] **RELIC-F-EXIT【W5】收口**：新建 `tools/day31_relic_boss_rhythm_check.gd`（≥12 断言：技能施放期间移速下降或站定 / 时间分配比例符合目标（白盒统计）/ 大范围技能出现次数 > 贴身追击 / 公平底线公式零破坏）+ 回归 62 件套 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（Boss 走走停停节奏/躲技能反打手感，交 #5）✅（探针 26/26 六段全绿；runner 66→67 件套 + day26 锚点 1748→1774；回归 59/67 = 8 FAIL 全挂 D-26×5 + 用户会话美术 PNG 透明键在途×3（day8/day10/day11_12 图集断言），RELIC-F 零引入；主观项登记归 #5）

### RELIC-E Boss 宝箱收获 + 通关成就感（SPEC §6 · ⭐P0 · 用户明确要求 · 独立可先行 · G 项并入本批）
- [ ] **RELIC-E1【W1】章 Boss 战后宝箱环节**【🟢 **F-49 地基已落地（2026-08-19 第 66 轮确认 · `4f1e791`）**：传送门延迟结算（wave_manager 敌全灭/Boss 死不再立即 `_end_wave` → `_open_exit_portal` → 玩家 `enter_portal` 手动推进）已含「手动推进/可先捡宝箱再进」语义 + `loot_chest.gd` 金色方块占位（金币 50/经验 30，day31_portal_check 24/24 + 回归 62/62 全绿）已存在——**RELIC-E1 落地时：① 宝箱演出直接复用 loot_chest 既有机制（占位升级）② 「手动进入下一层」= 进传送门语义（已就绪零重做）③ 本章 Boss 宝箱 = loot_chest 奖励替换为遗物三选一（enemy_damage.gd :89-104 三选一先例 + inventory 直装）】**：章 Boss 击杀后不再直接跳下一层 → 收获房间——① 宝箱开启演出（占位：宝箱色块/复用 VfxPlayer 特效 + 开启音效 + 光效，美术占位口径）② 掉落：遗物三选一（**复用精英三选一 UI 逻辑**，enemy_damage.gd :89-104 三选一先例 + skill_controller 装配或 inventory 直装）③ 玩家可停留查看属性变化 → 手动进入下一层/结算；**不打断现有流程（可跳过）**；GameManager._on_node_completed 分支接线（boss 节点 → 宝箱流程 → 手动推进）
...
## LEVEL_DESIGN 关卡设计扩展（2026-08-18 用户拍板 · 规格 docs/LEVEL_DESIGN_SPEC.md · ✅ 已拆解 2026-08-19 第 62 轮 · ✅ 方案已定 2026-08-19 第 32 轮（SOLUTION_PLAN.md，方案师实测复核锚点：enemy_spawner.gd:172/:82/:148 + enemy_damage.gd:36 + enemy_boss.gd:85 + status_component.gd:140/:160 + game_manager.gd:392 + main.gd:240 + data_loader.gd:482/:605/:644 + boss_patterns.json boss_id=invoker/predator + elements 无 type 白名单校验 + excel_export.py:544 --check-only 缺陷复认）· 未拆解前禁动工 · 拆解完成即解锁 · 方案唯一事实源 LEVEL_DESIGN_SPEC.md · 独立目标日 Day 31+ 与 RELIC 同窗口 · 承接方 = #3 执行者（用户 08-18 23:1x 拍板解除「#3 勿自行开工」· D-015 交接））

> 📌 **第 32 轮（2026-08-19 00:4x · 方案师）确认块**：P0 = 增量 #89 无新增机器可验证项（F-01~F-49/AF-P0 全 🟢 待真人回归）→ **🔴P0 无新增**；🟠 LEVEL_DESIGN 规格已由 #2 第 62 轮（`9ebbf5f`）函数级拆解 → **本轮方案已定（SOLUTION_PLAN.md 第 32 轮，锚点实测复核 9 项逐一一致 + 2 处行号微差修正：status_component _revert :157→:160 / get_enemy_sprite_config :600-604→:605）**——承接方 = #3 执行者（D-015），执行序 A→B→C→E→D 可选→EXIT，回归硬门槛 62 件套 1534 断言；F1-E 批四 **4-1 [x] 已收口（`2aeb717`/`716a9d8` fx_config 数据侧）→ 上轮零开工挂账部分解除，剩 4-2~4-4/EXIT 消费端待 #3**；RELIC 全批方案已定（第 31 轮）git 实测仍零开工 → 挂账观察（跨 1 轮）；D30-T3/EXIT 纯 Owner/#4 域；⚠️ build/ 含 F-45~48 + F1-E-4-1 但不含 F-49 → 传送门/宝箱等下次打包。工作区在途 TEST_REPORT（#4）+ art_ai 工具（美术域）不碰；本轮仅 SOLUTION_PLAN/TASKS 两 docs 挂账。

> 📌 **第 62 轮（2026-08-19 00:05 · #2）函数级拆解**（实测锚点 HEAD=`144321a`；规格书入库 = 总指挥第 6 轮 `57ff1d8`；总指挥 D-014 已拍板 §8 五决策点：**D1 默认 8 边缘点 inset=40 ✅ / D2 Boss 登场 anchor(0.5,0.3) 采纳 ✅ / D3 dialogue 台词框本期先用 banner 样式占位 ✅ / D4 特殊波参数化挂 TECH_DEBT（批 D 保留可选，成本 >0.5 天则降级登记）/ D5 attr 白名单以消费点实测登记（crit/攻速缺失则挂 TECH_DEBT）✅**）。**规格三主目标**：① **固定出生点表 spawn_points**（根治 F-48「最后一个敌人死活不出现」随机死角 → 点位固定后可读可控可设计「怪从哪来」演出感，配合 Aggro Leash 320 双保险）② **boss_phase_events 独立演出表**（Boss 血线横幅/特效/音效/台词/震屏/buff 一表打通，与 boss_pattern「换技能」/ phases「技能池+速度」三者阈值各自独立可配，互不阻塞）③ **正向属性状态 attr**（通用 buff 一次到位：attack%/speed%/armor/crit 等，与负面 slow 同构，金手指 = +900% 整局 attr 概念成立但本期不做记 TECH_DEBT）。**关键拆解决策（本轮实测定锚）**：① **已数据化确认**（waves/enemies/boss_skill/boss_pattern/enemy_sprites/enemy_behavior 全在 Excel 唯一事实源）→ 本次收口仅三块硬编码：`enemy_spawner.gd:172 _get_random_spawn_position`（±200×±120 随机 + min_dist 110 + 40 次尝试 + _clamp_to_ground 兜底）/ `:82+:148` swarm_wave if/else（count×2 + HP×0.5）/ `enemy_damage.gd:35-36` + `enemy_boss.gd:85 _check_phase_transition` 链（血线只换技能池无演出）② **数据层落地 = Excel 两新 sheet**（spawn_points / boss_phase_events，data_schema.py :228-257 SHEETS dict 形注册先例 enemy_sprites/behavior_map/audio_config/fx_config）+ waves 表加 `spawn_set`/`spawn_order` 列；生成 data/spawn_points.json + data/boss_phase_events.json 独立文件（结构同 sheet 平铺）；elements 表策划按需加 type=attr 行（如 frenzy）③ **FK 校验新增**（excel_export.py 校验清单：spawn_set 引用不存在 point_id / boss_phase_events 引用不存在 boss_id → 报错；**⚠️ --check-only 已知缺陷（check_only 参数未消费 = 校验路径仍全量导出，已登记未修）本轮标注顺手修或登记 TECH_DEBT**）④ **演出执行器 = 新脚本 `boss_phase_player.gd`**（独立，F4 拆分范式；banner 仿 game_manager.gd:392 `_show_elite_banner` Label+tween 淡出上浮 1.5s 自毁；vfx/sfx 走 VfxPlayer/audio_manager 注册表；未知类型 warn + 跳过；camera 复用 main.gd:240 `_trigger_camera_shake(level)`；buff 走 status_component apply_effect）⑤ **attr 状态 = status_component.gd:140 `_apply` / :157 `_revert` 加 attr 分支**（复用 slow 范式：进入 `_target[target_attr] *= (1 + value/100)` + 记录 orig → 到期/刷新还原；value 可正可负；叠加/免疫/max_stacks/同源刷新/异源独立沿用现有组件规则零新增；target_attr 不存在 warn + 跳过；**damage_multiplier 消费点已实测全乘**：weapon_controller:338/354、skill_controller:178/341、turret:104、orbit_weapon:117、melee_sweep:108）⑥ **美术硬性约束**：点位/演出全部占位（纯色/文字/复用现有 VfxPlayer 特效），零美术生成。**批次依赖（规格 §8）**：批 A 数据地基先行 → B/C 依赖 A；E 依赖 A 的 elements 示例行（可选）；D 可选独立。**执行序建议：LD-A（数据地基）→ LD-B（固定出生点）→ LD-C（Boss 演出）→ LD-E（attr 状态）→ LD-D（特殊波参数化可选）→ LD-EXIT**。每任务一收口 commit 带 LD 编号；**回归硬门槛 = 62 件套 1534 断言 + baseline BASELINE CLEAN**；**验收锚点（规格 §8）**：回归 62 件套全绿 + 新增探针 ≥3 组 + Excel 改 spawn_set/boss_phase_events 导出后游戏内实测生效（#4 测试 + #5 试玩各走一轮）。

### LD-A 数据地基（两新 sheet + 管线 + 接口 · 前置批 · B/C/E 全依赖）
- [x] **LD-A1【W2】Excel 两新 sheet + waves 新列（数据侧）**【✅ 收口 2026-08-19 #3 执行者 `96e4cd5`：spawn_points sheet 11 行（8 边缘 edge inset40 + boss_top edge/north/inset60 + arena_center anchor 0.5/0.45 + ring_outer ring/300）+ boss_phase_events sheet 7 行（invoker 100 banner/60 banner+buff/40 camera + predator 100 dialogue/66 vfx/33 sfx = 6 类型全覆盖）+ waves G/H 列 spawn_set/spawn_order（wave 1/10/20 示例填值，其余 17 波留空 = 缺省边缘均匀组）】：docs/GameData.xlsx 新增 `spawn_points` sheet（**≥9 行 × point_id/type/direction/x/y/radius/inset/min_dist_player 8 列**：默认 8 边缘点 n/s/e/w/ne/nw/se/sw（type=edge，inset=40 拍板 D1）+ `boss_top`（edge north inset 60）或 `arena_center`（anchor 0.5/0.45，Boss 登场 anchor(0.5,0.3) 拍板 D2 时用）+ `ring_outer` 示例（type=ring radius=300，ring 圆周均分示例）——anchor 型 x/y 为 0~1 局部比例运行时乘竞技场尺寸）+ 新增 `boss_phase_events` sheet（**≥6 行 × boss_id/hp_threshold_percent/seq/event_type/param/once 6 列**：示例 = invoker 100 登场 banner + 60 段 banner「狂怒！火力全开」+ buff frenzy（elements 表 type=attr 行）+ 40 段 camera heavy + 击杀不触发口径；predator 同理 2-3 行；event_type 枚举 banner/vfx/sfx/dialogue/camera/buff 各 ≥1 覆盖）+ waves sheet 加 `spawn_set`（JSON 数组列如 ["north","south_east","ring_outer"]）与 `spawn_order`（sequence/random，缺省 sequence）列（20 波**全部留空 = 缺省边缘均匀组**，仅 1-2 波示例填值供探针端到端验证）。**测试点**：sheet 双行表头规范（第 1 行英文 + 第 2 行中文注释，F1-G-尾升级范式）；spawn_points ≥9 行含 8 边缘 + ring + anchor 三型；boss_phase_events 含 5 类型覆盖 + 同阈值 seq 多条示例
- [x] **LD-A2【W1】excel_export 注册 + FK 校验**【✅ 收口 2026-08-19 #3 执行者 `96e4cd5`：data_schema 注册 spawn_points（spawn_points.json/root spawn_points/key point_id/dict）+ boss_phase_events（boss_phase_events.json/root events/key None（boss_id 非唯一，唯一性检查跳过）+ param json_cols）+ waves json_cols 补 spawn_set；excel_export 构建段两表（spawn_points 平铺 dict + boss_phase_events 按 boss_id 分组、组内 hp_threshold+seq 排序）+ FK 校验新段（spawn_set→point_id 非法报错 / boss_id→boss_pattern∪enemies(boss) 并集非法报错，三态实测验证）+ **--check-only 已知缺陷顺手修（check_only 参数未消费 → 校验+roundtrip 自检后直接返回不写盘，纯只读回归护栏）**；导出 15 旧 JSON 零 diff + 2 新文件】：tools/data_schema.py 注册 `spawn_points`（file: spawn_points.json / root: "spawn_points" / kind: "dict" / key: point_id，仿 enemy_sprites :228-234）与 `boss_phase_events`（file: boss_phase_events.json / root: "events" / kind: "dict" / key: boss_id，仿 audio_config :246-251）+ waves sheet 注册 spawn_set/spawn_order 两列（json_cols 补 spawn_set）；tools/excel_export.py 构建段追加两表解析（spawn_points 平铺 {point_id: {...}}；boss_phase_events 按 boss_id 分组 {boss_id: [events...]}，组内按 hp_threshold_percent+seq 排序）+ **FK 校验新增**（spawn_set 引用不存在的 point_id / boss_phase_events 引用不存在的 boss_id → 报错退出，仿既有 FK 检查段；**顺手修 --check-only 已知缺陷（check_only 参数未消费）或登记 TECH_DEBT**）；导出 → spawn_points.json + boss_phase_events.json 两新文件，**其余 16 JSON 零 diff 断言**（前三批先例）。**测试点**：JSON 校验通过；两新文件结构正确（spawn_points dict / boss_phase_events 分组排序）；FK 校验三态（合法/坏 point_id 报错/坏 boss_id 报错）；其余 JSON 零 diff
- [x] **LD-A3【W1】DataLoader 接口**【✅ 收口 2026-08-19 #3 执行者 `96e4cd5`：get_spawn_points() 懒加载（_spawn_points is_empty 重试标记）+ get_spawn_set(wave)（读 waves 行 spawn_set/spawn_order，空/缺失 → {spawn_set: [], spawn_order: "sequence"} 缺省回退）+ get_boss_phase_events(boss_id)（懒加载 events 分组，缺失 → 空数组零崩）】：新增 `get_spawn_points() -> Dictionary`（懒加载 spawn_points.json 缓存 + 空表标记，仿 get_enemy_sprite_config :600-604 范式）+ `get_spawn_set(wave: int) -> Dictionary`（读 waves 行 spawn_set/spawn_order，缺省 → 默认边缘组标记，仿 get_wave :482）+ `get_boss_phase_events(boss_id: String) -> Array`（读 boss_phase_events[boss_id]，缺失 → 空数组零崩）；未命中/损坏 → 空数据（消费端缺省回退，零崩）。**测试点**：白盒读三接口 → 数据正确；缺失 boss_id → 空数组；表空 → 空字典（消费端兜底）
- [~] **LD-A-EXIT【W5】收口**【🟡 部分收口 2026-08-19 #3 执行者 `96e4cd5`：新建 `tools/day31_level_design_data_check.gd` **24/24 PASS**（≥12 要求达成：spawn_points 11 键三型/键集完整/edge inset40/boss_top/anchor/ring + boss_phase_events 分组排序/6 类型/param 解析/once + 三接口读值/缺省回退/未知 boss 空数组 + FK 数据侧合法 + waves 示例填值/缺省）+ runner 并入 **64 件套** + day26 锚点 **64/1602** 同步 + baseline **BASELINE CLEAN** + excel_export --check-only EXIT=0 且只读不写盘（缺陷修复验证）；**⚠️ 回归硬门槛 = 59/64（5 FAIL：day2_hero/day3_skill/day5_weapon/day31_charsel/day31_player_model）——全部根因 = 用户会话在途代码**（工作区未提交 `player_anim.gd` D-26 `_add_anim_from_frames` 无条件调用 + `sprite_frame_factory.gd` 新 `create_from_frames` 内 `SpriteFrames.set_frame_offset` = **Godot 4.3 无此 API** → 角色动画链 SCRIPT ERROR，与 LD-A 改动零关联，LD-A 探针/presentation/day26 均 PASS）；**待用户会话收口（适配 Godot 4.3 或移除该调用）后复跑回归恢复 64/64**】：新建 `tools/day31_level_design_data_check.gd`（**≥12 断言**：spawn_points 键齐全（8 边缘+ring+anchor）/ boss_phase_events 分组+排序正确 / FK 校验三态（白盒调校验函数）/ 三接口读值正确 / 缺省回退（无 spawn_set 波 → 默认边缘组）/ 其余 JSON 零 diff）+ 回归 62 件套 + baseline **BASELINE CLEAN**

### LD-B 固定出生点生成（enemy_spawner 按点位生成 · 依赖 LD-A）
- [x] **LD-B1【W1】enemy_spawner 按点位生成**【✅ 收口 2026-08-19 #3 执行者：`enemy_spawner.gd` 新增 `_get_spawn_position()`（表驱动主路径：`_spawn_override` 点位组 + sequence 轮换 index 推进/random 组内随机（`_rng` 实例防全局 RNG）+ edge/anchor/ring 三型解析（edge 8 向边缘/角落 + inset 内缩；anchor x/y 比例 × 竞技场尺寸；ring 圆周均分按已生成数取角）+ min_dist_player 尝试换点（≥3 次或全覆盖）仍过近原样生成不静默丢弃 + `_clamp_to_ground` 钳制）+ `_get_arena_rect()`（world→ground `get_ground_rect`；缺失缺省 1536×864 原点对齐——探针确定性）+ `spawn_wave` 加 `spawn_override: Dictionary = {}` 可选参数（缺省兼容旧调用）；`_create_enemy` 位置改走 `_get_spawn_position()`；**`_get_random_spawn_position` 原函数保留**（缺省回退路径 + day31_wave5_repro_check:119 直接调用零破坏）】：`_get_random_spawn_position()`（:172）→ 新增 `_get_spawn_position(entry: Dictionary) -> Vector2`（**表驱动主路径**：读 `get_spawn_set(wave)` → 点位队列**按 spawn_order 轮换**（sequence = 数组循环 index 递增 / random = 组内随机；单只连续生成多只时 index 推进防同一角落堆叠）；点位解析——edge：按 direction 映射竞技场边缘 + inset 内缩（仿 F-44 `_clamp_to_arena` 边界钳制）；anchor：x/y 比例 × 竞技场尺寸；ring：center + radius 圆周均分（按已生成数取角）；仍过 `min_dist_player` 兜底（尝试 3 次换点，仍近原样生成不静默丢弃——沿用现兜底语义）+ `_clamp_to_ground` 钳制；**缺省回退**：spawn_set 空/点位表空/point_id 不存在 → 保留现 `_get_random_spawn_position`（±200×±120）兜底零回归；**F-48 修复不回归硬门槛**（生成点必在视野内：点位 inset 40 边缘 + Aggro Leash 320 双保险，day31_flee_bound 18/18 零改动）。**测试点**：白盒直构造点位表 → 各 type 解析正确（edge 内缩 / anchor 比例 / ring 均分）；sequence 轮换 index 推进；缺省回退走随机路径；`_clamp_to_ground` 兜底保留
- [x] **LD-B2【W1】wave_manager 透传 spawn_set**【✅ 收口 2026-08-19 #3 执行者：`wave_manager.gd:92-99` `spawn_wave` 调用点显式组装 `spawn_override`（config 的 spawn_set/spawn_order，缺省空数组/sequence → 生成器缺省回退零回归）；生成器 `spawn_wave` 第三参可选 → 唯一调用点 wave_manager 之外零影响】：`spawn_wave`（wave_manager.gd :70 附近调用链）透传 wave 的 spawn_set/spawn_order 给 enemy_spawner（spawn_wave 参数扩展 Dictionary 缺省空 → 兼容旧调用零回归；day14_15/精英/Boss 各路径统一走透传）。**测试点**：白盒 spawn_wave 带 spawn_set → spawner 收到并解析；不带 → 缺省回退
- [x] **LD-B3【W1】探针扩展**【✅ 收口 2026-08-19 #3 执行者：day31_level_design_data_check +§6 出生点消费 **12 断言**（>8 要求：GameManager 在位 / edge north 内缩 (768,40) / anchor 比例 (768,388.8) / ring 首角 0° + 次角 45° 推进 / sequence north→south→north 循环 / random 组内随机（种子 42）/ 缺省回退 F-48 门 ≤233.5 + 盒内 ±200×±120 / point_id 不存在随机兜底 / min_dist 过近原样生成（注入 mock player）/ _clamp_to_ground 接线（注入 mock world clamp+10 偏移验证）+ 探针卫生还原）+ runner 预期 24→**36** + day26 锚点 64/**1614** 同步】：day31_level_design_data_check +§出生点段（**≥8 断言**：edge 点位生成位置在竞技场内且 inset 生效 / anchor 比例换算正确 / ring 圆周均分角度正确 / sequence 轮换 index 推进不重叠 / random 组内随机 / 缺省回退随机路径 / F-48 视野内硬门槛（生成点距玩家 ≤233.5 对角线）/ _clamp_to_ground 兜底）。**回归硬门槛**：day31_flee_bound 18/18 零改动（F-48 修复）+ 62 件套
- [~] **LD-B-EXIT【W5】收口**【🟡 部分收口 2026-08-19 #3 执行者：探针 **36/36**（LD-A 24 + LD-B §6 12）+ flee_bound **22/22** 零改动（F-48 不回归）+ wave5_repro **9/9** + day26 锚点 **64/1614** + excel_export --check-only EXIT=0（本轮纯消费端零数据改动）+ **全量回归 59/64（5 FAIL = day2_hero/day3_skill/day5_weapon/day31_charsel/day31_player_model，全部根因 = D-26 用户会话在途 `set_frame_offset` Godot 4.4 API 误用（`sprite_frame_factory.gd:92` + `player_anim.gd:99`，工作区未提交），与本批零关联，与上轮 FAIL 清单完全一致零新增）** + PLAYTEST 主观项登记（LD-B 固定出生点行：怪从可预判方向涌入 / Boss 正上方登场 / F-48 不回归 / 点位观感）+ baseline 待 #4 快照；**EXIT 门槛统一挂 D-26：用户会话收口后复跑回归恢复 64/64 即转 [x]**】：回归 62 件套 1534 断言 + day31_level_design_data_check 全段 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（怪从视野边缘涌入的可预判感，交 #5）

### LD-C Boss 阶段演出（boss_phase_events 触发 + 演出执行器 · 依赖 LD-A）
- [ ] **LD-C1【W1】新建 `scripts/boss/boss_phase_player.gd`（演出执行器）**：注册表驱动——`play_events(boss_id, threshold_hit)` 读 `get_boss_phase_events(boss_id)` → 过滤 `hp_threshold_percent == threshold` 的 events → 按 seq 顺序执行：**banner**（param.text/color/duration → 仿 game_manager.gd:392 `_show_elite_banner` Label + tween 淡出上浮 1.5s 自毁范式，自持样式不依赖 GM 静态函数，color 缺省红）；**vfx**（param.name → VfxPlayer 注册表按名调用，未知名 push_warning + 跳过）；**sfx**（param.name → audio_manager.play_sfx，未知名 push_warning + 跳过）；**dialogue**（param.text/speaker → 先复用 banner 样式占位，D3 拍板；后接台词框扩展时替换）；**camera**（param.strength/count → main.gd:240 `_trigger_camera_shake(level)`，count 连续调用）；**buff**（param.target/effect/duration → status_component.apply_effect 或对应组件，target=self= Boss 自身 / player=玩家，frenzy 示例行）；**once 语义**（once=1 只触发一次，触发后本地标记防重；once=0 每次进该阈值段触发，慎用）；未知 event_type push_warning + 跳过。**测试点**：白盒 play_events 各类型 → banner 节点生成 + vfx/sfx 调用 / 未知名跳过不崩 / once 只一次 / seq 顺序执行
- [ ] **LD-C2【W1】phase 钩子接线**：`enemy_damage.gd:35-36` 存活命中 `_check_phase_transition()` 链 → Boss 血量 ≤ 阈值时（**> 上一阈值 且 ≤ 本阈值**语义与 phases 一致）同处追加 `boss_phase_player.play_events(boss_id, threshold)`；**击杀瞬间不触发**（沿用决策 D6：死亡帧残留横幅，die() 路径不接）；同帧多次命中只执行一次（once=1 本地标记兜底）；100 阈值（开局登场演出）= Boss 入场时触发（enemy.gd initialize 或 spawn 钩子，仿 _show_boss_banner 位置）。**测试点**：白盒血量降至阈值 → 演出触发且 seq 有序 / 击杀瞬间零触发 / once 防重 / 100 开局触发
- [ ] **LD-C3【W1】探针扩展**：day31_level_design_data_check +§演出段（**≥8 断言**：阈值语义（> 上一阈值 ≤ 本阈值）/ 击杀瞬间不触发 / once 只一次 / seq 顺序 / 5 类型各执行一次 / 未知名跳过不崩 / 100 开局登场 / buff 附加生效）。**回归硬门槛**：day18_19_boss_check 48/48 + day30_boss_skill 49/49 零改动（相位/技能链不动）+ 62 件套
- [ ] **LD-C-EXIT【W5】收口**：回归 62 件套 + day31_level_design_data_check 全段 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（Boss 血线演出横幅/特效/台词观感，交 #5）

### LD-E attr 正向属性状态（status_component 扩展 + elements 示例行 + buff 联动 · 依赖 LD-A）
- [ ] **LD-E1【W1】status_component attr 分支**：`_apply`（:140-155）match 新增 `"attr"` 分支——`target_attr` = def 键（如 damage_multiplier/move_speed/armor/crit_chance/crit_damage/attack_speed）：`if target_attr in _target: inst["orig_" + target_attr] = _target[target_attr]; _target[target_attr] = float(_target[target_attr]) * (1.0 + float(def.get("value", 0.0)) / 100.0)`（**value 可正可负**：+100 = +100% / -30 = -30%）；target_attr 不存在 → push_warning + 跳过（防脏数据崩游戏）；`_revert`（:157+）match 补 `"attr"` 对称还原（读 inst["orig_" + target_attr] 回写）；叠加/免疫/max_stacks/同源刷新/异源独立**沿用现有组件规则零新增**（slow 范式完全同构）。**测试点**：白盒 attr 效果 → 属性乘算正确（+50% → ×1.5）；负值（-30% → ×0.7）；到期还原；刷新重算；target_attr 缺失 warn + 跳过；max_stacks/免疫沿用
- [ ] **LD-E2【W2】elements 示例行（数据侧）**：docs/GameData.xlsx elements sheet 新增 type=attr 行（≥2：`frenzy` 狂暴 = {type: attr, value: 50, target_attr: damage_multiplier, duration: 10} / `swift` 迅捷 = {type: attr, value: 30, target_attr: move_speed, duration: 5} 示例；**attr 白名单按消费点实测登记**：damage_multiplier ✓ 全乘（weapon_controller:338/354、skill_controller:178/341、turret:104、orbit_weapon:117、melee_sweep:108）/ move_speed ✓ / armor（平直减伤走 armor 类型，attr 乘算可选）/ **crit_chance/crit_damage/attack_speed 若实际属性名缺失 → 登记 TECH_DEBT（D5 拍板）**）；data_schema 注册（elements 已有 type 枚举校验，若校验白名单需加 attr）+ excel_export 导出 → elements.json 新增 attr 行（其余 JSON 零 diff）。**测试点**：导出后 elements.json attr 行在位且 type=attr；未知 target_attr 不在白名单 → 探针断言或 TECH_DEBT 登记
- [ ] **LD-E3【W1】buff 事件联动**：boss_phase_events 的 `buff` 类型（LD-C1 已执行器）→ 消费 attr 效果（frenzy 行 → Boss 狂暴 attack+50% 10s，演出+数值一表打通）；target=player 分支（对玩家附加，慎用——探针白盒覆盖 + PLAYTEST 主观项）。**测试点**：Boss 血线 60% buff 事件 → Boss damage_multiplier ×1.5 生效 10s 后还原；target=player 白盒
- [ ] **LD-E-EXIT【W5】收口**：新建 `tools/day31_level_design_attr_check.gd`（**≥10 断言**：attr 乘算/负值/还原/刷新/max_stacks/免疫沿用 / elements attr 行在位 / buff 事件联动（Boss 狂暴生效与还原）/ target=player 白盒 / 未知 target_attr warn）+ 回归 62 件套 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（Boss 狂暴时玩家应对体感，交 #5）

### LD-D 特殊波参数化（可选批 · 规格 §5 · 成本 >0.5 天则降级记 TECH_DEBT）
- [ ] **LD-D1【W2】wave_generation 扩展（数据侧）**：docs/GameData.xlsx wave_generation sheet（或新 special_rules 表，方案师定案）加行：`special / count_mult / hp_mult / damage_mult`（swarm_wave = 2/0.5/1 现值；curse/high_pressure 等标记语义散落 wave_manager/event_manager 的逐个核清后入表）；data_schema 注册 + excel_export 导出（其余 JSON 零 diff）。**测试点**：导出后键在位；swarm 现值 2/0.5 零漂移
- [ ] **LD-D2【W1】spawner 读表替换 if/else**：enemy_spawner.gd:82（count×2）+ :148（HP×0.5）→ 读 wave_generation 扩展行（special 匹配 → count_mult/hp_mult/damage_mult 应用，**缺省 1/1/1 零行为变化**）；wave_manager/event_manager 的 curse/high_pressure/chest_enemy 语义若散落 → 一并核清入表（评估成本，>0.5 天则只收 swarm 主项 + 其余登记 TECH_DEBT）。**测试点**：白盒 swarm 波 → count×2 + HP×0.5 与现行为一致（探针断言零漂移）；无 special 行 → 缺省 1 倍
- [ ] **LD-D-EXIT【W5】收口（可选）**：day31_level_design_data_check +§特殊波段（≥6 断言）+ 回归 62 件套 + baseline **BASELINE CLEAN**；若降级 → 登记 TECH_DEBT_PLAN 不做 EXIT

### LD-EXIT 总收口
- [ ] **LD-EXIT【W5】总收口**：全批探针全绿（day31_level_design_data_check + attr_check）+ 全量回归 **62 件套 ≥1534 断言** + baseline **BASELINE CLEAN** + Excel --check-only 通过（含 FK 校验新逻辑）+ DATA_DICT_GUIDE.md 补两新表说明（策划改数手册收口后由 #3 或 #1 顺手更新）+ PLAYTEST 主观项登记（点位可预判感 / Boss 血线演出 / attr 狂暴体感，交 #5）+ TECH_DEBT_ISSUES 新债登记（如有：--check-only 缺陷 / crit·攻速属性缺失）
