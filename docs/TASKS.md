...
> 📌 **F1-E-4 第四批 FX 抽表（2026-08-18 第 60 轮拆解 · 🏠 主窗口承接 · 数据管线铁律：改 docs/GameData.xlsx → tools/excel_export.py → 探针，data/*.json 禁手改）**：沿前三批（SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX）同模式——Excel sheet → data_schema 注册 → excel_export 构建 presentation.json → DataLoader 接口（懒加载 + const 兜底）→ 消费点改读 → 探针 → 回归。**实测锚点**：`vfx_player.gd:17-29` FX_CONFIG 10 键（5 旧 hit/crit/death/levelup/pickup + 5 新 fireball/turret_deploy/blade_burst/meteor/shield，各含 path/frames/size(Vector2i)/fps）、`set_effect` :43-62（FX_CONFIG.has 未知键 push_warning → cfg 取用 → load(path) null 静默跳过 → create_from_sheet 构建）、`spawn` :65-73 静态（load VfxPlayer.tscn + set_effect）；**回归硬门槛 = `tools/day23_vfx_check.gd` §1 配置层断言 FX_CONFIG 10 键（:138-143）+ set_effect 未知键/命中白盒（:164-171）**；excel_export.py:399-434 presentation 构建段（audio_map 先例 :424-434）+ data_schema.py:241-245 注册范式（dict 形 id 主键）+ day31_presentation_check 探针（273/273，§1-§5 已含 enemy_sprites/behavior_map/audio_map 段）。**关键决策：FX_CONFIG const 保留为兜底**（F 系列缺省兜底约定）→ day23_vfx_check §1 零改动硬门槛 + 抽表零数值变化（仅帧配置数据化；size 为 Vector2i 无法直存 JSON → Excel 拆 size_w/size_h 两列，导出组装 {"x","y"}，DataLoader 消费组装 Vector2i，仿 get_enemy_sprite_config :610-612 先例）。**执行序 F1-E-4-1 → 4-2 → 4-3 → 4-4 → EXIT，每任务一收口 commit 带 F1-E-4 编号**。
