...
- [~] **PS-D1【W2】routes.json chapter 字段（⛔ 部分落地 + 阻塞登记：chapters 字段已落地 4 章定义 3/4/4/4 + 章末类型章1=event 章2-4=boss（`e9f4289` 数据层，走 Excel routes sheet → excel_export.py），day31_chapter_check 5/5 + 回归 52/52 全绿 ✅；**⚠️ boss_layers 映射调整执行阻塞**——F-27 用户拍板「15 层双 Boss [9,14]」vs 章节化 4 章三 Boss 位（章2/3/4 末 = 层 7/11/15 → 0-based [6,10,14]）冲突（day14_15/fb5 探针红），已按方案 R5 兜底回滚 boss_layers 至 [9,14]，**交方案师/用户裁决**（二选一：① 保留双 Boss [9,14] 章节化仅做章界表现 ② 改三 Boss [6,10,14] 同步 3 处探针锚点 + F-27 复核））（§8）**：docs/GameData.xlsx routes sheet 扩展（chapter_id / 主题怪池 / 精英位 / 章末节点类型）+ **4 章层数 3/4/4/4**（章 1 层 1-3 / 章 2 层 4-7 / 章 3 层 8-11 / 章 4 层 12-15）+ 章末节点类型（**章 1 = 事件休息+奖励，无 Boss**（D3）/ 章 2-4 = Boss）→ excel_export.py → **⚠️ 触及 day14_15 53/53 数据结构层 + F-28 通关判定（Boss 关击杀即通）+ F-27 15 层双 Boss 映射 + 大地图显示 → 逐探针验证，禁一次改完再验**
- [ ] **PS-D2【W1】章末事件节点 + Boss 位映射（§8 · 本轮第 48 轮细化拆解）**：分两块——**D2a 章末事件节点（不依赖裁决 · 可先行）**：route_generator 消费 chapters 的 `end_type`——章 1 末层（层 3）节点生成强制 `"event"`（章 2-4 末层强制 `"boss"`），复用现有事件面板/奖励结算 10 型（GameManager `_start_event` 链路，D16 已通，章末事件可带「休息 + 奖励」语义：heal 全量 or 金币 or 属性，数值走 events.json 现有条目 or 新条目走 Excel events sheet）；**D2b 章 Boss 位映射（依赖方案师裁决 · 阻塞标注）**：boss_layers 从 F-27 双 Boss `[9,14]` 调整为 4 章三 Boss 位 `[6,10,14]`（章 2/3/4 末层 = 第 7/11/15 关 Boss）——**须先裁决（F-27 用户拍板复核）**，落地时改 docs/GameData.xlsx routes sheet boss_layers 列 → excel_export.py → day14_15 53/53 + day18_feedback5 27/27 + day30_g_map 20/20 + day31_chapter_check 锚点同步；章间可插整备节点（商店/回血/事件）做呼吸感（数据驱动，可选增强不阻塞）
- [ ] **PS-D3【W1】大地图章界（§8 + G-R1 承接 · 本轮第 48 轮细化拆解）**：route_select_panel.gd（183 行，G-R1 网格画布已落地）——`setup()` 读 `route.chapters`（DataLoader.get_routes 已解析）→ 每章起始层上方渲染**章界横幅/分隔线**（Label「第 N 章」+ Line2D 分隔线，数据驱动：chapters 缺省空 → 零显示零改动，旧 routes.json 兼容）；**硬门槛：不动画布架构/布局函数（_layout_pos/_render/_draw_paths 语义零改动），day30_g_map 20/20 零改动**；占位标准 = 色块 + Label 零美术（08-07 策略遵守）
- [x] **PS-D4【W1】探针 `tools/day31_chapter_check.gd`（≥10 断言 · 已建 5/5 + §4 章界段待 D3 后扩展）**：4 章拓扑合法（层数 3/4/4/4 + 章末类型正确）/ 章 1 无 Boss 章末事件 / 章 Boss 击杀即通（F-28 兼容）/ 大地图章界显示 / **day14_15 53/53 + day18_feedback5 27/27 + day30_g_map 20/20 + day30_f1_scaling 回归全绿**【✅ 已建 2026-08-16：§1 数据层 4 章 / §2 层数拓扑 / §3 boss_layers 维持 [9,14] 兼容 + chapters JSON 解析，5/5 全绿；D2a/D3 落地后扩展 §4（章末事件生成 + 章界显示断言）】
- [ ] **PS-D-EXIT【W5】回归**：47 件套 + baseline **BASELINE CLEAN**【收口条件：D2a 章末事件 + D3 章界先行收口（零裁决依赖）→ D2b 待方案师裁决落地后一并 EXIT；回归 52 件套 ≥1046 断言】
