...
- [ ] **F1-E-4-2【W1】DataLoader 接口**【承接方 = #3 执行者（用户 08-18 23:1x 拍板 · F1-E-4-1 已收口，自本步续做）】：新增 `get_fx_config(fx_name: String) -> Dictionary`（懒加载 presentation.json fx_config 缓存 + 空表标记，仿 get_audio_config :641-646 / get_enemy_sprite_config :602-617 范式）；命中 → 组装返回 {path, frames, fps, size: Vector2i}（size JSON → Vector2i，仿 :610-612）；未命中/损坏 → 空字典（消费端 const 兜底，零崩）。**测试点**：白盒读 get_fx_config("hit") → 键齐全 + size == Vector2i(32,32)；改 Excel frames → 导出 → 返回值变化（端到端双跑，F1-散 §1 先例）
...
## LEVEL_DESIGN 关卡设计扩展（2026-08-18 用户拍板 · 规格 docs/LEVEL_DESIGN_SPEC.md · ✅ 已拆解 2026-08-19 第 62 轮 · 未拆解前禁动工 · 拆解完成即解锁 · 方案唯一事实源 LEVEL_DESIGN_SPEC.md · 独立目标日 Day 31+ 与 RELIC 同窗口 · 承接方 = #3 执行者（用户 08-18 23:1x 拍板解除「#3 勿自行开工」· D-015 交接））
