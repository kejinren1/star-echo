...
### Day 28 — 全量测试 + 性能　🎯 当前目标日（F-31 · P0 用户拍板首段 + #4 测试域 · 方案师第 13 轮头部确认 13:1x · #2 第 38 轮 14:05 拆解 F-31）

#### 🚨 D28-F31【W1 主责 / W2 只读核验】F-31 武器升级体系（反馈 2 三子项 · P0 用户拍板 · 2026-08-08 第 38 轮拆解）
...
##### D28-F31-1【W1 主责 / W2 ◐只读】初始武器出商店池（子项 a）
- [ ] **W1** `scripts/autoload/data_loader.gd` 新增纯函数 `get_starting_weapon_ids() -> Array[String]`：遍历 `get_all_characters()`（或 characters 数组）收集 `starting_weapon` 字段**去重**返回（实测 10 把）。零数据改动（characters.json 已是单一事实源）。
- [ ] **W1** `scripts/ui/shop.gd` `_build_shop_pool()` :172-178 武器循环首行加跳过：`var starting_ids: Array = DataLoader.get_starting_weapon_ids()`（循环外收集一次）→ `if wid in starting_ids: continue`（放 `evolution_result` 跳过之后）。
- [ ] **W2 ◐只读核验**：characters.json 10 把 starting_weapon ↔ weapons.json 36 把交叉全命中（#2 已实测，执行时复核即可，零写）。
- **测试点**：`_build_shop_pool()` 武器段 == 23；10 把起始武器 id 零出现；被动 23 + 遗物 2 不受影响（总池 48）。

##### D28-F31-2【W1】升级面板移除「武器升级」选项（子项 b）
- [ ] `scripts/ui/level_up_panel.gd` `_roll_options()` :64-72 **删除武器升级池段**（`weapon_controller` 遍历 `weapon.level < weapon.max_level` → append weapon_upgrade 的整段 for 块）；:61-63 的 `weapon_controller` 获取保留（进化池 :74 复用）。
- [ ] 保留属性池（:58-60 `leveling.upgrade_options`）+ 进化池（:73-104，含方案 A 保底 :105-117）——**属性/进化两型即可 roll，3 选 1 恒成立**（属性池 12 项 ≥3 保底）。
- [ ] `_apply_option` :137-141 weapon_upgrade 分支**保留**（防御性零回归；铁砧升级 UI 亦可复用 `weapon.upgrade()` 语义）；注释更新（:147 desc_builder option_tooltip「属性/武器升级/进化三型」→ 面板实际两型，weapon_upgrade 分支留作铁砧/兼容路径）。
- **测试点**：未满级武器场景 `_roll_options(99)` 结果零 `type == "weapon_upgrade"`；满级+持核心 `_roll_options` 仍必含 evolution（F-20 保底不回归）；属性池仍正常 roll。

##### D28-F31-3【W1】铁砧 anvil 120G 零消费点闭环（子项 c）
- [ ] **入池**：`scripts/ui/shop.gd` `_build_shop_pool()` 新增「服务池」段（武器池/被动池/遗物池之后）：遍历 items 收集 `effects.shop_weapon_upgrade == true` 条目（实测仅 anvil）→ `_build_item_resource(iid)` 入池（不设 is_passive/slot，独立类别）。
- [ ] **购买分支**：`_purchase_item()` :319 新增第三分支（`item.stat_bonuses.get("shop_weapon_upgrade") == true`，置于武器/被动分支之前）：
  - 收集 `GameManager.player.WeaponController.equipped_weapons` 中 `level < max_level` 的可升级武器（无 → push_warning「无可升级武器」**拒绝购买不扣费**）
  - 有 → 弹**武器升级选择 UI**（建议 `shop.gd` 内新建轻量选择行/小面板：仿 LevelUpPanel 卡片渲染，只列可升级武器，显示当前 Lv → Lv+1 + 数值；点选 → `weapon.upgrade()` + `GameManager.economy.spend_coins(120)` + `shop_items.remove_at(index)` + `purchase_made.emit` + `AudioManager.play_sfx("shop")`；取消不扣费）
  - anvil 为一次性商品卡（购买后移除；刷新重随机）
- [ ] `desc_builder.gd` :49/:70 `shop_weapon_upgrade` 中文映射已就绪（「商店武器升级」）→ anvil 卡片 tooltip 自然生效；图标：anvil 无专属帧 → `icon_index` 0 兜底或复用现有占位帧（**占位纯色口径，零新图，W3 P1 可延**）。
- **测试点**（白盒）：anvil 出现在 `_build_shop_pool()`；无可升级武器 → 拒绝且金币不变；可升级 → 选 1 把 +1 级 + 扣 120G + 商品移除；升满后（level==max_level）不再出现在可升级列表。

##### D28-F31-EXIT【W5】回归 + 探针
- [ ] **新探针 `tools/day28_f31_check.gd`**（四段）：§1 商店池口径（武器 == 23 / 10 起始零出现 / 被动 23 + 遗物 2 / anvil 在池）；§2 升级面板 `_roll_options` 零 weapon_upgrade + 属性池可 roll + 进化保底不回归（满级持核心必含）；§3 铁砧闭环白盒（拒绝不扣费 / 升级 +1 级 + 扣 120G + 移除 / 满级不再列）；§4 回归抽样。
- [ ] **回归同步 2 处必红**（b 落地后）：`tools/day5_weapon_check.gd` :208-222（「选项池应含 weapon_upgrade」→ 反向断言「不含」，可顺带验证属性池仍可 roll）；`tools/day13_build_check.gd` :494-507（「未满级武器 → 有 weapon_upgrade 选项」→ 反向）。`tools/day18_feedback5_check.gd` :142 测 option_tooltip weapon_upgrade 纯函数（分支保留）**不红勿动**；`tools/day16_event_check.gd` 事件奖励 weapon_upgrade（GameManager `_apply_event_reward` 路径）**与面板无关不红保留**（事件奖励随机武器 +1 级仍合法）。
- [ ] `python tools/baseline_check.py` → **BASELINE CLEAN**；回归基准 = **二十八件套 ≥733 断言 + day28_f31 新增**（#37 14:02 已兑现 733，F-31 落地后 ≥ 28 件套 + N）。
- [ ] git commit 收口（b + c 同批，**禁 b 单独收口**；改前 commit、改后 baseline_check 护栏）。
- [ ] 收口后交 #5 登记真人回归（升级面板只剩属性/进化 / 商店不再刷起始武器 / 铁砧 120G 买武器升级 / 进化链经铁砧升满仍可达）；目标日仍 Day 28（#4 性能段交 Owner 未决维持观察）。
