...
### F3 状态机规范化【已拆解 2026-08-13 第 45 轮 · 待执行 · **方案已定：SOLUTION_PLAN.md 第 20 轮**】（T-031~036：GM 状态 context 化 / enemy Boss 阶段枚举+状态表 / player 行为态枚举 / 双标志收敛 / 类型统一 · TECH_DEBT_PLAN §4 F3 + §8 自研两形态）
...
- [ ] **F3-T0【W1】docs/CODE_STYLE.md 状态机规范章节（批次 A 首步 · 新建）**：写入 §2.6 两种固定形态（① 扁平流程态 = enum + match + `_transition(next, context)`；② 行为/表现态 = enum + 状态表 Dictionary）+ 四条禁令（多 bool 组合 / 字符串状态值 / int 字面量状态 / 状态切换散落多处）+ §8.6 能力上限与质疑流程（单机 ≤8 态 / 转移条件 ≤10 / 状态表 ≤20 行，超限停手先问）；F5 复用为评审清单
- [ ] **F3-T1【W1】GM `_set_state` → `_transition(next, context)` 升级（T-031 收口 · 批次 A）**：game_manager.gd:146 `_set_state(next: int)` 升级签名 `_transition(next: GameState, context: Dictionary = {})`——同值早退幂等保留 + 赋值 + `_state_context` 存储 + emit（**signal `state_changed` 保留单参签名防 hud 回归**，F2 执行偏差维持；context 经新查询接口 `get_state_context() -> Dictionary` 读取）；8 处调用点（:184/:209/:229/:265/:309/:419/:472/:497）`_set_state(` → `_transition(`，按需传 context（如 SHOP 带 `{"from_battle": _shop_from_battle}`）；验证：grep `_set_state` 零残留 + 白盒驱动 MENU→BATTLE 断言 context 透传
- [ ] **F3-T2【W1】GM 正交维度归一（T-032 · 批次 A）**：局状态四维（current_state × route.is_empty × _shop_from_battle × is_boss_wave）→ ① 派生查询接口 `_is_route_mode() -> bool`（= not route.is_empty()）替代 :182/:223/:252/:368/:486 五处裸判断；② `is_boss_wave`（置位 :205/:207 · 复位 :200/:499）/ `_shop_from_battle`（置位 :254/:418 · 复位 :505）赋值点收敛 → 移入 `_transition` 相邻的 BATTLE/SHOP 转换点统一赋值，禁散落；③ 语义注释入 CODE_STYLE（正交维度 = context 承载，非独立状态）；验证：grep `route.is_empty()` 仅 _is_route_mode 内 + 行为探针（旧制/路线制两路径状态流转一致）
- [ ] **F3-T3【W1】route 节点类型枚举化（T-036 之 GM 侧 · 批次 A）**：game_manager.gd:294-311 `match node_type:`（"battle"/"elite"/"boss"/"shop"/"event"）+ :417 `prev_type == "battle"/"elite"` → 新建 `enum RouteNodeType { BATTLE, ELITE, BOSS, SHOP, EVENT, UNKNOWN }` + 转换纯函数 `route_type_from_string(s: String) -> RouteNodeType`（数据层字符串→枚举单点转换，未知值 UNKNOWN + push_warning）；消费点 match 改枚举；**数据来源 routes.json 不变**（仅代码侧收敛）；验证：grep 状态赋值处字符串 match 零残留
- [ ] **F3-T4【W1】enemy Boss 阶段机 → 阶段枚举 + 状态表（T-033 · 批次 B）**：`_current_phase_idx: int`（:155）→ `enum BossPhase { P1, P2, P3 }` + `_phase: BossPhase` + PHASE_TABLE 状态表（per TECH_DEBT_PLAN §8.5 范式：skills/weights/ai_interval 由 phases 数据构建或数据驱动）+ `_transition_phase(next: BossPhase)` 统一入口（同值早退 + 赋值 + 进入钩子：现有 `_reset_boss_phase` 侧写保留）；`_check_phase_transition`（:578）/`_reset_boss_phase`（:586）内部改枚举；**⚠️ 债清单 T-033「4 个并行 bool」过时（实测无）→ 收口时登记修正**；验证：day18_19_boss_check 48/48 断言零改动（行为等价）
- [ ] **F3-T5【W1】enemy `_is_dying` 冗余删除（T-035 · 批次 B）**：:126 `_is_dying: bool` + :173/:177/:199/:265 守卫 `if not is_alive or _is_dying:` + :797 置位 → 删 `_is_dying`，守卫改 `if not is_alive:`（die :795 is_alive=false 后即进入死亡态，重复 die 由 is_alive 拦截）；验证：grep `_is_dying` 零残留 + 死亡流程探针（die 幂等/清理/掉落单次）
- [ ] **F3-T6【W1】player 行为态枚举化（T-034 · 批次 B）**：`_is_walking`（:114）归并 → 新建 `enum PlayerState { IDLE, WALK, ATTACK, SKILL, HIT, DEAD }` + `_state: PlayerState` + 动画映射表 `const ANIM_MAP := {PlayerState.ATTACK: "attack", PlayerState.SKILL: "skill", PlayerState.HIT: "hit", ...}` + `_transition_state(next: PlayerState)` 统一入口（进入钩子侧写 `_play_attack_anim/_play_skill_anim/_play_hit_anim` :336/:344/:361）；**`_last_stand_active`（:105）= F-13 low_health 机制标志，不进行为态，保留**；**`_facing_left`（:116）= 朝向，保留**；验证：day29_elin_anim_check 14/14 + day29_attack_check 20/20 零改动（动画行为等价）
- [ ] **F3-T7【W1】audio int 字面量 → GameState 枚举（T-005/T-036 · 批次 C）**：audio_manager.gd:90-95 `match state:` 的 `0 / 1, 2, 3 / 4` → `GameState.MENU / GameState.BATTLE, GameState.SHOP, GameState.ROUTE_SELECT / GameState.GAME_OVER`（GameState 为 GameManager Autoload 枚举，audio 侧引用常量）；验证：day24_audio_check 14/14 零改动 + BGM 状态机行为等价
...
- [ ] **F3-T9【W1】状态流探针（批次 C · 新建 `tools/day30_f3_flow_check.gd`）**：白盒驱动 GM 状态流转 ≥14 断言——合法序列 MENU→ROUTE_SELECT→BATTLE→SHOP→BATTLE→GAME_OVER→MENU（逐一 `_transition` + 断言 current_state 与 context）；非法序列拒绝（如 GAME_OVER→BATTLE 跳态 → 断言早退/push_warning 不崩）；route 模式 vs 旧波次制双路径等价；state_changed 信号次数/值核对
- [ ] **F3-EXIT【W5】批次 C 尾收口**：grep 验收三连（状态切换单一入口 / 无字符串状态 / 无新增 bool 标志）+ 双探针全绿 + 回归 **35 件套 ≥866 + day30_f3_compliance + day30_f3_flow** + baseline **BASELINE CLEAN** + TECH_DEBT_ISSUES T-031~036 转已收口
...
## Boss 技能与效果系统（2026-08-12 用户拍板 · 规格 docs/BOSS_SKILL_SPEC.md · 📋 已拆解 2026-08-13 第 45 轮 · **方案已定：SOLUTION_PLAN.md 第 20 轮**）
...
- [ ] **BS-A1【W2】elements sheet 升级为 effect 表（§4.3）**：docs/GameData.xlsx elements sheet 字段升级（id/type 即时·持续/duration/tick_interval/value/scaling_attr+ratio/target_attr/max_stacks/icon/vfx/sfx），现有 5 元素状态（fire/ice/lightning/poison/plasma）字段映射进统一结构**数值不变**；data_schema.py 注册映射；excel_export.py 重生成 elements.json（结构升级，数值零漂移断言）
- [ ] **BS-A2【W1】通用 `StatusComponent` 抽取（§6.2-2）**：新建 `scripts/systems/status_component.gd`（Node 组件：效果实例列表 + tick 循环 + **O1 叠加规则**（同源刷新不叠层 / 异源独立实例各自 tick / max_stacks 上限）+ 到期还原 target_attr + 查询剩余秒数/层数接口）；enemy.gd:257-290 状态机迁入组件（行为等价）；skill_controller.gd:99-160 燃烧 dps 并入（删重复口径）；player 挂组件（玩家/Boss/怪共用同一组件）
- [ ] **BS-A3【W1】统一施加入口 `apply_effect(source, target, effect_id, params)`（§6.2-3）**：新建 `scripts/systems/effect_apply.gd`（或 StatusComponent 静态入口）：武器特殊效果 / 被动 / 玩家技能 / Boss 技能全走它；weapons.json:1756「施加中毒」从文本升级为结构化 effect 引用（Excel weapons 表对应列改）；**O2 软控运行时补齐**：减速（改 move_speed）/ 麻痹（禁行动）/ 减防（改 defense）三类型真实落地
- [ ] **BS-A4【W1】HUD 玩家状态栏（O4）**：hud.gd 玩家状态栏实时显示 StatusComponent 剩余秒数/层数（可读性原则）
- [ ] **BS-A5【W1】探针 `tools/day30_effect_check.gd`（≥16 断言四段 · §11 验收 3/4/5 机器侧）**：DoT 跳数符合 interval / 同源刷新不叠层 / 异源独立各自 tick / max_stacks 上限 / 到期移除并还原属性（减防恢复）/ 三类型软控行为（减速·麻痹·减防）/ 免疫表（硬控免疫软控保留）
- [ ] **BS-A-EXIT【W5】回归**：35 件套 ≥866 + day30_effect ≥16 + baseline **BASELINE CLEAN**（StatusComponent 抽取不动行为，回归前序探针零改动预期）
