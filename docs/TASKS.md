...
### F4 上帝脚本拆分【已拆解 2026-08-14 第 46 轮 · 待执行 · 方案已定（SOLUTION_PLAN.md 第 21 轮）】（T-047/048：enemy1097/player732/GM686 拆分，目标 <400 行，数值快照零漂移验证 · TECH_DEBT_PLAN §4 F4）
...
### F5 回归与收口【未开始 · F4 收口后按 TECH_DEBT_PLAN §4 拆】（全量回归 + 策划改数手册 CODE_STYLE/DATA_DICT_GUIDE）

> ✅ **方案师第 22 轮（2026-08-15 00:1x · 等待任务拆解确认）**：F4 已收口（`dc77e47`→`0551cd9`，TEST_REPORT #46 = 46 件套 · 1061 断言全绿）→ **F5 拆解窗口已开**（TECH_DEBT_PLAN §4 F5：回归复跑 + 性能对比 + **docs/DATA_DICT_GUIDE.md 策划改数手册 = 唯一新交付物** + 新功能恢复门槛；CODE_STYLE.md 已由 F3-T0 建成）——**待 #2 第 47 轮拆解，本轮不标「方案已定」**；F4 遗留登记 = GM 686→623 行 <400 未达（T4/T5 拆 63 行，放宽判据 or 续拆交 #1 裁决）；玩家侧技能系统（08-14 20:0x 用户拍板 D1-D7，规格 docs/PLAYER_SKILL_SPEC.md）= 新功能恢复门槛后首个 P0 级大块亦待 #2 拆解；F1-E 🏠 主窗口承接维持。预研要点见 SOLUTION_PLAN.md 第 22 轮。
...
## G 系列：框架拓展（2026-08-12 用户拍板 · 规格唯一来源 docs/FRAMEWORK_EXPANSION.md · 📋 已拆解 2026-08-14 第 46 轮 · ⏰ 动工窗口 = 08-13 18:00 后已过 → 解锁执行 · 方案已定（SOLUTION_PLAN.md 第 21 轮））
...
## 玩家侧技能系统（2026-08-14 用户拍板 · 规格唯一来源 docs/PLAYER_SKILL_SPEC.md · 📋 已拍板待 #2 拆解 · 未拆解前禁动工）

> 📌 **登记（2026-08-14 20:0x · 用户会话 · D1-D7 全部拍板）**：用户 19:1x-19:4x 连续讨论定案——**多技能位 3 槽（空格 + 鼠标左键 + 鼠标右键，弃数字键）** / **无敌帧不写死（效果表 invulnerable 类型动态调）** / **章节化 4 章（章 1 无 Boss，章末=事件休息+奖励；层数建议 3/4/4/4）** / **精英掉落 80%（桌游成功率口径，20% 替代奖励）** / **章 Boss 招牌技必掉** / **局外等级奖励门槛动态可配（Excel 表）** / **D7 掉落物角色差异化（Boss 掉"技能核心"物品 → per_character 变体：法师=光柱/剑士=剑气爆发，新表 skill_relics）**；玩家版数值口径：telegraph 归零 / 半径 ×0.6 / CD ×1.5-2 / 伤害挂属性缩放。附带反馈：**剑士星刃主动技能"挺垃圾的"→ D7 落地时替换为剑气爆发**（交 #5 追踪区同步）。**待 #2 拆解建议分块**：①多技能位+键位路由（skill_controller 列表化 + input map 鼠标双键 + HUD 3 格 + UI 消费输入防误触）②SkillExecutor 新类型（dash/blink/leap/spawn/buff）+ invulnerable 效果 ③skill_relics 掉落表 + per_character 变体 + 三选一装配（升级面板同款）④章节化 routes chapter 字段 + 章末事件节点 + 大地图章界 ⑤局外等级奖励（门槛动态配）。关联 BOSS_SKILL_SPEC（执行器中立复用）+ FRAMEWORK_EXPANSION（大地图承接）。**工作流硬性**：只按规格拆解执行，禁止单条对话动工（08-12 教训）。
