...
- [ ] F1-C 护甲公式统一（T-006：player 平直减 vs enemy 百分比两套 → 统一）【✅ 方案已定：SOLUTION_PLAN 第 18 轮 · **用户 2026-08-10 拍板口径「伤害-护甲=最终伤害」平直减法** → 执行阻塞解除 · 待执行：① enemy.gd :761-763 百分比改平直减 `max(amount-armor,1.0)` 对齐 player（player 零改动=玩家数值零漂移）② stats.formulas armor_reduction/armor_final 死公式标记删/待 F1-G ③ 验证：enemy armor=0 全伤 / armor=3 减 3 / 大 armor 保底 1.0 + player 受击回归锚点（day4/day18_feedback 不变）+ 回归 32 项全绿】
...
- [ ] **F1-G-尾 删数据 3 键落 Excel（T-050 收尾 · #2 第 43 轮拆解 08-11）**【✅ 方案已定：SOLUTION_PLAN 第 18 轮——实测 3 键分布 anvil:574 / bait:351 / mech_heart:683 · items_effects 长表删 3 行（item_id+key）→ excel_export --check-only → 导出 → 回归 32 项 · desc_builder 中文映射 3 处本轮不删（F5 清理）· CONSUMED_BONUS_KEYS 白名单不含 3 键零改动 · T-050 状态 → 已收口，F1-G 行整体转 [x]】：3 键 = **no_weapon_armor_bonus / special_enemies_next_wave / auto_turret_per_wave**（已 grep 确认零代码消费；anvil/bait/mech_heart 为**保留条目**仍有数据引用，非删除对象——TASKS:2363 旧注「3 键删数据（anvil/bait/mech_heart…）」为笔误，以 TECH_DEBT_ISSUES T-050 登记为准）【数据管线铁律：改 docs/GameData.xlsx → `items_effects` 子表删 3 行 effect 键 → `python tools/excel_export.py --check-only`（校验不过禁提交）+ 导出 → data/items.json 更新 → 回归 32 项全绿 + TECH_DEBT_ISSUES T-050 状态 → 已收口，F1-G 行整体转 [x]】
