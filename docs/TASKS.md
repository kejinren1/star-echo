...
- [~] F1-E 表现配置抽表（T-016~024：SPRITE_MAP/BEHAVIOR_MAP/BGM/SFX/FX/SHEET_CONFIG/初始枪/炮台默认 → Excel presentation sheet）【✅ 第一批 SPRITE_MAP 已收口（总指挥第 4 轮 2026-08-18 承接动工：GameData.xlsx 新增 enemy_sprites sheet 23 敌 + data_schema 注册 presentation.json/enemy_sprites + excel_export 构建 + DataLoader.get_enemy_sprite_config 消费（Vector2i/Color/scale 组装 + 未命中按 category 兜底 const）+ enemy.gd 改读；新探针 day31_presentation_check 246/246 全绿；day31_skill_icon_check 22/22（HUD se_skill_sword_arc 图标补丁）；两探针并入 runner 58→60 件套）· **✅ 第二批 BEHAVIOR_MAP 已收口（执行者第 56 轮 `b410a8b`：Excel enemy_behavior 表 9 行 + data_schema 注册 + excel_export 构建 + presentation.json behavior_map 9 条（13 JSON 零 diff）+ data_loader get_enemy_behavior（懒加载+枚举名解析+const 兜底）+ enemy.gd 行为解析改读 + 探针 261/261 + 回归 60/60）**· **✅ 第三批 BGM/SFX 已收口（总指挥第 5 轮 2026-08-18：audio_config 12 行抽表 + get_audio_config + audio_manager._resolve_audio_path 消费改读（const 兜底）+ 探针 273/273 + 回归 61/61 1504 断言 + baseline CLEAN，见下方 F1-E-3）**· **✅ 第四批 FX 已收口（2026-08-19 #3 执行者 `f67630b`：F1-E-4-1~EXIT 全闭环——fx_config sheet 10 行 → get_fx_config（懒加载+Vector2i 组装）→ vfx_player.set_effect 改读（FX_CONFIG const 兜底 = day23_vfx_check 18/18 零改动）+ 探针 +§6 fx 段 13 断言 286/286 + runner +portal 24 → **回归 63/63 1578 断言** + baseline CLEAN + T-019 转已收口，见下方 F1-E-4）**· **✅ 第五批 SHEET_CONFIG 已拆解（2026-08-19 第 63 轮 · 承接方 = #3 执行者，见下方 F1-E-5）**· **✅ 第六批 初始武器 + SKILL_ICON_MAP 已收口（2026-08-19 #3 执行 `38f7c2e`/`80fc19d`/`3b3aff8`/`20b03f9`/`c2b5a0e`：T-004 starting_gun 抽表（8/0.4/180/knockback0/max1 ranged，两键不进表）+ 消费端改读（补设 360/1.5 + remove_meta 保 day13 硬门槛 + shop.gd 排除初始枪防商店池 +1）+ T-022 skill_icon_map 独立表 5 行 + get_skill_icon_index + hud 两消费点改走（const 兜底），探针 316/316 + day7/day8 豁免占位 + 回归 59/64 挂 D-26，见下方 F1-E-6）**· **✅ 第七批 炮台默认值已拆解（2026-08-19 第 65 轮 · 承接方 = #3 执行者，见下方 F1-E-7：T-024 turret.gd:13-15 字段默认 + setup 兜底两处散落 → turret_config 表，TURRET_DEFAULTS const 兜底 = day13 炮台段 6b 零改动硬门槛；duration 属 skill 域不拆入，weapons se_auto_turret duration=20 未消费键登记观察）**· **F1-E 批次全 7 批拆解齐备（批一~七已收口 · 阶段 F 全闭 🎉），承接方 = #3 执行者**（用户 2026-08-18 23:1x 拍板解除「#3 勿自行开工」历史约定，拆解+方案齐备即 #3 直接执行，每批 Excel→导出→消费→探针→回归，const 兜底）】
