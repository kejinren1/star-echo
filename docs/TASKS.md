...
- [ ] **LD-B1【W1】enemy_spawner 按点位生成**：`_get_random_spawn_position()`（:172）→ 新增 `_get_spawn_position(entry: Dictionary) -> Vector2`（**表驱动主路径**：读 `get_spawn_set(wave)` → 点位队列**按 spawn_order 轮换**（sequence = 数组循环 index 递增 / random = 组内随机；单只连续生成多只时 index 推进防同一角落堆叠）；点位解析——edge：按 direction 映射竞技场边缘 + inset 内缩（仿 F-44 `_clamp_to_arena` 边界钳制）；anchor：x/y 比例 × 竞技场尺寸；ring：center + radius 圆周均分（按已生成数取角）；仍过 `min_dist_player` 兜底（尝试 3 次换点，仍近原样生成不静默丢弃——沿用现兜底语义）+ `_clamp_to_ground` 钳制；**缺省回退**：spawn_set 空/点位表空/point_id 不存在 → 保留现 `_get_random_spawn_position`（±200×±120）兜底零回归；**F-48 修复不回归硬门槛**（生成点必在视野内：点位 inset 40 边缘 + Aggro Leash 320 双保险，day31_flee_bound 18/18 零改动）。**测试点**：白盒直构造点位表 → 各 type 解析正确（edge 内缩 / anchor 比例 / ring 均分）；sequence 轮换 index 推进；缺省回退走随机路径；`_clamp_to_ground` 兜底保留
- [ ] **LD-B2【W1】wave_manager 透传 spawn_set**：`spawn_wave`（wave_manager.gd :70 附近调用链）透传 wave 的 spawn_set/spawn_order 给 enemy_spawner（spawn_wave 参数扩展 Dictionary 缺省空 → 兼容旧调用零回归；day14_15/精英/Boss 各路径统一走透传）。**测试点**：白盒 spawn_wave 带 spawn_set → spawner 收到并解析；不带 → 缺省回退
- [ ] **LD-B3【W1】探针扩展**：day31_level_design_data_check +§出生点段（**≥8 断言**：edge 点位生成位置在竞技场内且 inset 生效 / anchor 比例换算正确 / ring 圆周均分角度正确 / sequence 轮换 index 推进不重叠 / random 组内随机 / 缺省回退随机路径 / F-48 视野内硬门槛（生成点距玩家 ≤233.5 对角线）/ _clamp_to_ground 兜底）。**回归硬门槛**：day31_flee_bound 18/18 零改动（F-48 修复）+ 62 件套
- [ ] **LD-B-EXIT【W5】收口**：回归 62 件套 1534 断言 + day31_level_design_data_check 全段 + baseline **BASELINE CLEAN** + PLAYTEST 主观项登记（怪从视野边缘涌入的可预判感，交 #5）
