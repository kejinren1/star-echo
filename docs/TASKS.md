...
### F5 回归与收口【已拆解 2026-08-16 第 47 轮 · 待执行 · 方案已定（TECH_DEBT_PLAN §4 F5 + SOLUTION_PLAN.md 第 22 轮预研）】（全量回归 + 性能对比 + 策划改数手册 DATA_DICT_GUIDE.md = 唯一新交付物 · TECH_DEBT_PLAN §4 F5 · git 收口 f5-stabilize）
...
- [ ] **F5-T1【W5】全量回归复跑（批次 A）**：47 件套 ≥1046 断言（含 day31_spawner_deadlock_check 7 新断言 + F4 拆分后全链）一键跑通 + baseline **BASELINE CLEAN**——机器侧回归确认，无新探针预期；⚠️ 须在 items 图集锚点同步（day11_12/day20/day24_f13 25→54 帧 + day26 runner 锚点 46→47）之后执行
- [ ] **F5-T2【W1】性能对比基线（批次 A）**：复用 `tools/day28_perf_check.gd` 7/7（同屏 50 敌逻辑帧 6.88ms≈145fps / 最差 14.9ms / static 53MB）作为性能基线，与 F0 快照对比零劣化登记；真机帧率主观项由 E-0 承接（不新增探针）
- [ ] **F5-T3【W1】docs/DATA_DICT_GUIDE.md 策划改数手册（批次 B · 唯一新交付物）**：覆盖 GameData.xlsx 全部 sheet（characters/weapons/items/enemies/waves/routes/events/elements/boss_skill/boss_pattern 及新表 skill_relics/局外门槛）——每表写「改哪个 sheet → 跑什么命令（tools/excel_export.py）→ 看什么校验（--check-only / manifest 指纹 / 探针锚点）」，与 F1.0 管线（data_schema.py 注册 / json_to_excel.py / excel_export.py）逐一对应；**先读 tools/data_schema.py 注册表为准盘点 sheet 清单**；验收「策划可自助改数」
- [ ] **F5-T4【W1】新功能开发恢复门槛（批次 C）**：提交前过「配置化评审清单」（CODE_STYLE §5/§8 既有：数值走 DataLoader / 状态走枚举+transition / 跨层信号 / 数据改 Excel）；**玩家侧技能系统 = 恢复门槛后首个新功能大块，F5 收口为其开工前置**（排期：F5 → 玩家侧技能系统）
- [ ] **F5-T5【W1】F4 遗留决策登记（批次 C）**：GM 686→623 行 <400 判据未达——登记 TECH_DEBT_ISSUES T-046 收口附注「放宽判据：enemy/player 达标 + GM 相对 F2 首拆 783 行净减 160 行」（GM 已拆 SaveSystem/DebugConsole/EventManager/UIPanelFactory 四组件，继续拆边际收益低）；交 #1/Owner 裁决，不阻塞 F5 其余项
- [ ] **F5-EXIT【W5】收口（批次 C）**：DATA_DICT_GUIDE.md 落盘 + 回归 47 件套 ≥1046 + baseline **BASELINE CLEAN** + TECH_DEBT_ISSUES 债清点（T 编号残留归零）+ F5 标题 [x] + 阶段 F 全闭（F1-E 维持 🏠 主窗口承接）+ PLAYTEST 主观项登记（E-0 终审完整局 + F-37 G 系列新 UI 交 #5）
...
## 玩家侧技能系统（2026-08-14 用户拍板 · 规格唯一来源 docs/PLAYER_SKILL_SPEC.md · ✅ 已拆解 2026-08-16 第 47 轮 · 未拆解前禁动工 · 拆解完成即解锁 · 待执行）
