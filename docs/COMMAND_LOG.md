...
## 2026-08-18 23:3x · 第 6 轮续 3（用户对话指令：反馈专员降频执行）

### 决策 D-018：反馈专员自动化优化落地（用户「好，可以去做」拍板）

- **内容**：按审批结论 + 用户批准，调整 RS-反馈专员（automation-1786034269281）：
  1. **频率 2h → 4h**（rrule FREQ=HOURLY;INTERVAL=2 → INTERVAL=4，每日 9 轮 → 约 5-6 轮，跳过高峰班次不变）；
  2. **空转零产出规则**（prompt 新增，优先级仅次于高峰空转）：每轮先只读判定「无新增 🟠/🔴/🟡 反馈项 + 无已落地修复待登记 + TEST_REPORT 无新轮次」三项全无 → 零产出空转（不写文件、不 commit、不产生增量编号、不写 PLAYTEST），仅输出一行「空转：无待处理反馈（省 token）」；
  3. **next_run_at 手动修正**：automation_update 改 rrule 后返回的 next_run_at（00:37）仍是旧 2h 频率计算值（已知坑）→ 手动 UPDATE automations 表 next_run_at = last_run(22:38) + 4h = **次日 02:38**，4h 节奏自本轮起生效。
- **预期收益**：反馈专员每日轮次减半 + 无反馈轮零产出，回执类 token 消耗 ≈ 降 60-70%；反馈响应延迟 ≤4h（用户直派仍由主窗口/总指挥即时处理，不受影响）。
- **结果**：配置已生效（ACTIVE）；✅ 风险点 = 增量编号稀疏化（其他岗读 PLAYTEST 追踪区不依赖编号连续性，已确认无影响）。

---

## 2026-08-18 23:3x · 第 6 轮续 4（用户对话指令：代码复用优先全员铁律）

### 决策 D-019：代码复用优先铁律（用户 23:3x 拍板「代码优先从已有方案/社区资源/已有资料里找，不要轻易重新写」）

- **内容**：给**所有实际写代码的岗位**立规矩——实现任何功能/写任何代码（含探针），**优先复用、禁止轻易重写**：
  1. **项目内已有实现**（scripts/ tools/ addons/ 既有函数、容器工厂、状态信号、数据驱动读法）优先调用或参考；
  2. **社区资源**（Godot 官方文档与示例、Asset Library、addons、gdscript-codegen/godot-tscn-format 等技能模板、GitHub 开源实现）优先查找采用；
  3. **已有资料**（docs/ 规格、既有探针范式、TECH_DEBT 记录）优先对齐。
  4. 只有确认现有方案确实不满足需求时才允许从零写；**从零写之前必须在回复中说明「已检索哪些现有资源、为何不可用」**。
- **落地岗位（3 个写码岗 prompt 已同步更新）**：#3 执行者（automation-1786034260084）新增【代码复用优先】纪律段 + 执行步骤 2 内嵌「实现前先检索复用」；#4 自动化测试（automation-1785854507532）新增同纪律（探针沿用 day*_check 范式、回归套件在既有 62 件套上增量）；总指挥（automation-1786971635885）新增【代码复用优先·全员铁律】段 + 审批 3b 新增「复用审查」（未检索即重写 = 质量瑕疵）。
- **理由**：项目已积累大量可复用实现（容器工厂/状态信号/数据驱动/探针范式），重写=重复造轮子 + 回归风险 + token 浪费；用户明确要求先检索再动手。
- **结果**：✅ 三个写码岗 prompt 全部生效（ACTIVE），决策本条目 = 单一事实源；后续任何岗位从零写码须附检索说明，总指挥审批时据此审查。

---

## 2026-08-19 03:0x · 第 7 轮（03:00 夜间自检轮 · 回归阻塞处置 + 审批）

### 决策 D-020：回归 5 FAIL 挂账处置（用户会话在途 API 误用，不代修）

- **事实**：#3 第 64 轮（`3509b74`）登记回归阻塞 + #4 TEST_REPORT #63（02:00）独立验证一致——day2/day3/day5/charsel/player_model 五探针 FAIL，根因 = **用户会话在途 D-26 动画改动**（`sprite_frame_factory.gd:92` / `player_anim.gd:99,101` 调 `set_frame_offset/get_frame_offset` = **Godot 4.4 API，项目 4.3 引擎无此方法**）→ 每探针 4 SCRIPT ERROR（断言 0 失败）。#4 报 58/63 vs #3 报 59/64 = runner 件套数差异（63 vs 64），同批 FAIL 无矛盾。
- **决策**：**挂账登记，不代修**——文件在用户会话在途（M 状态，红线不碰）；待用户会话收口时由收口方改 **4.3 兼容实现**（帧偏移走 region/offset 属性或动画帧索引，禁用 4.4 独有 API），修复后复跑全量回归（64 件套 1602 断言口径）。
- **影响与应对**：F1-E 批五/批六、RELIC、LD-B/C/E 的 **EXIT 全量回归门槛暂时无法全绿** → 各批照常执行（探针独立验证已过，如 LD-A 24/24），EXIT 统一挂「等用户会话收口复跑」；不阻塞后续批次推进。
- **动作**：登记本条目 + LOOP_HEALTH 第 15 条心跳观察区；日报③④转述。

### 审批（近 6h 提交 · 8 个）

| 提交 | 岗位 | 判定 | 说明 |
|---|---|---|---|
| `96e4cd5`+`3509b74` #3 第 64 轮 LD-A 收口 | #3 | ✅ **实质·高质** | 两新表（spawn_points 11 点位三型 / boss_phase_events 7 事件 6 类型）+ FK 三态校验 + **顺手修 --check-only 只读缺陷**（D-011 老账）+ DataLoader 三接口 + 探针 24/24 + runner 64 件套 1602 锚点 + **主动登记回归阻塞且与 #4 交叉验证一致 = 负责任** |
| `95752eb` #2 第 63 轮批五 SHEET_CONFIG 拆解 | #2 | ✅ **实质** | 函数级拆解（icon_config sheet 3 行 → excel_export 第 6 根键 → get_icon_config → IconAtlas 改读 + §7 icon 段 13 断言），static 类经 Engine.get_main_loop 访问 Autoload 新范式标注 |
| `681ab36`+`f67630b` #3 第 63 轮批四 FX 收口 | #3 | ✅ **实质·高质** | get_fx_config 懒加载 + vfx_player 改读 + §6 fx 段 13 断言 + 回归 63/63 1578 断言 + T-019 收口 → **F1-E 4/7 批** |
| `9ebbf5f` #2 第 62 轮 LEVEL_DESIGN 拆解 | #2 | ✅ **实质** | 用户 22:57 拍板新规格批 A→B→C→E→D 函数级拆解，承接 #3 |
| `1ccb419`+`e49eb5b` #1 第 66/67 轮 | #1 | 🟡 状态回执（职责内） | #1 本职=记录与摘要刷新，数值有实质更新（4/7 批 / 用户会话在途登记），密度可接受 |
| 反馈专员 02:38 轮零提交 | #5 | ✅ 符合 D-018 | 无新反馈项 → 空转零产出规则不写不 commit = **预期行为**，非空转异常 |

- **复用审查**：LD-A 数据地基仿 F1-E 批四 fx_config 先例（excel_export 注册 + 懒加载 + FK 校验），--check-only 修复为顺手收老账，**未发现未检索即重写**。
- **结论**：近轮无伪产出；执行者连续 2 轮（63/64）实质产出，token 价值高；#2 拆解密度优；#1/反馈专员行为符合职责设计。
